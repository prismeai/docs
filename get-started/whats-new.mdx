...
<Update label="Latest Release" description="Releasing version 25.12.011029">
...
- Backends: Implemented passwordless authentication using Azure Entra ID for both PostgreSQL and Redis
- api-gateway: Added a text parameter to the /contacts/search API to search across id / email / firstName / lastName
- AIG: Search input now queries across id / email / firstName / lastName instead of only emails (Internal Ticket #3477)

### Bugfixes
- AIK: Fixed options.excludeParameters with Azure providers
- AIK: Fixed cases where Canvas content was displayed in the chat in addition to the Canvas
- AIK: Fixed translated agent categories not displaying correctly on the home page
- AIK: Fixed non-streaming /query API calls that could incorrectly switch to streaming mode when any tool was invoked
- AIK: Fixed a crash in the code interpreter for Anthropic models when combined tool calls occurred
- AIG: Adding a workspace as a suggestion no longer removes its labels, and removing it only removes the suggestions label
- Vector Store: Fixed getNearbyChunks returning each chunk twice (Internal Ticket #3484)
</Update>

<Update label="v25.11.241042" description="Releasing version 25.11.241042 (Historical release)">
### Features
- **DataTable**: Add sortable attribute to allow sorting DataTable from events

### UI
- **AIS**: Display a loading while img is streaming
- **Builder**: New TimeList page block
...
- **Builder**: Fix "Automation not found" error event that was missing in workspace activity feed
- **AIS**: Fix markdown display in AgentMessage responses
</Update>
