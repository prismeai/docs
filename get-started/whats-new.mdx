...
<Update label="Latest Release" description="Releasing version v25.12.161451">
### Features

- **AI Store:** Conversations history in sidebar rewritten for better performance and stability.
- **AI Store:** Agents now have a home view without creating an empty conversation before the first message.
...
- **Crawler:** Reduced default polling interval (3s instead of 5s).
- **Crawler:** Added ability to override Unstructured OCR configuration via parsers configuration.
- **Crawler:** Allow forcing skip_ocr=false even if environment defaults set it to true.
- **UI / UX: Studio / Pages:** Fix display of backtick (`) character in input fields by switching to Fira Code font.
- **UI / UX: Studio / Pages:** Language selector added to user menu.
<Frame>
  <img src="/gifs/newlanguageselector.gif" alt="Language Selector" />
</Frame>
- **UI / UX: Studio / Pages:** Updated link colors in news section.
- **UI / UX: AI Store:** Display a proper loading state while streaming images.
- **UI / UX: AI Store:** Agent home no longer creates a conversation until the first user message is sent.
- **UI / UX: AI Knowledge:** A popover can now be opened directly from the AI Knowledge project to chat with the agent.
<Frame>
  <img src="/gifs/newpopover.gif" alt="AI Chatbot Popover" />
</Frame>
- **UI / UX: Global**: Fix typo in French welcome popup.
- **Infrastructure & Operations:** Helm – prismeai-core: Enable prismeai-events horizontal pod autoscaling by default.
- **Infrastructure & Operations:** Helm – prismeai-apps: Increase default resource requests and limits to production recommendations.
- **Infrastructure & Operations:** Crawler: Set default Apache Tika server timeout to 10 minutes (instead of 5 minutes).
- **Security:** All services: Dependencies security upgrades.
...
- **AI Store:** Fix display issues when streaming tags.
...
- **Studio:** Fix refresh of products list when platformRole is updated.
