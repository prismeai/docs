...
<Update label="v27.6.3-hotfix" description="Hotfix for version v27.6.3">

**Bugfixes:**
- **AI Store:** Fixed every conversation failing to start since v27.6. Four services that AI Knowledge, AI Store, AI Insights, AI Collection and AI Governance depend on (Conversations Service, Conversations Service App, Teams Service, Teams Service App) were missing from the v27.6 release, so a platform upgraded to v27.6 kept a version of them incompatible with the new runtime.

**Upgrade:**
- Only the `prismeai-workspaces` image changes: set its tag to `v27.6.3-hotfix` and keep every other service on `v27.6.3`. See [Updates → Hotfix images](/self-hosting/operations/updates#hotfix-images).
- Then re-run the platform pull of the `base2` group. The four workspaces are reported as `imported`, everything else as `skipped`.

</Update>

## Previous Releases

<Update label="v27.6.3" description="Releasing version v27.6.3">

**Improvements:**
- **Knowledges:** A long agent description is now shortened in the playground, so it no longer clutters the interface.
- **api-gateway:** Clearer `Failed authentication from provider …` error logs after a failed SSO login. See [Authentication → Troubleshooting](/self-hosting/entreprise/authentication#troubleshooting).

**Bugfixes:**
- **Connectors > Confluence:** Fixed browsing your own spaces when creating a knowledge connection, which now also works when the instance keeps legacy OAuth parameters alongside the platform authentication. See [Confluence](/apps-store/marketplace/connectors/confluence).
- **AI Governance:** Fixed the invitation link and code not being copied properly. See [Identity & Access → Creating an Invite](/products/ai-governance/identity-access#creating-an-invite).

</Update>

<Update label="v27.6.2" description="Releasing version v27.6.2">

**Features:**
- **Knowledges:** Outlook can now feed a knowledge base: pick a mail folder as the source of a connection, exactly as you would a SharePoint site or a Google Drive folder. See [Connectors](/products/ai-knowledge/connectors).
- **Collections:** The Collection app has a brand new configuration app, displaying a collection's data and filtering it from a plain-language question translated to SQL. See [Collection](/apps-store/marketplace/collection).
- **Agentic RAG:** The MCP server URL is now exposed in AI Collection and in Knowledges, and auto-filled when you pick a knowledge base. See [Advanced RAG → Agentic RAG with the Filesystem MCP](/products/ai-knowledge/advanced-rag#agentic-rag-with-the-filesystem-mcp).
- **Builder > Activity feed:** A **Trace** tab renders the full execution trace when a correlation id filter is active: a collapsible call tree, error navigation, and the detail of each step in readable and raw form. See [Activity → Trace mode](/products/ai-builder/activity#trace-mode-correlation-id).
- **Agent Factory:** A **View trace** action on each answer opens its detailed execution trace — the automations that ran, timings, retrieved passages, tool calls and errors. Available in both the Playground and Chat, for users holding the `agent-factory:traces:read` permission, granted to organization Admins by default. See [Playground → View trace](/products/agent-factory/playground#view-trace).
- **Embed:** The chat widget now runs as an isolated iframe — no remote code, MV3-safe — with a programmatic API and a `customCss` option. **Breaking:** host page CSS no longer styles the widget, use `customCss` and `cssVariables` instead. See [Embed](/products/agent-factory/embed).
- **LLM Gateway:** The identity headers Prisme.ai sends to LLM providers can now be made mandatory, either in observation mode or with a strict rejection, through `header_policy`. See [Model Governance](/products/ai-governance/model-governance).
- **LLM Gateway:** Additional body parameters can now be configured for every provider.
- **Automations:** Custom headers starting with `x-` can now be sent to automations exposed as webhooks, and used by them. **Breaking:** headers starting with `x-prismeai-` are now always removed before the automation runs, `x-prismeai-workspace-id` included. An automation reading `{{headers['x-prismeai-workspace-id']}}` to identify the calling workspace must read `{{run.sourceWorkspaceId}}` instead. See [Automations → Triggers](/products/ai-builder/automations#triggers).
- **Workspaces:** New `POST /v2/workspaces/{id}/files/restore` endpoint, recovering deleted files from storage versioning (S3, GCS, Azure blob soft-delete) and recreating their database entry. Requires object versioning enabled on the storage backend. See [Files → Restore a deleted file](/products/ai-builder/files#restore-a-deleted-file).
- **prismeai-runtime:** New opt-in security audit event `runtime.fetch.requested`, recording the outbound fetch URL, method, header names and body size — never a body or a header value. Enabled with `FETCH_EMIT_REQUEST_EVENT=true`. See [Environment variables](/self-hosting/kubernetes/environment-variables).
- **PostgreSQL:** PostgreSQL can now be isolated by schema, through a `?schema=<name>` parameter on the connection URL. This runs Prisme.ai on a managed Postgres where the database user has no `CREATEDB` right. See [PostgreSQL → Single-database deployments](/self-hosting/databases/postgresql#single-database-deployments-schema-isolation).
- **Runtime:** `splitText` now returns the `startChar` and `endChar` character offsets of each chunk. See [Text Module → splitText](/products/ai-builder/module-text#splittext).
- **Auth:** OIDC profile claims can now be read from the provider's `userinfo` endpoint, for IdPs that keep them out of the `id_token`. Opt in with `userinfo_endpoint`. See [Authentication → Reading profile claims from the userinfo endpoint](/self-hosting/entreprise/authentication#reading-profile-claims-from-the-userinfo-endpoint).

**Improvements:**
- **Knowledges:** A documentary connector can now feed any knowledge base you have the right to write to, not only the ones you created yourself: bases shared by a colleague, where you are administrator or editor, are accepted at configuration and at sync. See [Connectors → Pick the destination](/products/ai-knowledge/connectors#4-pick-the-destination).
- **Knowledges:** Your own MCP server can always be plugged in from **Knowledge > Connectors** — a dedicated card sits among the available providers, and **New connection** offers it even with no existing connection or a single connector in the catalog. See [Connectors → Connecting your own MCP server](/products/ai-knowledge/connectors#connecting-your-own-mcp-server).
- **Agent Creator:** An access screen replaces the unusable editor. Opening an agent you are not allowed to modify — typically a published, public agent of your organization, or a link received from a colleague — used to display the full editor, where nothing was modifiable anyway: every save was refused, without explanation. An explicit screen now appears on opening, with a shortcut to open the agent in the chat (what most people were actually looking for) and a way back to the agent list. It distinguishes a refused access from an agent that does not exist, and is translated in the six languages of the product. No right is changed: what was accessible stays accessible, only the information now arrives at the right moment.
- **Agent Factory:** An agent's knowledge bases are now created with the `tika-ocr` parser rather than `tika`, making them more capable out of the box. See [RAG Settings → Document Parsing](/products/ai-knowledge/rag-settings#document-parsing).
- **Agent Factory:** The MCP tool description is now carried through to the approval request, so whoever validates a tool call sees what that tool does. See [Tool Permissions](/products/agent-factory/tool-permissions).
- **Agent Factory:** Hooks now declare a `timeout_ms` on the client side. See [Hooks](/products/agent-factory/hooks).
- **Connectors:** Querying and revoking a connection now targets the connection you own, instead of all of them. See [Connected accounts](/products/agent-factory/connector-accounts).
- **AI Governance & Knowledges:** Model lists are now conditioned by model capability, both when creating a knowledge base with the LLM parser and in the AI Governance default model settings. See [Available Models](/products/ai-governance/available-models).
- **AI Insights:** The Overview "Active Users" KPI is split into **Active Users** (distinct people) and **Active Sessions**, so the audience size is no longer inflated by repeat sign-ins. See [Organization dashboard → The KPIs explained](/products/ai-insights/dashboard#the-kpis-explained).
- **Chat:** The clarification panel height is capped, with an internal scroll.
- **Chat:** The slides PPTX export is now reliable across renderers. See [Deck Designer → Export formats](/products/agent-factory/slides#export-formats).
- **Agent evaluations:** Batch runs are resilient to provider rate limits. See [Evaluations](/products/agent-factory/evaluations).
- **Guardrails:** Guardrails now enforce cross-workspace authentication, the webhook body and the agent model. See [Guardrails](/products/agent-factory/guardrails).
- **Crawler:** Large reliability pass with multiple fixes, making a document lost in transit impossible even when vital components crash. Enable it with `CRAWLER_RELIABILITY_MODE=full`. See [Crawler & Searchengine → Reliability mode](/self-hosting/entreprise/crawler-searchengine#reliability-mode).
- **Crawler:** Spiders can now sleep, significantly reducing the memory an idle `prismeai-crawler` pod needs. Enable it with `CRAWLER_RELIABILITY_MODE=full`. See [Crawler & Searchengine → Reliability mode](/self-hosting/entreprise/crawler-searchengine#reliability-mode).
- **Platform:** Security updates and dependency upgrades.

**Bugfixes:**
- **Connectors > GitLab:** Fixed the user's OAuth connection being lost several times a day. See [GitLab](/apps-store/marketplace/connectors/gitlab).
- **Connectors > Google Workspaces:** Fixed creating drafts and sending emails: the recipient, the subject and the message text are now enough, the technical formatting of the email being handled for you. Subjects holding accented or special characters display correctly. See [Google Workspaces](/apps-store/marketplace/connectors/google-workspaces).
- **Knowledges:** Fixed choosing a SharePoint, Google Drive, Outlook, OneNote or Confluence folder when creating a connection to a knowledge base: it works again for any user signed in with their own account, without having to be a member of the connector's workspace.
- **knowledge-sync:** Fixed a payload that is not the file being indexed as its content.
- **Agent Factory:** Fixed `can_edit` not being returned to the client for the editing surfaces.
- **Agent Factory:** Fixed the error body being dropped on early exits of `messages/send`.
- **Agent Factory:** Fixed the `InvalidVariableName` error occurring in some cases with Google Vertex, which does not return delta indexes during tool call SSE streaming.
- **Agent Factory:** Fixed the agent's last message being duplicated when reloading a conversation after a validation request (`human_ask_clarification`).
- **Agent Factory:** Fixed waiting for documents indexing when there are more than 5 documents.
- **Agent Factory:** Fixed the agent description that could not be cleared, and advanced settings now show the real default values. See [Settings](/products/agent-factory/settings).
- **Agent Factory:** Fixed `human_ask_clarification` not shipping the user first tool on its usage, which impacted web search, canvas and any other cross-conversation tool usage. See [User-first tools](/products/agent-factory/user-first-tools).
- **Google Vertex:** Fixed URL attachments being dropped instead of inlined for Gemini, so Gemini on the Vertex provider now takes images into account. See [Model Governance → Supported Providers](/products/ai-governance/model-governance#supported-providers).
- **Builder:** Fixed the export button disappearing from the Activity feed when the `source.correlationId` filter is used. See [Activity → Exporting the feed](/products/ai-builder/activity#exporting-the-feed).
- **Builder:** Fixed testing an automation failing with "Failed to fetch" after around 60 seconds on long executions.
- **Runtime:** Fixed `fetch.timeout` not being applied on the direct call short circuit. See [Automations → `fetch`](/products/ai-builder/automations#fetch).
- **Crawler:** Fixed one oversized metadata value rejecting a whole document.
- **Themes:** Fixed the cases where a custom app CSS theme overwrote the organization one when switching from an app to another product. See [Platform Customization](/products/ai-governance/platform-customization).
- **api-gateway:** Fixed a failed organization roles read being cached.
- **Governance:** Fixed the cases where a member role could not share an agent because of its cross authorizations.

</Update>

<Update label="v27.5.7" description="Releasing version v27.5.7">

**Features:**
- **Collections:** Declarative, idempotent schema migrations: a collection declares the conversions it needs, and each one runs once, by itself, when the schema is applied. See [Collection → Changing a property type](/apps-store/marketplace/collection).
- **Connectors:** New OneNote App+MCP connector.
- **Chat:** `human_ask_clarification` is now an interactive panel above the input: numbered options, free text, skip, and multi-question flows with progress and a back step. See [Capabilities → Asking the user for clarification](/products/agent-factory/capabilities#asking-the-user-for-clarification).
- **AI Governance:** JSON import and export on Agents Controls, Join Rules and Capabilities, with multiselect on Capabilities. See [Identity & Access → Importing and exporting rules](/products/ai-governance/identity-access#importing-and-exporting-rules).
- **Chat > Home:** Added a search bar to the Home agents page, searching server side across the whole catalog instead of the loaded page. See [Store → Finding agents](/products/ai-securechat/store#finding-agents).
- **Builder:** The Activity feed export now covers every event matching the current filters instead of the events already loaded, fetched page by page with a live count on the button. See [Activity → Exporting the feed](/products/ai-builder/activity#exporting-the-feed).
- **Embed:** Full-palette `cssVariables` theming for the chat widget, as a mount option or a `data-css-variables` attribute, applied inline so a host palette always wins over the embed stylesheet. See [Agent Creator → Theming the embed](/products/agent-factory/overview#theming-the-embed).
- **Workspaces:** A workspace can now authorize its scheduled runs to read and rotate its own secrets, by declaring a role its automations take on for the call that needs it. See [Secrets Module → Reading a workspace secret with no user](/products/ai-builder/module-secrets#reading-a-workspace-secret-with-no-user).
- **App+MCP:** MCP Resources are now authorized by the workspace role rather than by the agent allowlist. See [Connector architecture → Authorization layers](/apps-store/marketplace/connectors/architecture#authorization-layers).
- **Runtime:** Added `encodeUriComponent` and `removeDiacritics` to the text module. See [Text Module](/products/ai-builder/module-text).

**Improvements:**
- **Agent Factory:** Improved the `human_ask_clarification` tool so models better understand how to ask for a clarification, without having to process the current todos first.
- **Agent Factory:** System tools are never gated behind a HITL approval. See [Tool Permissions](/products/agent-factory/tool-permissions).
- **Agent Factory:** Sending a message while the agent waits for a tool approval now denies the pending tools, instead of making the previous message disappear from the conversation.
- **Agent Factory:** Import and export now replace the current API URL with `${PRISME_API_URL}` on export and inject the current one back on import, fixing cross-platform imports.
- **Agent Factory:** Image token estimation is closer to reality.
- **Agent Factory:** Connecting a tool from an agent's tool menu now establishes a connection the agent actually uses, and the displayed state reflects what it has.
- **AI Engage:** Announcement cards say where they come from, carry a "New" tag for 7 days, are ordered most recent first, and no longer repeat the pre-title on the content card. See [Announcements](/products/ai-engage/announcements).
- **AI Governance:** Interactive user autocompletion in the "Invite members" modal. See [Identity & Access → Inviting members](/products/ai-governance/identity-access#inviting-members).
- **Governance:** The platform MFA admin action is now called "Reset MFA".
- **Chat:** An unknown or forbidden agent now shows an explicit error and blocks sending, instead of silently streaming to another agent; a configured home agent that no longer exists reports the error rather than falling back to the first accessible agent.
- **Chat:** Message actions and the timestamp are revealed on touch devices.
- **Connectors:** Every sensitive value is redacted in activity.
- **Capabilities:** Knowledge compatibility can be re-probed without re-saving each capability.
- **Storage:** Elasticsearch and OpenSearch errors are readable.
- **prismeai-events:** Native Elasticsearch `aggPayload` mapping for error codes, fixing the empty workspace "Top errors" chart in some cases and the noisy "Unknown NamedWriteable" error logs.
- **Platform:** Applications can use every icon of the library, loaded on demand: a missing icon no longer takes the whole application down with a "crashed" screen, no icon has to be added to the platform beforehand, and startup downloads 8 KB less.
- **ai-collection:** Handle the MCP behavior of models that return invalid JSON.
- **Platform:** Security upgrades.

**Bugfixes:**
- **AI Governance:** Fixed subscription creation and modification errors on API queries.
- **AI Governance:** Fixed Agents Controls quota limits never being saved, the webhook they were sent to did not exist.
- **AI Governance:** Fixed the models fallback mapping so the left, source model can be chosen directly, instead of clicking "Add mapping" until reaching the right one.
- **Govern:** Fixed admins' access to Govern pages, the `orgs:roles:read` permission was missing from the Admin native role. Creating, updating and deleting an organization role now requires `orgs:roles:manage`.
- **Govern:** Fixed duplicating a role more than once.
- **Govern:** Events are scoped by organization across both `orgSlug` mappings.
- **Agent Factory:** Fixed the "Total agents" card showing the currently loaded agents instead of the total number of agents.
- **Agent Factory:** Fixed non-organization-admin agent editors seeing only published changes instead of the current draft.
- **Agent Factory:** Fixed tracking the current todos after a HITL approval.
- **Agent Factory:** Fixed processing several tool approvals when the tools are executed in parallel, that is several tools sent by the same LLM turn under an "always ask" policy.
- **Chat:** Fixed keeping the initial user message and the agent's clarification question in the same conversation when answering `human_ask_clarification`, instead of opening a second conversation with the answer.
- **Chat:** Fixed a HITL or clarification card disappearing when the user refreshes before replying to it.
- **Chat:** Fixed the canvas refresh on an in-place patch and the version list update, and stopped rendering several cards for one canvas re-emitted within a turn.
- **Chat-embed:** Fixed the agent avatar, read from the agent's configured icon instead of falling back to the default one.
- **api-gateway:** Fixed the token exchange API not resolving the user's organization, which left accounts without membership and 403 on every organization-scoped resource. SSO tied to an organization is now stricter on authentication, and therefore more trustworthy. See [Authentication](/api-reference/authentication).
- **api-gateway:** Reserved organization slugs are rejected on creation and rename, an organization slugged `subscriptions` or `join` was permanently shadowed by the route of the same name.
- **prismeai-runtime:** Fixed the "runtime.yml not found" error on HTTP calls targeting unknown workspaces.
- **prismeai-runtime:** Restored gateway parity in the cross-workspace API key membership check.
- **AI Insights:** Fixed the consumption page crashing on a null metric.
- **Crawler:** Propagate the real Elasticsearch rejection on a `SaveError`.

</Update>

<Update label="v27.5.6" description="Releasing version v27.5.6">

**Features:**
- **api-gateway:** A native client can now open an authenticated web session in the system browser through a single-use ticket, so flows such as connector OAuth work from a mobile app without ever putting its API token in a URL. See [Authentication → Web session ticket](/api-reference/authentication).
- **AI Insights:** Overview, agent Overview and Conversations now share a single date filter, with 7, 30 and 90 day presets plus a free custom range. See [Organization dashboard → The date filter](/products/ai-insights/dashboard#the-date-filter).
- **Runtime:** New `fetch.timeout` option bounding a call that receives nothing: an MCP server that accepts the connection and never answers no longer holds the calling automation for the 300s undici default. Defaults stay unchanged and are configurable with `FETCH_HEADERS_TIMEOUT` and `FETCH_BODY_TIMEOUT`. See [Automations → `fetch`](/products/ai-builder/automations#fetch).

**Improvements:**
- **Agent Factory:** Raised the default budgets of the agent profiles: Workflow and Light Agent now run up to 10 turns on a 100,000 token message budget, Full Agent 30 turns on 300,000, and Orchestrator 50 turns on 300,000. See [Settings → Defaults per profile](/products/agent-factory/settings#defaults-per-profile).
- **Agent Factory:** Cached input tokens are now counted at 0.1x in the message token budget, so replaying the conversation history no longer consumes the whole budget over the turns. See [Runtime Safeguards → Configurable budgets](/products/agent-factory/runtime-safeguards#configurable-budgets).
- **Agent Factory:** Clarified the advanced model and budget settings: "Max Tokens" became "Max Response Tokens" (caps a single reply) and "Token Budget" became "Message Token Budget" (all internal steps of one message), each with an info tooltip. See [Settings → Model Settings](/products/agent-factory/settings#model-settings).
- **Chat:** Now displays an explicit error message instead of the generic "LLM gateway stream returned no data" when the model takes more than 60s to send its first token (default value of the `WORKER_STREAM_FIRST_CHUNK_TIMEOUT` runtime environment variable).
- **AI Governance > Models:** The capability tabs above the model list are now derived from the catalog, and disappear when no model declares a capability.
- **prismeai-workspaces:** Added timing debug logs to S3 gets, enabled with `LOG_LEVEL=debug`.
- **E2E testing framework:** Upgraded the models API testing usage, fixed the cases where SSO login was blocked by the testing browser, and various functional upgrades.

**Bugfixes:**
- **Agent Factory:** Fixed the manual approval of a tool dropping the other tool calls of the same message, which blocked the conversation. See [Tool Permissions](/products/agent-factory/tool-permissions).
- **Agent Factory:** Fixed duplicated canvases, where several v1 artifacts were created for a single requested canvas.
- **Chat:** Fixed the canvas issue occurring when several canvases are requested for edition by the artifact.
- **Chat:** The confirmation and clarification card is now displayed at the end of the message instead of the top.
- **Chat:** Fixed the processed files detail surviving a conversation reload, and kept the activity history whole across human approvals.
- **AI Insights:** Aligned the Usage Rhythm day labels with the data displayed.
- **Knowledges:** `chunkSize` and overlap are now correctly applied: `auto` sends the overlap you chose (and only when you chose one), the overlap slider is bounded by half the chunk size, and the re-index modal seeds itself from the store instead of the default preset. See [RAG Settings → Chunk Overlap](/products/ai-knowledge/rag-settings#chunk-overlap).

</Update>

<Update label="v27.5.5" description="Releasing version v27.5.5">

**Features:**
- **Automations:** Automations can now be exposed as MCP tools directly from their own definition, without duplicating their schema in the connector configuration.
- **Knowledges:** WebDAV servers (Nextcloud, ownCloud) can now feed a knowledge base: connect the connector to your space, pick a folder, and its content is indexed and kept up to date. The connection stays private to whoever created it, nothing is published to the organization's shared catalog. See [Connectors](/products/ai-knowledge/connectors).
- **Builder:** Automations now support conditional schemas, validating both input and output. `arguments` is now deprecated. See [Automations → Input & output schemas](/products/ai-builder/automations#input--output-schemas).
- **Builder > Observability:** Added an organization selector to display metrics for a specific organization and workspace only. See [Builder Observability → Organization selector](/products/ai-builder/observability#organization-selector).
- **Runtime:** New crypto module for hashing, HMAC, base64 and secure random in automations. See [Crypto Module](/products/ai-builder/module-crypto).
- **Runtime:** Added the `applyPatch`, `diff`, `search` and `substitute` functions to the text module. See [Text Module](/products/ai-builder/module-text).

**Improvements:**
- **Agent Factory:** Agents now reply in your interface language when your message doesn't clearly indicate which language to use, for example when you open the conversation with a simple greeting.
- **Prompt Library:** Prompt categories are now persisted through the workspace API key.
- **Platform:** Added tooltips to the agent list filters.
- **prismeai-runtime:** Enforce scope on resource creation in the access-manager `checkAccess`.
- **AI Governance > Observability:** Performance improvements.
- **Platform:** Security upgrades.
- **Helm chart prismeai-core:**
  - `1.12.0`: Added `sidecar` & `initContainers` values to all deployments, and `events.cleanupJobPodLabels` to the prismeai-events deployment.
- **Helm chart prismeai-apps:**
  - `0.7.0`: Added `sidecar` & `initContainers` values to all deployments.

**Bugfixes:**
- **AI Insights:** Fixed the Interactions, Messages per day and Top agents charts to count distinct messages instead of LLM requests. See [Dashboard](/products/ai-insights/dashboard).
  - Days recorded before this change keep the old LLM request counter, so the message total can look much lower than the numbers shown in Messages per day and Top agents for those days.
- **User profile > My usage:** Fixed the "My usage" page to show both the message and LLM request counters; the previous "Messages" counter only showed LLM requests.
- **Knowledges:** Fixed sync errors in knowledge connectors.
- **PostgreSQL:** Fixed the `$inc` operation on a NULL column.
- **prismeai-runtime:** Stopped recreating `runtime.yml` on `workspaces.deleted`.
- **prismeai-platform:** Fixed nginx failing to start (`could not build map_hash`) on deployments with a long console domain. `map_hash_bucket_size` is now baked to 128 in the image, with no per-environment config patch needed.

</Update>

<Update label="v27.5.4" description="Releasing version v27.5.4">

**Features:**
- **Knowledges:** Connectors sync documents from external sources such as SharePoint and Google Drive into a knowledge base, with per-user connected accounts so an agent reads only what each user is allowed to see. See [Connectors](/products/ai-knowledge/connectors) and [Connected accounts](/products/agent-factory/connector-accounts).
- **Knowledges:** Tags can now be set at crawl entry on web auto-discovery, so every discovered page inherits them. See [Crawl a Website → Tag the pages at crawl entry](/products/ai-knowledge/crawl-website#tag-the-pages-at-crawl-entry).
- **Knowledges:** Added tag cards above the document list to toggle page results by selected tag. See [Documents → Tag cards](/products/ai-knowledge/documents#tag-cards).
- **App Vertex:** Added tool calling support for Gemini 3.x models. See [Model Governance → Supported Providers](/products/ai-governance/model-governance#supported-providers).
- **Knowledges:** `max_num_results` can now be configured from an agent, overriding the default of 5 chunks for that agent's searches. See [RAG Settings → Overriding Top K from an agent](/products/ai-knowledge/rag-settings#overriding-top-k-from-an-agent).

**Improvements:**
- **Knowledges:** Improved the uploading and tagging experience overall.
- **AI Insights:** The agent Conversations tab is now gated behind RBAC. See [Conversations](/products/ai-insights/conversations).
- **Agent Factory & Chat:** Connected knowledge bases now follow their activation permission in the chat tool picker.
- **Agent Factory:** Playground attachments are now uploaded to Storage instead of agent-factory.

**Bugfixes:**
- **AI Insights:** Fixed cases where conversations were not analyzed.

</Update>

<Update label="v27.5.0" description="Releasing version v27.5.0">

**Features:**
- **LLM Gateway:** Multi-level quotas, expressed in requests per minute (RPM) and tokens per minute (TPM), with dedicated quota endpoints. Limits cascade over four levels: model, organization (plus a monthly budget), agent, then user by role with per-user overrides.
- **LLM Gateway:** Live usage gauges (used value and percentage, RPM and TPM). Consumption is written through on each call, so it is displayed in real time instead of with a delay.
- **Agent Factory:** Per-agent rate limits, propagated to the LLM Gateway and capped by the retention plan. See [Model Governance → Configuring Agent Models](/products/ai-governance/model-governance#configuring-agent-models).
- **Agent Factory:** New per-conversation usage endpoint, returning the tokens and cost of a given conversation.
- **AI Insights:** New per-user and per-agent analytics axes (6 new v1 endpoints) to see who consumes what, and how much. See [Consumption](/products/ai-insights/consumption).
- **AI Insights:** New `v1/users/:userId` endpoint returning a single user's consumption, broken down per agent and as a daily series. Access is self-only.
- **Governance:** New Platform Ops endpoint and dashboard for cross-organization observability (superadmin), with unique users, a daily timeseries, and periods of up to 90 days.
- **Governance:** Bulk delete inactive or draft agents, based on the filters applied to the Agents tab. See [Model Governance → Agent hygiene and cleanup](/products/ai-governance/model-governance#agent-hygiene-and-cleanup).
- **Governance:** Test image generation models through the image endpoint.
- **Agent Factory:** TLS certificate verification can now be disabled for custom MCP servers, for internal servers using a self-signed certificate.
- **Builder:** Added an "Import a file" button to the workspace Files pane.

**Improvements:**
- **LLM Gateway:** A quota rejection now returns a real HTTP status (429, OpenAI-compatible, with `Retry-After`) instead of a 200.
- **AI Insights:** Storage ingestion embeddings are now attributed to their organization and user, making them visible in analytics and in per-organization quotas.
- **AI Insights:** The conversation status now returns `score` and `insight_id`, so the conversation list shows the score inline along with a link to the full analysis.
- **Governance > Observability:** Simplified and faster, by dropping the expensive `runtime.automations.executed` aggregation in favor of reading pre-aggregates.
- **Governance:** The `user_id` can now be exposed and copied from the interface.
- **Chat:** Improved canvas behavior when partial content changes, for both code and plain text.
- **Storage:** Files uploaded from the chat no longer have an expiration time.
- **Embed:** Cache-busted assets, so clients always load the current embed build, and added message feedback.

</Update>

<Update label="v27.4.3" description="Releasing version v27.4.3">

**Features:**
- **Builder & Governance:** Create a workspace from a template, with validation of the referenced template. See [Workspace Templates](/products/ai-governance/workspace-templates).

**Improvements:**
- **Knowledge:** Tag filters in knowledge base search now match exactly; partial matching is no longer allowed. See [Documents → How tags are matched at search time](/products/ai-knowledge/documents#how-tags-are-matched-at-search-time).
- **Knowledge:** A `filename` tag filter is now automatically converted into `file_name` when the LLM misspells it while calling the `conversation_file_search` tool.
- **AI Insights:** The feedback table now shows the agent name along with its id.
- **Agent Factory:** A toast is now displayed when inviting users from outside the organization to an agent.
- **Builder:** Workspace imports whose app no longer exists are now displayed, so they can be removed.
- **Bedrock:** Properly handle & emit Bedrock SSE error / exception chunks.
- **prismeai-workspaces:** Always log GCS & Azure Blob retry attempts.

**Bugfixes:**
- **api-gateway:** Fixed org members being removed by mistake upon some join rules updates.
- **api-gateway:** Fixed a 500 internal error when importing a members CSV including users created only after they were invited in the organization.
- **prismeai-events:** Fixed the workspace events cleanup API with OpenSearch.
- **LLM Gateway:** Fixed the "The LLM gateway stream did not return any data" error occurring when the first token takes more than 5 seconds to arrive.
- **Google Vertex:** Fixed the handling of Google Vertex streaming errors & the failover upon rate limits (Google Vertex returns stringified errors inside its SSE chunks).
- **Bedrock:** Fixed disabling SSE streaming through `excludeParameters: ['stream']` on Bedrock providers. See [Model Governance → `excludeParameters`](/products/ai-governance/model-governance#excludeparameters).
- **Azure OpenAI:** Fixed model aliases with Azure OpenAI.
- **Govern > Models:** Dropped null fields from the model PATCH payload, fixing the ValidationError raised when the failover model is null.
- **Evaluate:** Fixed cases where content including commas was merging the question with the expected response of the test card.
- **Agent Factory:** Fixed deduplicated RAG search results being misread as an empty file.
- **Knowledge:** Fixed unchanged webpages being shown as still used when recrawling.
- **Knowledge:** Fixed the crawler config synchronization on `knowledge_base` parser change.

</Update>

<Update label="v27.4.2" description="Releasing version v27.4.2">

**Features:**
- **prismeai-runtime:** New `/sys/collections/schema` API to dump collections SQL schema (also works on MongoDB platforms to generate their SQL counterpart).
- **Builder:** Added shared permissions notifications to the overview activity section.
- **AI Insights:** AI Insights admins can now view all agent conversations, not only their own.

**Improvements:**
- **Agent & Workspace sharing:** Improved the UX when sharing with one or more members, including a redesigned sharing modal and the removal of duplicate buttons.
- **Governance:** Exporting users now considers the applied filter, whether it is a text filter or a status; the export is based on the current view.
- **Agent Factory:** Keep the editor & playground on the agent draft until Publish.
- **Agent Factory & Secure Chat:** Surface `user_first` activation for agent skills.
- **Platform & Agent Factory:** Sanitize capability technical names for LLM providers.
- **Agent Factory:** Made `.md` import one-shot to stop spam-clicking from creating many agents.

**Bugfixes:**
- **prismeai-workspaces:** Fixed workspace imports to gracefully handle various slug renamings instead of throwing errors.
- **LLM Gateway:** Fixed a tool calling loop occurring with Bedrock models.
- **Agent Factory:** Fixed discard leaving stale edits in the editor tabs.
- **Builder:** Fixed duplicate event display.
- **Engage:** Fixed the payload sent when clearing optional fields on an announcement.

</Update>

<Update label="v27.4.0" description="Releasing version v27.4.0">

**Features:**
- **Testing:** New open source end-to-end testing framework, based on Playwright & Cucumber, challenging most of Prisme.ai features.
- **Agent Creator:** Added a `get-artifact` system tool so models can retrieve (and then fix, improve, …) previously generated artifacts.
- **Agent Factory, Knowledge, Builder & Governance:** Share with several members at once, either from a checkbox list or by importing a CSV file.
- **prismeai-events:** New `/sys/events/shards` API to troubleshoot Elasticsearch/OpenSearch shards usage & consumption by pattern. See the [Elasticsearch or OpenSearch documentation](/self-hosting/databases/elasticsearch).
- **Evaluate:** Added pagination and a bulk delete action to the agent evaluation section.

**Improvements:**
- **Evaluate:** Added a toast notification on upload success or failure.
- **Evaluate:** Added a confirmation popup when deleting a test.
- **Governance > Observability:** Group & surface errors with no source automation in a dedicated "No parent" row.
- **prismeai-runtime:** Disabled nested variables interpolation.

**Bugfixes:**
- **Platform:** Disabled browser translation to prevent a crash in builtin-app wizards, including Agent Factory.
- **api-gateway:** Fixed enrolling users' MFA before they actually confirmed the OTP enrollment.
- **prismeai-runtime:** Fixed a runtime crash when a chat message includes DSUL-like invalid expressions.
- **Chat:** Fixed sharing the first generated canvas right after the message completion when it included more than 1 canvas.
- **Agent Factory:** Fixed temperature floating values with PostgreSQL.
- **Agent Factory:** Fixed the Hook capacity that wasn't triggering the workspace's automations.
- **AI Governance:** Fixed infinite pending conversation analysis on the agent's conversations.
- **Knowledge:** Fixed the display of Tika-parsed PDF files that wasn't showing the correct number of pages, and hide page numbers when there is no reason to have any.

</Update>

<Update label="v27.3.3" description="Releasing version v27.3.3">
...
- **Automations:** Improved the [`wait` instruction](/products/ai-builder/automations#wait-instruction) so `filters.oneOf` can use any MongoDB-like condition.
...
<Update label="v27.3.2" description="Releasing version v27.3.2">

**Features:**
- **Chat:** Added a model selector directly in the prompt input.

  <Frame>
    ![Model selector in the prompt input](/gifs/model-picker-preview.gif)
  </Frame>

- **Agent:** Star a conversation to pin it to the top of the list.

  <Frame>
    ![Star a conversation to pin it to the top](/gifs/star-conversation-preview.gif)
  </Frame>

- **Home:** Announcement content is now displayed when clicking an announcement on the home page.

  <Frame>
    ![Click a home page announcement to view its content](/gifs/announcement-content-preview.gif)
  </Frame>

- **api-gateway:** New `/v2/login/token-exchange` API to exchange an external OIDC provider `id_token` for a Prisme.ai JWT.
- **Platform:** Shared permission catalog with fine-grained Agent Factory gating.

**Improvements:**
- **api-gateway:** SSO logins are now also linked to existing accounts by mapped email.
- **prismeai-runtime:** Improved stale runtime model detection and automated rebuild, fixing "Automation not found" or outdated automation errors that could occur when prismeai-runtime crashed during an import.
- **Helm chart prismeai-core:**
  - `1.10.2`: Added `secretKeys.user` support for all databases (broker, permissions, events, users, sessions, collections, contexts, documents, searchengines).
- **Helm chart prismeai-apps:**
  - `0.5.17`: Added `secretKeys.user` support for all databases (broker, permissions, events, users, sessions, collections, contexts, documents, searchengines).

**Bugfixes:**
- **prismeai-workspaces:** Fixed a git pull/push crash when injecting user/password directly in the git repository URL with no auth config declared.
- **Governance:** Fixed creating/updating roles when the org has more than 20 roles or API keys.
- **Collections:** Ensure workspace collections are removed on workspace deletion, even when the last Collection app was already uninstalled.
- **Agent Creator:** Fixed the import of expected answers when importing evaluations from a CSV file.
...
- **Chat:** Added a `slides` artifact type with live preview and PPTX/PDF export.
  ...
  <Frame>
    ![Slides artifact: live preview with PPTX/PDF export](/gifs/slides-pptx-preview.gif)
  </Frame>
...
- **Knowledge:** Allow re-indexing a file with per-document parser and chunking overrides.
  ...
  <Frame>
    ![Re-index a knowledge base file with per-document parser and chunking overrides](/gifs/kb-reindex-preview.gif)
  </Frame>
...
<Update label="v27.2.0" description="Releasing version v27.2.0">
  ...
  **Improvements :**
  ...
  - **Builder Automations:** Complex conditions previously required YAML; they are now fully supported in the UI, including branch deletion
...
<Update label="v27.1.0" description="Releasing version v27.1.0">
  ...
  **Bugfixes:**
  ...
  - **Agent Creator Analytics:** Fixed users metric: it now counts real users instead of sessions.
  - **Agent Creator Analytics:** Fixed messages metric: it now counts real messages instead of LLM calls.
  ...
  - **Agent Creator:** Fixed playground: it now always uses the current agent config instead of the published config.
  - **Agent Creator:** Fixed playground: it no longer creates conversations in Securechat.
...
<Update label="v27.0.0: One Platform" description="Releasing version v27.0.0: One Platform">
  ...
  **Prisme.ai One Platform** is a fully modular, composable architecture for deploying AI agents at enterprise scale. Every service is API-first, independently deployable, and built around open standards: **A2A** (Agent-to-Agent), **MCP** (Model Context Protocol), and **A2UI** (Agent-to-UI).
  ...
  <Frame>
    ![Prisme.ai One Platform: Homepage preview](/gifs/homepage-preview.gif)
  </Frame>
  ...
  **Core Services:**
  ...
  <Frame>
    ![Agent Creator: Create and manage agents](/gifs/creationAgent-preview.gif)
  </Frame>
  ...
  **Analytics & Governance:**
  ...
  <Frame>
    ![AI Insights: Conversation analytics and governance](/gifs/insights-preview.gif)
  </Frame>
  ...
  **Boosted UI:** All product interfaces have been redesigned for speed, clarity, and a modern enterprise experience.
  ...
  _Prisme.ai One Platform: Modular. Composable. Enterprise-ready._
</Update>
...
<Accordion title="January 2026 Releases">
  ...
  <Update label="v26.01.080927" description="Releasing version v26.01.080927">
    ...
    **Features:**
    ...
    <Frame>
      ![New Agent Creation with Helper Agents](/gifs/generateAgentInfos.gif)
    </Frame>

    - **AI Store:** Agents can now be called using @mentions in chat

    <Frame>
      ![Mention Agents in Chat](/gifs/mentionAgentInChat.gif)
    </Frame>
  </Update>
</Accordion>
...
<Accordion title="December 2025 Releases">
  ...
  <Update label="v25.12.181015" description="Releasing version v25.12.181015">
    **Features & Improvements :**
    ...
    <Frame>
      ![Document Processing](/gifs/documentprocessingoptions.gif)
    </Frame>
    ...
    **UI :**
    ...
    <Frame>
      ![Language Selector](/gifs/newlanguageselector.gif)
    </Frame>
    ...
    <Frame>
      ![AI Chatbot Popover](/gifs/newpopover.gif)
    </Frame>
...
<Accordion title="November 2025 Releases">
  <Update label="v25.11.241042" description="Releasing version 25.11.241042">
    ...
    - **AIC**: Fixed an issue where the number of lines in AI Collection was not displaying, depending on a certain height of the Datatable.
