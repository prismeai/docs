...
### I Builder
- **Runtime**: New `truncate` function to truncate strings in automations
- **Runtime**: New `lower` and `upper` functions for string manipulation
...
### AI Knowledge (AIK)
- **AIK**: Added Reranking, filtering, contextual compression to the RAG pipeline
...
### API Gateway
- **API Gateway**: Support SMTP server configuration for emails
- **API Gateway**: New allowAnonymousUsers security option to prevent anonymous user access to builders APIs
- **API Gateway**: Better management of lowercase login SSO to avoid duplicate accounts
...
### API Gateway & SSO
- **API Gateway**: Azure SSO improvements
...
### Console & Pages
- **Console** and **Pages**: INTERNAL_API_URL no longer crashes console and pages
...
### Studio & Editor
- **prismeai-studio**: Fix email validation not being taken into account
- **RichTextEditor**: Fix losing caret position on Firefox/Win when page is in studio iframe
...
- **Console** and **Pages**: Upgrade to latest npm package versions
