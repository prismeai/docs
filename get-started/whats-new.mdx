...
- **Agent Factory:** Added icons to agents.
...
- **Agent Factory:** Fixed `messages/stream` API never showing error details.
- **Agent Factory:** Fixed evaluations not taking the selected model into account.
- **Agent Factory:** Fixed playground — now always uses the current agent config instead of the published config.
- **Agent Factory:** Fixed playground — no longer creates conversations in Securechat.
- **Agent Factory:** Fixed guardrails `mask` action.
...
- **Organization API Key on Agent Factory API** — Organization-level API keys are now properly accepted by the Agent Factory API, unblocking org-scoped automation.
...
**Core Services:**
- **Agent Factory** — Create and manage agents with 5 profiles (simple to orchestrator), A2A Protocol, human-in-the-loop, streaming, and 15+ system tools (planning, memory, delegation, reflection, budget tracking).
...
<Frame>
  <img src="/gifs/creationAgent-preview.gif" alt="Agent Factory — Create and manage agents" />
</Frame>
...
- **AI Knowledge:** [SharePoint connector](https://docs.prisme.ai/products/ai-knowledge/sharepoint-connector) is now available, enabling automatic synchronization of documents from SharePoint. Refer to the [official documentation](https://docs.prisme.ai/products/ai-knowledge/sharepoint-connector) for setup instructions.
