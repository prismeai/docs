...
- **Helm chart prismeai-core 1.9.11 & prismeai-apps 0.5.16:** Updated the default Kubernetes resource requests/limits with a *Balanced* profile. See [Kubernetes Resources & Autoscaling](/self-hosting/kubernetes/resources).
- **Helm chart prismeai-apps 0.5.10:** Updated default environment variable values with recommended production values (`REQUEST_MAX_SIZE=20mb`, `KERNEL_POOL_SIZE=10`) and automatically calculate the functions NodeJS `--max-old-space-size` from the k8s memory limit. The `NODE_OPTIONS` environment variable must be removed for the Helm upgrade to succeed, as it is now added natively by the chart.
- **Helm chart prismeai-core 1.9.7 & prismeai-apps 0.5.11:** Added the `global.extraCABundle` value to inject a custom CA bundle in all core and apps services.
- **Helm chart prismeai-core 1.9.8 & prismeai-apps 0.5.12:** Added the `global.proxy` value to globally configure an HTTP/HTTPS proxy.
- **Helm chart prismeai-core 1.9.10:** Added the `secretKeys.password` value for all MongoDB/Postgres databases.
- **Helm chart prismeai-apps 0.5.15:** Added the `secretKeys.password` & `tlsCaFile` values in `global.storage.searchengines`.
