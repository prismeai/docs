...
- **Knowledges:** `max_num_results` can now be configured from an agent, overriding the knowledge base's Top K for that agent's searches. See [RAG Settings → Overriding Top K from an agent](/products/ai-knowledge/rag-settings#overriding-top-k-from-an-agent).
