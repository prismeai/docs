...
<Update label="Latest Release" description="Releasing version v26.03.021352">

**Features & Improvements:**
- **Runtime:** Introduced a secrets module to store encrypted secrets from automation code. Secrets can only be retrieved by the `fetch` instruction.
...
- **Native Workspaces:** AI Knowledge, AI Governance, and other native workspaces are now bundled in the `prismeai-workspaces` image and can be imported from the Platform workspace UI (super admins only).
- **Platform Versioning:**
...
- **AI Knowledge:** SharePoint connector is now available, enabling automatic synchronization of documents from SharePoint. Refer to the [official documentation](https://docs.prisme.ai/products/ai-knowledge/sharepoint-connector) for setup instructions.
...
- **prismeai-functions:** Implement `NODEJS_PACKAGES_WHITELIST_IMPORT` and `NODEJS_PACKAGES_BLACKLIST_IMPORT` environment variables.
- **prismeai-functions:** Implement `PYTHON_BUILTIN_MODULES`, `PYTHON_PACKAGES_WHITELIST_IMPORT`, `PYTHON_PACKAGES_AUTO_INSTALL`, and `PYTHON_PACKAGES_DIR` environment variables.
