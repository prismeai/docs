...
Because they draw on the same unified foundation as every other product, these agents pair advanced capabilities with the platform's built-in governance and traceability, so richer functionality never means looser control.
...
## Implementation in Knowledges

Creating a RAG \+ Tools agent in Prisme.ai's Knowledges is straightforward:

<Tabs>
  <Tab title="1. Create New Agent">
    Start by creating a new agent in the Agent Creator.
...
<CardGroup cols="2">
  <Card title="Agent Creator Guide" icon="store" href="/products/agent-factory/creating-agents">
    Learn how to create agents effectively
  </Card>

  <Card title="Testing Best Practices" icon="vial" href="/products/agent-factory/evaluations">
