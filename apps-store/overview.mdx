...
description: "Build and reuse capabilities: connectors, MCP servers, and custom code, published as versioned apps across the platform."
---
...
Prisme.ai provides a powerful and flexible App Store that enables you to extend the platform with various integration points, connect to external data sources, and create reusable components across your organization. Alongside ready-made apps, **you build your own** connectors, **MCP servers**, and custom code in [Builder](/products/ai-builder/overview), then declare them as **capabilities** available across the platform. In the current Builder UI, open any workspace and click **+** beside **Imports** to browse the App Store.
...
## Build once, govern everywhere: your own capabilities

Ready-made apps are only half the story. The real power for an enterprise is that **you build your own capabilities in [Builder](/products/ai-builder/overview)**, custom **connectors**, **MCP servers**, and custom code, and expose them the same way the built-in apps are exposed.

Once built, a capability is **declared at one of two scopes**:

<CardGroup cols={2}>
  <Card title="Platform-level" icon="globe">
    Publish the capability for the **whole platform**. Every organization can install and reuse it, the shared, curated catalog for your enterprise.
  </Card>
  <Card title="Tenant-level" icon="building-lock">
    Scope the capability to a **single organization** (tenant). Only that org sees it, perfect for team-specific or sensitive integrations.
  </Card>
</CardGroup>

This is the **build-once, reuse-everywhere** model: a connector or MCP server developed once becomes a governed building block that any authorized agent or workspace can call, instead of being re-implemented (and re-secured) in every project.

<Note>
**Why this matters as agents start generating code.** When many teams, and increasingly agents themselves, generate integration code, you risk a sprawl of unreviewed, inconsistently-secured connectors: the exact chaos Prisme.ai exists to prevent. Declaring capabilities centrally lets you **pool security and quality**: credentials are managed once, access is governed by role and scope, and every call is traced. You get the agility of fast integration **without** each new connector becoming a new attack surface or a new silo.
</Note>
