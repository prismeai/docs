...
In Builder, the custom code you write can be declared as a **capability** scoped platform-level (the whole platform) or tenant-level (a single organization), so it is built once, reviewed once, and reused as a governed building block. This pools security and quality: credentials are managed once, access is governed by role and scope, and every call is traced. This matters as agents increasingly generate their own code, so you avoid a sprawl of unreviewed, inconsistently-secured logic.
