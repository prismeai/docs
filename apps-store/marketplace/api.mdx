...
API integrations are a core component of the Prisme.ai App Store, enabling your AI solutions to connect with external services and systems. These integrations allow you to incorporate data from other platforms, trigger actions in external systems, and build comprehensive workflows that span multiple services.

Alongside ready-made integrations, in Builder you build your own, connectors, MCP servers, and custom code, and declare them as **capabilities** scoped either **platform-level** (the whole platform, every organization) or **tenant-level** (a single organization). This build-once, reuse-everywhere model pools security and quality: credentials are managed once, access is governed by role and scope, and every call is traced, so fast integration never turns into a sprawl of unreviewed, inconsistently-secured connectors.
...
The App Store includes API integrations across several categories:
...
While the App Store includes many pre-built integrations, you can also create custom API integrations for your specific needs:
...
<Steps>
  ...
  <Step title="Publish as a Capability">
    Publish your workspace as an app and declare it as a capability, platform-level for the whole enterprise, or tenant-level for a single organization, so any authorized workspace or agent can reuse it
  </Step>
</Steps>
...
## Using API Integrations with Knowledges and Agents

API integrations can significantly enhance your Knowledges bases and agents:
