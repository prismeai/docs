...
In Builder you build your own connectors, MCP servers, and custom code, then declare them as **capabilities**, scoped either **platform-level** (available to the whole platform and every organization) or **tenant-level** (a single organization). This is the **build-once, reuse-everywhere** model: instead of each team, or, increasingly, each agent, re-implementing and re-securing the same integration, a capability is built once, reviewed once, and reused as a governed building block. It pools security and quality across your enterprise: credentials are managed once, access is governed by role and scope, and every call is traced. You get fast integration without a sprawl of unreviewed, inconsistently-secured connectors.
...
<Steps>
  ...
  <Step title="Publication">
    ...
    - Declare it as a capability at the right scope, platform-level for the whole enterprise, or tenant-level for a single organization
