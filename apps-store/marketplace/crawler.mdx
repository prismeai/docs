...
The Crawler app provides an extensive `websites_config` map to tailor its behavior to your specific needs, domain by domain.  
<Note>Do not include www. prefix in websites_config domains</Note>
...
<AccordionGroup>
  <Accordion title="Target websites">
    ...
    This configuration start crawling the 3 configured domains and **follows every discovered URL belonging to these domains**.  
    For https://quotes.toscrape.com/tag/love/, it will **only follow urls under `/tag/love/` path : in that case, only https://quotes.toscrape.com/tag/love/page/2/.  
  </Accordion>
  ...
  <Accordion title="URL blacklist">
    ...
    - **excludes**all URLs paths beginning with `/publications` for issy.com and with an `id=` query string parameter 
    - **keeps crawling** every discovered URL for quotes.toscrape.com  
  </Accordion>  
  ...
  <Accordion title="Sitemap crawling">
    Crawl only the URLs listed in a sitemap by providing an websiteURL ending with **sitemap.xml** :  
