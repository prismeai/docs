...
<Tabs>
  ...
  <Tab title="Document Metadata">
    ...
    All metadata is stored in the `custom_metadata` field and indexed for search and filtering. Labels are exposed as `mip:label:*` keys (`mip:label:id`, `mip:label:name`, …) — the same keys the [client-side file gate](/products/agent-factory/hooks#microsoft-purview--mip-sensitivity-labels) evaluates before upload, so one classification policy can be enforced at both ends.
  </Tab>
