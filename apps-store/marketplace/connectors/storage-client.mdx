...
description: 'Deprecated connector to the Prisme.ai Storage backend — use the Knowledges connector instead'
---

<Warning>
  **This connector is deprecated. Use [Knowledges](/apps-store/marketplace/connectors/knowledges) instead.**

  Knowledges covers the same knowledge-base surface with the current naming (`knowledges`, `documents`, `webSources` instead of `vectorStores` and `vectorStoreFiles`) and the same credential-free authentication. Install it for any new agent or automation, and migrate the existing ones when convenient.

  This page is kept for the workspaces that still run the Storage connector. It stays functional, and remains the only connector exposing the administrative surface — per-store API keys, RBAC access bindings, agent skills and admin diagnostics — which Knowledges does not cover. The legacy files-registry operations documented below are deprecated on the backend and will be removed with the legacy API.
</Warning>
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to manage files, vector stores and RAG. → *Agent builder* tab.
  </Card>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator manage files, vector stores and RAG through the Storage MCP tools.
    ...
    This connector runs in the **tenant app-instance context** and forwards the caller's Prisme.ai identity to the Storage backend. Your agent is identified by the `agent_id` and `user_id` that Agent Creator injects through the capability *Scope*; the Storage backend then enforces its own RBAC bindings against that user; there is no credential for the agent to hold or paste.
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    <Note>
      The connector is pre-wired to the Storage backend of the **current environment** (sandbox or prod) via `slug:storage`; there is no `baseUrl` to set. Anonymous calls (no forwarded identity and no `accessToken`) are rejected by the Storage backend.
    </Note>
...
**"The calling agent could not be identified"**: The MCP capability *Scope* does not declare `agent_id` / `user_id`, so Agent Creator never injects the caller identity. Set the Scope to `context_id,agent_id,user_id` on the capability.

**"Anonymous request rejected" (401)**: The caller's Prisme.ai identity did not make it through to the Storage backend. This usually means the call is being made from a tool client that does not propagate the workspace identity. Use the Agent Creator capability or call from a DSUL automation (both forward identity correctly), or pass an explicit `accessToken` override.
