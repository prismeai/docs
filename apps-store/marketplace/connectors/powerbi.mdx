---
title: 'Power BI'
description: 'Query Power BI datasets with DAX, manage workspaces, reports, dashboards and refreshes from Agent Creator agents and Builder workflows, with per-user Microsoft Entra ID OAuth'
---
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to query and manage Power BI. → *Agent builder* tab.
  </Card>
...
<Tabs>
    ...
    **Goal:** let an agent you build in Agent Creator query and manage Power BI through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Connect a Microsoft account">
        On the first tool call, an unconnected user is prompted to sign in; Agent Creator surfaces a `connect_url`. Subsequent calls reuse the stored token.
      </Step>
    </Steps>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    <Note>
      Credentials are provisioned into the workspace's Secrets by the `onInstall` flow and resolved server-side. The agent allowlist (**Authorized agents**) gates which Agent Creator agents may call the MCP endpoint; see the *Agent builder* tab. Per-user tokens are refreshed automatically; a revoked token drops the connection and the user is asked to reconnect.
    </Note>
