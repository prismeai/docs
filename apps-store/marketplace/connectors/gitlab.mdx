...
<Steps>
  ...
  <Step title="Declare the OAuth authentication in Governance">
    ...
    Replace `<api-url>` with your environment's API URL — `https://api.sandbox.prisme.ai/v2` on sandbox, `https://api.studio.prisme.ai/v2` on production. Governance uses these URLs to (1) probe whether the calling user is already connected (`status_url`), (2) trigger the OAuth handshake when they are not (`connect_url`), and (3) revoke the session on demand (`disconnect_url`).
  </Step>
