---
title: 'GitLab'
description: 'Let Agent Creator agents manage GitLab projects, issues, merge requests and CI/CD pipelines through MCP, with per-user OAuth resolved server-side'
---
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to use GitLab. → *Agent builder* tab.
  </Card>
...
<Tabs>
    ...
    **Goal:** let an agent you build in Agent Creator use GitLab through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Connect a GitLab account">
        On the first tool call, an unconnected user is prompted to sign in: Agent Creator surfaces a `connect_url` to GitLab's authorization page. After sign-in the per-user token is stored and reused on subsequent calls; refresh is automatic.
      </Step>
    </Steps>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure authentication and the agent allowlist, and call GitLab operations from your automations.
    ...
    <Note>
      Credentials are stored in the workspace's Secrets (`gitlabAuth`) and resolved server-side. The agent allowlist (**Authorized agents**, secret `gitlabAuthorizedAgents`) gates which Agent Creator agents may call the MCP endpoint; see the *Agent builder* tab. Builder automations calling `GitLab.op:` instructions resolve the same workspace credentials.
    </Note>
...
**"The calling agent could not be identified"**: The MCP capability *Scope* does not declare `agent_id`, so Agent Creator never injects the agent identity. Set the Scope to `context_id,agent_id,user_id` on the capability (this is separate from the GitLab OAuth scopes), then allow the agent in the config app.
