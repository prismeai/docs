---
title: 'OneNote'
description: 'Microsoft OneNote notebooks, sections and page text for Agent Creator agents and Builder workflows, through Microsoft Graph with delegated per-user authentication'
---
...
<CardGroup cols={2}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to read and write OneNote. → *Agent builder* tab.
  </Card>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator read and write OneNote through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Connect a Microsoft 365 account">
        On the first tool call, an unconnected user is prompted to sign in; Agent Creator surfaces a `connect_url`. The `accessToken` mode needs no interactive sign-in.
      </Step>
