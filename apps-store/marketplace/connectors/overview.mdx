...
Each connector is a **single Prisme.ai workspace**, published as an app and installed as an app instance inside your workspace. That one workspace exposes both surfaces — DSUL App-mode instructions and an MCP server — and runs in your tenant context, reading your own configuration and credentials.
...
The connector wraps the external API as reusable operations, handles authentication (API key, OAuth2, JWT, and more), groups endpoints into LLM-friendly entity tools, and gates agent access with an allowlist.

<Card title="Connector Architecture" icon="sitemap" href="/apps-store/marketplace/connectors/architecture">
  Read the full architecture: tenant-context model, request lifecycle, authentication modes, and agent authorization.
</Card>
...
<CardGroup cols={2}>
  <Card title="Connector Architecture" icon="sitemap" href="/apps-store/marketplace/connectors/architecture">
    Understand how connectors are built
  </Card>
