...
<Tabs>
  <Tab title="Usage as App">
    ...
    Jira custom fields must already exist in Jira and be available for the target project and issue type. Use `listFields` to resolve the technical field ID, then pass values through `fields`.
    ...
    If Jira rejects a custom field during creation, verify the field context and create screen/layout in Jira. For team-managed Jira projects, the field must be added to the project work type layout. You can confirm API availability with `listCreateIssueTypes` and `getCreateIssueFields` before creating the issue.
  ...
  <Tab title="Usage as MCP">
    ...
    **Jira Data Center:**
    ...
    ### Discover and Use a Custom Field
    ...
    Use the returned `customfield_...` ID in `createIssue` or `updateIssue` only after Jira reports the field as available for that project and issue type.
