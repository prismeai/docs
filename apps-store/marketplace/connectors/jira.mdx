---
title: 'Jira'
description: 'Search, create and manage Jira Cloud and Data Center issues, projects, users, metadata and agile boards from Agent Factory agents and Builder workflows, with per-user OAuth or Personal Access Tokens'
---
...
- **Per-user OAuth1 RSA-SHA1** (`oauth1`, Data Center only): a Jira Application Link signs each request with a shared consumer key and RSA private key, plus an access token that each user obtains from that Application Link. Use it on Data Center instances where OAuth2 is not available. Selecting it on a Cloud deployment fails with `oauth1 is supported only with deployment=dataCenter.`
...
- A **Jira Cloud** site (`https://yoursite.atlassian.net`) or a **Jira Data Center 10.3** base URL (an absolute HTTP or HTTPS URL, optionally with a context path such as `/jira`, and without credentials, query string or fragment), plus an account with permission on the projects you intend to use.
...
- Cloud OAuth resolves the site through `https://api.atlassian.com/ex/jira/{cloudId}` (the `cloudId` is discovered from the granted site); the `apiToken` mode calls `https://{site}.atlassian.net` directly.
- On a **Data Center** deployment you supply the base URL yourself, the connector calls it directly, and every platform route is served under `/rest/api/2` instead of `/rest/api/3` (JQL search uses `/rest/api/2/search`). Agile operations stay on `/rest/agile/1.0` for both deployments. Data Center OAuth2 uses the instance endpoints `{baseUrl}/rest/oauth2/latest/authorize` and `{baseUrl}/rest/oauth2/latest/token`.
