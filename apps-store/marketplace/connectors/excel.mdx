---
title: 'Excel'
description: 'Let Agent Creator agents and Builder workflows read, write, and reshape Excel workbooks stored in OneDrive or SharePoint via Microsoft Graph'
---
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to work with Excel workbooks. → *Agent builder* tab.
  </Card>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator read and write Excel workbooks through Excel MCP tools.
    ...
    The connector resolves its Microsoft credential server-side; it is never exposed to the agent. Your agent is identified by the `agent_id` that Agent Creator injects through the capability *Scope*.
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the Excel app in a workspace, configure the Azure AD credential, and call Excel instructions from your automations.
    ...
    | **MCP Endpoint** | Auto-populated on install: the URL Agent Creator agents point a capability at (see the *Agent builder* tab). |
