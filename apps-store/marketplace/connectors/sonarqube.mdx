---
title: 'SonarQube'
description: 'Read and act on SonarQube / SonarQube Cloud code-quality data from Agent Creator agents and Builder workflows'
---
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to use SonarQube. → *Agent builder* tab.
  </Card>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator read and act on SonarQube code-quality data through MCP tools.
    ...
    The SonarQube credential is resolved server-side from the installed app's configuration and never exposed to the agent. Your agent is identified through the capability *Scope* that Agent Creator injects.
    ...
    | `projectId` | No* | Project ID, an alternative to `projectKey` |
