...
<Tabs>
  <Tab title="Agent builder (Agent Factory)">
    ...
    **Goal:** let an agent you build in Agent Factory read and act on SonarQube code-quality data through MCP tools.
    ...
    The SonarQube credential is resolved server-side from the installed app's configuration and never exposed to the agent. Your agent is identified through the capability *Scope* that Agent Factory injects.
