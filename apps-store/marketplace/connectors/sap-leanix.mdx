---
title: 'SAP LeanIX'
description: 'Query and manage SAP LeanIX Enterprise Architecture from Agent Creator agents and Builder workflows: fact sheets, Integration API and surveys'
---
...
The SAP LeanIX app exposes the [SAP LeanIX](https://www.leanix.net) Enterprise Architecture Management platform through three entity tools. It can be consumed two ways: as a remote MCP server that **Agent Creator** agents call as tools, or as a Builder app whose instructions you call directly from DSUL. It runs in the **tenant app-instance context** (it resolves the installing workspace's own LeanIX credential) and covers three LeanIX surfaces:
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to query and manage LeanIX. → *Agent builder* tab.
  </Card>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator query and manage SAP LeanIX through MCP tools.
    ...
    <Note>
      **Easiest registration: one-click "Add to catalog".** In the configuration app, org owners/admins see an **Add to catalog** button (under *Capabilities catalog*). It publishes the connector to your organization's **Capabilities catalog** as a ready-to-use capability (MCP endpoint and Scope `context_id,agent_id,user_id` pre-wired) so any builder can attach it from the picker without wiring the endpoint by hand. This is the recommended path; the manual steps below remain available if you prefer to wire the capability yourself.
    </Note>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure authentication and the agent allowlist, and call LeanIX operations from your automations.
    ...
    <Note>
      Credentials are provisioned into the workspace's Secrets and resolved server-side. The agent allowlist (**Authorized agents**) gates which Agent Creator agents may call the MCP endpoint; see the *Agent builder* tab.
    </Note>
