...
description: 'Drive, Docs, Sheets, Gmail and Calendar from Agent Creator agents and Builder workflows, with per-user OAuth, service-account or access-token auth'
---
...
- **Per-user OAuth2 with a central client** (`oauthCentral`, recommended): one Google OAuth Application is registered once by the platform maintainer; every end user signs in with their own Google account. Nothing to register per tenant: each workspace just installs the app and clicks *Connect*.
- **Per-user OAuth2 with a tenant client** (`oauth`): paste your own Google OAuth client ID/secret in the connector config app. Each user signs in with their own account against your client (PKCE authorization-code flow).
- **Service account with JWT Bearer** (`jwt`): a Google service-account key, with optional domain-wide delegation (`subject`) to impersonate users across a Workspace domain. No interactive sign-in.
...
<CardGroup cols={2}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to read and act on Google Workspace. → *Agent builder* tab.
  </Card>
  <Card title="Knowledge base owner" icon="database">
    You want Google Drive files indexed and searchable semantically, kept up to date. → *Knowledge base (sync)* tab.
  </Card>
...
<Tabs>
    ...
    **Goal:** let an agent you build in Agent Creator read and act on Google Workspace through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Add the Google Workspaces capability">
        Browse the capability catalog, select **Google Workspaces**, and enable it. The MCP endpoint URL and the *Scope* (`context_id,agent_id,user_id`) are already wired by the admin; there is nothing to paste, and the shared instance accepts every agent, so there is no allowlist step on your side.
      </Step>
      <Step title="Connect a Google account (OAuth modes)">
        On the first tool call, an unconnected user is prompted to sign in; Agent Creator surfaces a `connect_url`. Service-account (`jwt`) and `accessToken` modes need no per-user sign-in.
      </Step>
    </Steps>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure authentication and the agent allowlist, and call Google operations from your automations.
    ...
    <Note>
      Credentials are provisioned into the workspace's Secrets by the `onInstall` flow and resolved server-side. The agent allowlist (**Authorized agents**) gates which Agent Creator agents may call the MCP endpoint; see the *Agent builder* tab.
    </Note>
  ...
  <Tab title="Knowledge base (sync)">
    ...
    **Goal:** index Google Drive files into a knowledge base so an agent can search their **content**, and keep that base up to date.
    ...
    The full wizard, the run history and the connection settings are documented once for every provider in [Knowledges → Connectors](/products/ai-knowledge/connectors). This tab covers only what is specific to Google Drive.

    <Note>
      Google Drive is normally also published as a **catalog connector**, which appears under **Available Providers** and asks each person to sign in with their own Google account. Attaching this connector's own endpoint instead is what lets a base be fed by a **service account** (`jwt` mode, optionally with domain-wide delegation) rather than by a person — the credential then belongs to the workspace, and no reader ever signs in for the sync.
    </Note>
    ...
    | Answering a reader's question | The reader | A live per-reader access check (`drive` / `checkAccess`) against the Drive API |
    ...
    | Other files | Downloaded with the connector's own token and inlined as a bounded data URL, up to **4 MB**; inline text is capped at 200 000 characters. |
    ...
    <Note>
      **The Knowledge profile this connector declares** — the descriptor Knowledges probes at `gdrive://profile/knowledge`:
      ...
      | `read` | Native export to `text/plain`; binaries inline as a data URL ≤ 4 MB; 200 000 characters of inline text |
    ...
    **A large file is missing from the base**: binaries travel inline and are bounded at 4 MB. Above that the file is reported in the connection's synchronization errors rather than half-indexed.
