    ...
    **Goal:** let an agent you build in Agent Factory read and act on Google Workspace through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Add the Google Workspaces capability">
        Browse the capability catalog, select **Google Workspaces**, and enable it. The MCP endpoint URL and the *Scope* (`context_id,agent_id,user_id`) are already wired by the admin — nothing to paste.
      </Step>
      <Step title="Make sure your agent is allowlisted">
        The shared instance still gates callers by their `agent_id`. Ask the owner of the shared instance to tick your agent in the connector's **Configuration app → Authorized agents** (it may already be set to **Allow all agents**).
      </Step>
    ...
    <Note>
      Convenient, but your agent runs against a **shared, platform-managed instance**: its credentials and allowlist are owned by someone else and reachable by every agent granted the capability. Prefer Option B for anything beyond quick experiments.
    </Note>
