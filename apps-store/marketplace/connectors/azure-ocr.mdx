...
<CardGroup cols={3}>
  ...
  <Card title="Document Intelligence" icon="file-lines">
    Structured extraction for PDFs and complex layouts via the `document` tool, with prebuilt models (layout, read, invoice, receipt, ID document); async submit-and-poll is handled internally.
  </Card>
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to extract text from images and documents. → *Agent builder* tab.
  </Card>
...
- A **publicly accessible URL** for the files you analyze (see below), or base64 content for documents.
...
<Warning>
  Azure downloads the file itself: the connector only forwards the URL. Intranet hosts, `localhost` and anything behind a login or an `Authorization` header fail. Microsoft requires a [publicly accessible image URL](https://learn.microsoft.com/en-us/azure/ai-services/computer-vision/how-to/call-analyze-image-40) for Computer Vision, and for Document Intelligence a blob URL with a **SAS token** on a storage account open to public networks — never one behind a VNet or firewall ([FAQ](https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/faq)).

  Use a signed URL (SAS, S3 pre-signed) for private files, or `sourceType: base64` on `document`. The `image` tool has **no base64 mode**, so an unreachable image must first be made reachable.
</Warning>
...
<Tabs>
  <Tab title="Agent builder (Agent Creator)">
    ...
    **Goal:** let an agent you build in Agent Creator extract text and structured data from images and documents through the `image` and `document` MCP tools.
    ...
    - image: for a single image (photo, screenshot, receipt picture). Pass a PUBLICLY ACCESSIBLE image URL as `source` — Azure downloads it itself, so intranet or login-protected URLs fail. Default feature is Read (OCR); add Caption, Tags, Objects or People only when the user asks for visual analysis. Synchronous.
    - document: for PDFs and multi-page or structured documents. Pass a publicly accessible document URL (or base64 with sourceType: base64 when the file is private) as `source`, and pick the model by intent: prebuilt-read for plain text, prebuilt-layout for tables and structure, prebuilt-invoice / prebuilt-receipt / prebuilt-idDocument for those specific document types.
    Prefer outputContentFormat: markdown when feeding the result back into your own reasoning. Always pass a public URL when you have one; base64 inflates the request. Never retry an unreachable URL — ask for a signed link, or fall back to document + base64.
    ```
    ...
    | Tool | Description |
    |------|-------------|
    | `image` | Azure Computer Vision: image OCR and visual analysis. `action: analyze` sends a **publicly accessible** image URL and returns the extracted read text, captions, tags, objects and people. Synchronous. |
    | `document` | Azure Document Intelligence: structured document extraction. `action: analyze` submits a document (**publicly accessible** URL, or base64) to a prebuilt model, polls for completion, and returns the analysis (content, pages, tables, key-value pairs, paragraphs). Asynchronous. |
    ...
    |-----------|----------|-------------|
    | `action` | Yes | `analyze`. |
    | `source` | Yes | **Publicly accessible** URL of the image to analyze. No base64 alternative on this tool. |
    ...
    |-----------|----------|-------------|
    | `action` | Yes | `analyze`. |
    | `source` | Yes | **Publicly accessible** document URL, or base64 content when `sourceType` is `base64`. |
    | `sourceType` | No | `url` (default) or `base64`. Use `base64` whenever the document is not reachable from Azure's network. |
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure the per-service Azure credentials and the agent allowlist through its configuration app, and call its instructions from your automations.
    ...
    |-------------|-------------|---------|
    | `analyzeImage` | Synchronous OCR and visual analysis of an image by **publicly accessible** URL (`source`). `features` selects analysis (default `Read`; also `Caption`, `Tags`, `Objects`, `People`); `language` is a hint. | Raw Azure Image Analysis body: `{ readResult, captionResult, tagsResult, objectsResult, peopleResult }` (subset per requested features) |

    ### Document Intelligence

    | Instruction | Description | Returns |
    |-------------|-------------|---------|
    | `analyzeDocument` | Submit a document (**publicly accessible** `source` URL, or base64 when `sourceType: base64`) to a prebuilt `model`, poll to completion, and return the extraction. `pages`, `locale`, `outputContentFormat` (`text`/`markdown`) and `outputFormat` (`simple`/`raw`) tune the output. | `simple`: flattened text/markdown + key fields. `raw`: full Azure `analyzeResult` `{ pages, tables, keyValuePairs, paragraphs, content }` |
    ...
    Analyze a base64 document with a page range — the fallback when the file has no public URL:
...
| `AzureError` | Azure rejected the request (typically HTTP 400); often a `source` URL Azure could not download |
...
- **Azure cannot download the `source` URL**: a URL that works in your browser is not necessarily one Azure can reach. Symptom: a 4xx in `details.body`, e.g. `InvalidRequest - Image URL is badly formatted or not accessible`. Check it needs no authentication, is not on an intranet, and — for blob storage — carries an unexpired SAS token on a publicly reachable account. For documents, `sourceType: base64` sidesteps it.
...
<CardGroup cols={2}>
  ...
  >
Microsoft's Analyze API guide: the image URL must be publicly accessible.
  </Card>
  <Card
    title="Tool Agents"
    icon="robot"
