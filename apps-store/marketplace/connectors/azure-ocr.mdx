...
<Tabs>
    ...
    **Goal:** let an agent you build in Agent Factory extract text and structured data from images and documents through the `image` and `document` MCP tools.
    ...
    <Steps>
      ...
      <Step title="Add the MCP capability to your agent">
        ...
        The `agent_id` lets the connector identify and authorize your agent. `attachment_urls` gives the connector the real URLs of files uploaded in the conversation, avoiding URLs guessed by the model. Without `agent_id`, every call is rejected with an explicit "agent could not be identified" message.
      </Step>
    ...
    - document: for PDFs and multi-page or structured documents. Pass the document URL supplied with the conversation attachment (or base64 with sourceType: base64) as `source`, and pick the model by intent: prebuilt-read for plain text, prebuilt-layout for tables and structure, prebuilt-invoice / prebuilt-receipt / prebuilt-idDocument for those specific document types.
    Prefer outputContentFormat: markdown when feeding the result back into your own reasoning. Do not invent or rewrite attachment URLs. The connector can proxy private Prisme file URLs as base64 when that workspace option is enabled.
    ```
    ...
    <Note>
      If the workspace enables **Proxy private Prisme files through base64**, keep `sourceType: url` and pass the original Prisme attachment URL. The connector downloads trusted URLs from its own Prisme API (up to 25 MB), follows the platform-issued file redirect, and submits `base64Source` to Azure. Public URLs still use Azure's normal `urlSource` flow. The model never needs to generate or handle base64.
    </Note>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure the per-service Azure credentials and the agent allowlist through its configuration app, and call its instructions from your automations.
    ...
    3. Open the **Configuration app** (the link auto-populated on install) to paste the Azure endpoint hostname + API key for the service(s) you use, optionally enable private-file proxying, and allow the agents that may call the connector

    ## Configuration

    | Field | Description |
    |-------|-------------|
    | **Configuration app** | Auto-populated on install; open this link to set the per-service Azure credentials (Computer Vision and/or Document Intelligence endpoint + API key), configure private-file proxying, and manage the authorized-agents allowlist. |

    The configuration app drives everything; the app instance itself has no per-field credential form. Credentials are stored as four flattened workspace secrets (`azureOcrComputerVisionEndpoint`, `azureOcrComputerVisionApiKey`, `azureOcrDocumentIntelligenceEndpoint`, and `azureOcrDocumentIntelligenceApiKey`), the allowlist as `azureOcrAuthorizedAgents`, and the optional proxy flag as the boolean secret `azureOcrDownloadPrivateFilesAsBase64`. All are resolved server-side at call time. The proxy flag is `false` by default.

    ### Private Prisme file proxy (optional)

    Enable **Proxy private Prisme files through base64** when Azure Document Intelligence cannot resolve or reach your private Prisme API hostname. With the option enabled and `sourceType: url`:

    1. Public, non-Prisme URLs are still sent to Azure as `urlSource`.
    2. URLs served by the current Prisme API are downloaded inside the connector, including platform-issued redirects.
    3. Files up to 25 MB are encoded and submitted to Azure as `base64Source`.

    The initial URL must belong to the connector's own Prisme API origin and match a native `/v2/files/...` or file-content webhook endpoint; other URLs are never downloaded by the connector. This limits the proxy to trusted platform file endpoints. The setting affects Document Intelligence only; Computer Vision image analysis continues to require a public image URL.
...
```json
{
  "error": "ConfigurationError | ValidationError | SourceDownloadError | NetworkError | AzureError | Unauthorized | Forbidden | NotFound | RateLimited | ServerError | ProcessingFailed | Timeout",
...
| `SourceDownloadError` | The optional private-file proxy could not download the Prisme file, timed out, or rejected a file larger than 25 MB |
...
- **"The calling agent could not be identified"**: The MCP capability *Scope* does not declare `agent_id`, so Agent Factory never injects the agent identity. Set the Scope to `context_id,agent_id,user_id,attachment_urls` on the capability, then allow the agent in the config app.
...
- **Azure says it could not download a Prisme file URL**: Azure cannot reach the private Prisme hostname. Open the connector configuration app and enable **Proxy private Prisme files through base64**, then pass the original attachment URL with `sourceType: url`. The connector accepts trusted files up to 25 MB.
- **`SourceDownloadError`**: The connector-side download failed before Azure was called. Confirm the URL is still valid and belongs to the current Prisme API, and that the file is no larger than 25 MB.
