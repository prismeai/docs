...
<Note>
  This is the **legacy SharePoint connector**, superseded by the new **[SharePoint](/apps-store/marketplace/connectors/sharepoint)** connector (App+MCP, tenant-context, per-user Microsoft Entra OAuth). Use the new SharePoint connector for new work — it adds sites/drives/items/search/permissions/lists/directory tools, central or tenant OAuth, and per-agent authorization. This page is kept for reference only.
</Note>
