---
title: 'Gryzzly'
description: 'Read and write Gryzzly time-tracking data from Agent Creator agents and Builder workflows: projects, tasks, declarations, leave periods and payroll exports'
---
...
<CardGroup cols={3}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to use Gryzzly. → *Agent builder* tab.
  </Card>
...
<Tabs>
  ...
  <Tab title="Workspace builder (DSUL)">
    ...
    **Goal:** install the connector in a workspace, configure the Gryzzly API key and the agent allowlist, and call Gryzzly operations from your automations.
    ...
    <Note>
      The API key is provisioned into the workspace's Secrets and resolved server-side. The agent allowlist (**Authorized agents**) gates which Agent Creator agents may call the MCP endpoint; see the *Agent builder* tab.
    </Note>
