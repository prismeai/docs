---
title: 'Outlook'
description: 'Outlook mail, calendar and contacts from Agent Creator agents and Builder workflows, via Microsoft Graph with per-user Microsoft Entra OAuth'
---
...
<CardGroup cols={2}>
  <Card title="Agent builder" icon="robot">
    You build agents in **Agent Creator** and want them to read and act on Outlook mail, calendar and contacts. → *Agent builder* tab.
  </Card>
  <Card title="Knowledge base owner" icon="database">
    You want a mail folder indexed and searchable semantically, kept up to date. → *Knowledge base (sync)* tab.
  </Card>
...
<Tabs>
    ...
    **Goal:** let an agent you build in Agent Creator read and act on Outlook mail, calendar and contacts through MCP tools.
    ...
    <Steps>
      ...
      <Step title="Connect a Microsoft account (OAuth modes)">
        On the first tool call, an unconnected user is prompted to sign in; Agent Creator surfaces a `connect_url`. Application (`clientCredentials`) and `accessToken` modes need no per-user sign-in.
      </Step>
    </Steps>
  ...
  <Tab title="Knowledge base (sync)">
    ...
    **Goal:** index the messages of a mail folder into a knowledge base so an agent can search their **content**, and keep that base up to date.
    ...
    | Attachments | **Not ingested.** A message is a leaf, so the sync never descends into it; `hasAttachments` travels as metadata and the citation link opens the message. |
