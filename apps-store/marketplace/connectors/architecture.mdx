...
%%{init: {"themeVariables": {"fontSize": "20px"}, "flowchart": {"nodeSpacing": 45, "rankSpacing": 65, "wrappingWidth": 480}}}%%
flowchart TB
  Caller["<span style='display:inline-block;width:480px'><b>TB2: Prisme.ai platform services</b><br/>Agent Creator or Builder runtime</span>"]
...
| `F4` | Agent Creator or Builder runtime → execution context | MCP `tools/call` or explicit App instruction with business arguments |
...
%%{init: {"themeVariables": {"fontSize": "20px"}, "flowchart": {"nodeSpacing": 45, "rankSpacing": 65, "wrappingWidth": 480}}}%%
flowchart TB
  Caller["<span style='display:inline-block;width:480px'><b>TB2: Agent Creator caller</b><br/>tools/call + injected caller IDs</span>"]
...
| MCP capability scope | Which identity attributes Agent Creator injects into the call |
...
The agent allowlist gates **tool calls**. **MCP resources** are authorized by the workspace role of the caller instead: a resource read is a read of workspace-held content, so it follows the workspace's own RBAC rather than the list of agents allowed to invoke the connector's tools.
...
1. The user's request and tool arguments enter Agent Creator.
