...
DSUL is the language behind the foundation. It's an open-source, comprehensive declarative language that enables organizations to describe, build, and orchestrate sophisticated AI-powered applications using YAML-based meta-programming. This is what makes Prisme.ai's promise concrete: a single, versionable definition that bridges frontend interfaces, backend processes, automation workflows, and AI capabilities — the [Infrastructure-as-Code](/get-started/platform-overview#the-development-lifecycle) layer that keeps agents and applications reproducible instead of scattered across silos.
...
- **Components**: Reusable UI components that encapsulate specific functionality
