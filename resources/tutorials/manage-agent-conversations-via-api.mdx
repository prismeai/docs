...
field. Between frames, the server sends `: keepalive` SSE comment lines to keep
the connection open; ignore every line starting with `:`, as `EventSource` does.
Lifecycle frames use `{"event": "<name>", "data": {...}}`. Frames without an
`event` property are transport frames: discard only `{"keepAlive": true}`, the
keep-alive older servers send instead of the comment line. The server closes the stream with a summary frame shaped
