...
The product specs (`api-reference/<product>/swagger.yml`) are copies too: their source is `docs/<workspace>/swagger.yml` in the `prismeai-workspaces` repo, where `scripts/swagger-verify` checks them against the DSUL. Never edit either kind of copy here; fix the source and re-copy.
