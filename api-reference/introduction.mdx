---
title: "Overview"
description: "The Prisme.ai API: an API-first, agentic platform you can drive and integrate from anywhere."
---
...
As the API-first entry point to the unified foundation, it exposes every product as a facet of one governed, traceable system, giving technical teams the full-code door into the same platform business users reach through no-code interfaces.
...
The Prisme.ai API is organized into several product areas, each with its own set of endpoints. The Platform API is served by the core services; every other API is exposed by a product workspace under `/v2/workspaces/slug:<workspace>/webhooks`.

<CardGroup cols="3">
  <Card title="Platform API" icon="hammer">
    Workspaces, automations, pages, files, users, organizations and events
  </Card>
  <Card title="Agent Creator API" icon="robot">
    Agents, conversations, messages, tasks, artifacts and sharing
  </Card>
  <Card title="LLM Gateway API" icon="bolt">
    OpenAI-compatible model access, models, quotas and images
  </Card>
  <Card title="Governance API" icon="shield">
    Organization settings, subscriptions and observability
  </Card>
  <Card title="Knowledges API" icon="database">
    Knowledge bases, documents, web sources and search
  </Card>
  <Card title="Knowledge Connectors API" icon="plug">
    Third-party connections that sync documents into knowledge bases
  </Card>
  <Card title="Collections API" icon="table">
    Structured data collections, records and queries
  </Card>
  <Card title="Agent Evaluations API" icon="list-check">
    Evaluation cases and runs for agents
  </Card>
  <Card title="Capabilities API" icon="puzzle-piece">
    Catalog of MCP servers available to agents
  </Card>
  <Card title="Prompt Library API" icon="book">
    Shared prompts and showcases for the chat
  </Card>
  <Card title="Insights API" icon="chart-line">
    Usage analytics, conversation insights and feedback
  </Card>
</CardGroup>
...
<CardGroup cols="3">
  ...
  <Card title="Agent Conversation Tutorial" icon="messages" href="/resources/tutorials/manage-agent-conversations-via-api">
    List, load, and continue user conversations with the Agent Creator API
  </Card>
</CardGroup>
