...
<Frame>
  <img src="/images/api-authentication.png" alt="Prisme.ai API Authentication" />
</Frame>
...
<Frame>
  <img src="/images/auth-flow-diagram.png" alt="Authentication Flow Diagram" />
</Frame>
...
<Tabs>
  ...
  <Tab title="Authorization Configuration">
    Each microservice manages its own permissions system using the `@prisme.ai/permissions` package.
    
    Configuration is typically located in:
    - `src/permissions/config.ts`: Defines roles, object types, and access rules
    - `src/permissions/index.ts`: Instantiates the permissions system with the configuration
