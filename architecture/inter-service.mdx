...
Workspaces can emit events that other workspaces subscribe to:

```yaml
# Emit from Agent Factory
- emit:
    event: agents.updated
    payload:
      agent_id: '{{agent.id}}'

# Listen in another workspace
when:
  events:
    - agents.updated
do:
...
Used for:
- Cache invalidation across workspaces
- Async processing pipelines
- IAM synchronization (e.g., `iam.cache.invalidate`)
...
All three vector providers implement the **identical interface**:
...
Workspaces reference each other by slug in their `index.yml` config:

```yaml
# agent-factory/index.yml
config:
  value:
    ai_governance:
      slug: ai-governance-v2
      workspace_id: KOGm-LL
    tools_memories_slug: tools-memories
    storage_slug: storage
```

Some workspaces also store API keys as secrets:
...
config:
  value:
    agent_factory:
      slug: agent-factory
      api_key: '{{secret.AGENT_FACTORY_API_KEY}}'
```

This configuration is set at deployment time and can vary between environments (sandbox, production, etc.).
