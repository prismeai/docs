...
|--------|--------|-------------|---------|
| Agent Factory | Storage | Cross-workspace | File/vector operations |
| Agent Factory | LLM Gateway | Cross-workspace | Chat completions, embeddings |
| Agent Factory | AI Governance | Cross-workspace | API key validation |
| Agent Factory | Tools Memories | Cross-workspace | Long-term memory |
| Agent Factory | Guardrails | Cross-workspace | Input/output safety |
| Agent Factory | AI Collection | MCP (agent context) | Structured data tools |
| LLM Gateway | AI Governance | Cross-workspace | API key validation |
| Storage | LLM Gateway | Cross-workspace | Embeddings for RAG |
...
| AI Insights | LLM Gateway | Cross-workspace | Analysis LLM calls |
| Agent Evaluations | Agent Factory | API key (`x-api-key`) | Test agent conversations |
| Agent Evaluations | LLM Gateway | Cross-workspace | Judge LLM calls |
| Guardrails | LLM Gateway | Cross-workspace | Safety check LLM calls |
| Memories | LLM Gateway | Cross-workspace | Embedding generation |
...
### Pattern 1: Cross-Workspace Fetch

The most common pattern. Used when one workspace calls another's REST API. The calling workspace is automatically identified via `run.sourceWorkspaceId` on the receiving side:
