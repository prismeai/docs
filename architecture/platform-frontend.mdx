...
On startup, the platform shell fetches IAM context via the native `/me` endpoint:

```
GET /v2/me
```

This returns the user profile merged with the active organization's context:
...
- **Role** — User's role, permissions, and scopes in the active org
- **Organizations list** — All orgs the user belongs to

The sidebar renders dynamically from this config. If no AI Governance workspace is configured, the shell falls back to a degraded mode showing only the builder link. See [IAM Context](/products/ai-governance/iam-context) for details.
