  ...
  <Tab title="Helm Chart Updates">
    <Steps>
      ...
      <Step title="Review Chart Values">
        ...
        ```bash
        # Get current values
        helm get values prismeai-core -n prisme-system > current-values.yaml
        
        # Compare with new chart defaults
        helm show values prismeai/prismeai-core --version X.Y.Z > new-defaults.yaml
      ...
      <Step title="Upgrade Core Components">
        ...
        ```bash
        helm upgrade prismeai-core prismeai/prismeai-core --version X.Y.Z \
          -f values.yaml \
      ...
      <Step title="Update Product Modules">
        ...
        ```bash
        # Update Chat
        helm upgrade prismeai-securechat prismeai/prismeai-securechat --version X.Y.Z \
          -f securechat-values.yaml \
          --namespace prisme-system
          
        # Update Knowledges
        helm upgrade prismeai-knowledge prismeai/prismeai-knowledge --version X.Y.Z \
...
<Steps>
  <Step title="Roll Back Helm Releases">
    ...
    helm history prismeai-core -n prisme-system
    
    # Rollback to previous revision
    helm rollback prismeai-core [REVISION] -n prisme-system
    
    # Repeat for other releases
    helm rollback prismeai-securechat [REVISION] -n prisme-system
    ```
  </Step>
