...
Where:
- **`${eventsPrefix}`** corresponds to the `vectorStore.vectorIndexPrefix` key in the Knowledges workspace configuration (if ES/OS is used as a vector database)
...
[MongoDB Atlas Actions List](https://www.mongodb.com/docs/atlas/reference/custom-role-actions/)  
Prisme.ai requires access to three databases. Two custom roles should be configured:

#### Core Role

This role grants access to the `users` and `permissions` databases, which store user accounts and access control data. The following actions are required:
...
This role grants access to the `collections` database, which stores workspace data and configurations. The following actions are required:
