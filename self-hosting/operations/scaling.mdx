...
Index mappings are initialized when prismeai-events starts up. If indexes and index mappings are deleted (such as when formatting a volume) without first stopping prismeai-events, the next event persistence request sent to the cluster would cause it to automatically infer an incorrect index mapping, which would cause most other persistence requests to fail and result in data loss.  
When Prisme.ai events mapping are not initialized before first events write requests, ES/OS automatically infer mappings for `payload.*` nested fields, making it incompatible with the needed `flattened` (ES) / `flat_object` (OS) mapping on the entire `payload` field.  
...
3. Starting prismeai-events by editing the deployment and setting `replicas: 1` 

**If events data cannot be lost**, the [existing indexes must me manually reindexed with the appropriate mapping](#reindexing-with-default-mapping).    
...
```json
{
  "index_templates": [
    ...
    {
      "name": "index-template-events-<id1>",
      "index_template": {
        "index_patterns": [
          "events-<id1>"
        ],
