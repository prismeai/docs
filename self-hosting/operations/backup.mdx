...
<Steps>
  ...
  <Step title="Back Up the Encryption Keys">
    The `masterKeys` / `refKeys` pair encrypts connector tokens and organization SSO configurations at rest. Without it, a restored database is unreadable — back it up outside the cluster, in the environment's vault.

    ```bash
    kubectl get secret <RELEASE>-secrets-encryption -n <NAMESPACE> \
    ...
    <Warning>
      A database backup restored without these keys loses every encrypted value permanently. See [Encryption Keys](/self-hosting/operations/encryption-keys) for the full procedure, including recovery when a key is lost.
    </Warning>
  </Step>
</Steps>
