---
title: "Migration v27"
description: "Step-by-step guide for migrating an existing Prisme.ai installation from legacy products (AI Knowledge, AI Store) to the new v27 platform products (Agent Factory, LLM Gateway, Storage)"
---

This guide walks you through migrating a self-hosted Prisme.ai installation from the **legacy AI products** (**AI Store**, **AI Knowledge**, ...) to the **new platform products** (**Agent Factory**, **LLM Gateway**, **Storage** - vector store, ...).
...
If your LLM and vector store credentials are currently injected through environment variables on **prismeai-runtime**, you need to **duplicate** the existing AI Knowledge variables under the new LLM Gateway and Storage prefixes.
...
```bash
helm upgrade prisme-core prisme/prisme-core -f core-values.yaml -n prisme-system
...
<CardGroup cols={2}>
  ...
  <Card title="extended" icon="boxes-stacked">
    Legacy AI products (AI Knowledge, AI Store, …), which will disappear in a future release.  
  </Card>
  <Card title="one-product" icon="box-archive">
    Main AI products (LLM Gateway, Storage, AI Governance, …).
  </Card>
</CardGroup>
...
### 4.1 AI Governance — bridge the legacy admin token

The new **AI Governance** workspace needs the admin token from the legacy one to perform the migration.

<Steps>
  <Step title="Copy the legacy token">
    In **Builder**, open the legacy **AI Governance** workspace → **Settings** → **Secrets** and copy the value of `adminAccessToken`.
  </Step>
  <Step title="Paste it into the new workspace">
    Open the new **AI Governance** workspace → **Settings** → **Secrets**, paste the value into `adminAccessToken`, then **Save**.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Import from AI Knowledge">
    Open the **⋮** (three-dot) menu in the top-right corner and click **Import from AI Knowledge**.
  </Step>
...
<Steps>
  ...
  <Step title="Import from AI Knowledge">
    Open the **⋮** menu and click **Import from AI Knowledge**.
  </Step>
  <Step title="Verify each model">
    For every imported model:

    - Open it and click **Test** — the model response is shown below the button.
    - For **embedding** models, ensure the **dimensions** option is properly set. This was often left undefined in legacy AI Knowledge and must now be explicit.
  </Step>
</Steps>
...
<CardGroup cols={2}> 
  <Card title="AI Governance" icon="shield-halved" href="/products/ai-governance/identity-access">
