...
| `Builder` | `builder` |
| `Admin` | `org:admin` |
| `PlatformAdmin`, `superadmin` | `org:owner` |

This table is **not hard-coded**: it is the default value of the `LEGACY_ROLE_MAPPING` environment variable. The variable takes a JSON object of the form `{ "<legacy role>": "<org role>" }`. When it is left unset, the default mapping above applies.

A platform administrator can set this variable to **map legacy roles to the org roles of their choice**, fully overriding the default conversion — for example if your legacy install used custom roles, or if you want legacy roles to land on different org roles. Any legacy role missing from the mapping receives no org role from the default, so its users must have **Assign role** set explicitly on the join rule.

```bash
# Default value (equivalent to the table above). Override as needed.
LEGACY_ROLE_MAPPING='{"User":"org:member","KnowledgeAdmin":"agent-maker","Knowledge User":"agent-maker","Builder":"builder","Admin":"org:admin","PlatformAdmin":"org:owner","superadmin":"org:owner"}'
```

<Note>
  In the JSON value, the key for the "Knowledge User" role is written with a space (`"Knowledge User"`) — this is the exact key the variable expects.
</Note>
