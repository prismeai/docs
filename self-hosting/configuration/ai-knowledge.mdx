...
AI Knowledge creates between 1 and 2 indices per agent (so 2 - 4 shards with default redundacy settings).  
...
|-------|---------|---------|---------|
| Crawler index | Full-text storage | Raw document text | `<vectorIndexPrefix>_<AIK_ID>_` |
| Vector index | Semantic search | Chunk embeddings | `<crawlerPrefix>-searchengine-webpages-` |

* **vectorIndexPrefix** is the prefix configured in AI Knowledge configuration at `vectorStore.vectorIndexPrefix` key
* **AIK_ID** is the AI Knowledge workspace id 
...
Indexes are created automatically when the **first document** is added to an AI Knowledge project.
