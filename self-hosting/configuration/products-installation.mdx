...
- `prismeai-functions` only needs an npm registry if you plan writting custom JS/Python with extra packages beyond the ones bundled in the image. See [Custom Code Functions — NPM Registry Access](/self-hosting/entreprise/functions#optional-npm-registry-access).
...
| 2 | `base2` | Extended apps such as Crawler and search integrations. |
