...
| Redis | **Memorystore Redis Cluster** with **RedisJSON + RediSearch** modules for AI Knowledge. See [Redis](/self-hosting/databases/redis). |
| Object storage | **Cloud Storage** + **Cloud CDN** for public assets. |
...
The shared RWX PVC (used by `prismeai-functions`) must be backed by **Filestore with regional redundancy** so it survives a zonal outage.
...
Reference the `BackendConfig` from the Service via the `cloud.google.com/backend-config` annotation. The full annotation reference lives in [Helm install — Ingress and load balancer](/self-hosting/kubernetes/helm#5-ingress-and-load-balancer).
