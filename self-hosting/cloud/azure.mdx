...
| Redis | **Azure Managed Redis**. Clustered by default → set `BROKER_CLUSTER_HASH_TAG` (see below). See [Redis](/self-hosting/databases/redis). |
...
<Warning>
**Azure Managed Redis is clustered by default.** When it backs the **event broker**, you must set `BROKER_CLUSTER_HASH_TAG` (any non-empty value, e.g. `prismeai`) on every backend service, otherwise the broker fails at startup with `CROSSSLOT` errors on its multi-stream reads. See [Redis › Cluster mode](/self-hosting/databases/redis#cluster-mode-hash-tag).
</Warning>
