...
| **High-throughput** | Thousands of concurrent users, max throughput per pod. Requests are sized at the maximum each service can actually use — these are the chart defaults. |
...
Pick **Balanced** if your traffic is mostly steady and budget matters. Pick **High-throughput** if you need each pod to absorb large bursts on its own.
