...
Moving an existing platform to a new tag is a two-phase operation: bump the tags, then update the products from the Platform workspace. Follow [Updates](/self-hosting/operations/updates) for the full procedure.
...
| `MFA_FORCE_LOCAL`<br/>Helm: `prismeai-api-gateway.env` | `true` | Require a second factor (passkey or authenticator app) for all local (password) accounts. SSO and anonymous accounts are exempt. Set to `false` to disable. |
| `WEBAUTHN_RP_ID`<br/>Helm: `prismeai-api-gateway.env` | - | Domain passkeys are bound to (the console domain or a parent domain). Passkeys stay disabled while it is unset. Choose it once: changing it makes existing passkeys unusable. See [Enable passkeys](/self-hosting/entreprise/authentication#enable-passkeys). |
| `WEBAUTHN_ORIGINS`<br/>Helm: `prismeai-api-gateway.env` | `CONSOLE_URL` origins | Origins accepted when verifying a passkey, compared exactly. |
| `WEBAUTHN_RP_NAME`<br/>Helm: `prismeai-api-gateway.env` | `Prisme.ai` | Name displayed by the user's passkey manager. |
| `WEBAUTHN_PASSWORDLESS`<br/>Helm: `prismeai-api-gateway.env` | `true` | Allow signing in with a passkey alone. |
...
- Clients (the console, APIs, automations) hold an **access token JWT** and present it on every request. When the JWT expires, the client bounces through the gateway, which, if the session cookie is still alive, hands back a fresh JWT.
...
A single Prisme.ai platform can serve its **front-end (the console)** on **several domains at once** (e.g. `studio.example.com`, `team-a.example.com`, `team-b.example.com`), all backed by the same core services. This is the standard multi-tenant SaaS setup.
...
| `CRAWLER_RELIABILITY_MODE`<br/>Helm: `prismeai-crawler.env`, `prismeai-searchengine.env` | `legacy` | Choose `legacy`, `durable-webhooks` (durable, at-least-once webhooks), or `full` (also adds atomic crawl admission and idle shutdown). Set the same value on both services. In `full`, `/index` may return `503` under backpressure. See [Reliability mode](/self-hosting/entreprise/crawler-searchengine#reliability-mode). |

<Note>
  The crawler pair requires Redis to use `maxmemory-policy=noeviction`. The searchengine checks it at startup in every mode; the crawler also checks it in `durable-webhooks` and `full`. See [Redis configuration](/self-hosting/databases/redis#maxmemory-policy-per-instance).
</Note>
...
<Warning>
  If the master keys are lost, **all encrypted workspace data becomes permanently unrecoverable.** Back them up the moment they exist and store them in a vault or password manager. If it already happened, follow [Recover a Lost Encryption Key](/self-hosting/operations/encryption-keys).
</Warning>
