...
**Mandatory** if any base URL the platform calls — your LLM gateway, vector store, OIDC IdP, internal APIs reached from `prismeai-runtime` — is signed by a private PKI or self-signed certificate. Configure `global.extraCABundle` to propagate your CA bundle to every core pod in one shot. Full step-by-step → [Trusting an extra CA bundle](/self-hosting/kubernetes/configuration#trusting-an-extra-ca-bundle).
...
**Mandatory** if any base URL the apps call — Elasticsearch/OpenSearch for the crawler, Redis for the searchengine, an internal LLM endpoint, third-party APIs hit by user Custom Code — is signed by a private PKI or self-signed certificate. Configure `global.extraCABundle` to propagate your CA bundle to every apps pod; an init container automatically merges it with the system trust store so public HTTPS keeps working. Full step-by-step → [Trusting an extra CA bundle](/self-hosting/kubernetes/configuration#trusting-an-extra-ca-bundle).
...
This **core-ingress** object should already be configured in prismeai-core Helm chart default values.  
