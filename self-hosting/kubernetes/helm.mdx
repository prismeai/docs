...
global:
  ...
  broker:
    ...
    secretKeys: # Fill each key with the actual field name in your existingSecret (see rule below).
      url: ''
      user: ''
      password: ''
...
If a master key is ever lost or overwritten, follow [Recover a Lost Encryption Key](/self-hosting/operations/encryption-keys).
...
prismeai-api-gateway:
  storage:
    users:
      ...
      secretKeys: # Fill each key with the actual field name in your existingSecret.
        url: ''
        user: ''
        password: ''
...
global:
  ...
  storage:
    documents:
      ...
      secretKeys: # Fill each key with the actual field name in your existingSecret (see rule in "User-provided secrets" above).
        url: ''
        user: ''
        password: ''
...
<Note>
  Upgrading to a **new platform version** is not just a `helm upgrade`: once the services run the new tag, the products must be updated from the Platform workspace. See [Updates](/self-hosting/operations/updates).
</Note>
