...
Also declare your **LLM and vector store credentials** now as `WORKSPACE_SECRET_*` env vars on `prismeai-runtime` — the LLM Gateway and Storage workspaces won't function without them. See [LLM & vector store credentials](/self-hosting/kubernetes/configuration#llm--vector-store-credentials) for the full list.
...
If any service reports an error, revisit the corresponding values in steps 2–4 or 6 and re-run `helm upgrade` before continuing.
