...
Don't forget to append your **HTTP-based** external hosts to `noProxy` (managed Elasticsearch/OpenSearch, S3 / Blob / GCS, internal LLM gateway, OIDC IdP) — Mongo/Postgres/Redis are binary protocols and bypass the proxy natively.

Full step-by-step → [HTTP/HTTPS Proxy](/self-hosting/kubernetes/configuration#httphttps-proxy).

---

## 5. Install both charts
