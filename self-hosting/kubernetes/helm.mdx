...
- Object storage (S3-compatible, Azure Blob, GCS) and/or a **RWX persistent volume** — see [Requirements](/self-hosting/requirements).
...
prismeai-api-gateway:
  ...
  env:
    - { name: UPLOADS_MAX_SIZE, value: "100000000" }  # 100 MB max upload
    - { name: SERVER_KEEPALIVE_TIMEOUT, value: "70000" }
    - { name: ACCOUNT_VALIDATION_METHOD, value: "auto" }   # auto | email | manual
...
prismeai-workspaces:
  env:
    - { name: UPLOADS_MAX_SIZE, value: "100000000" }       # 100 MB
    - { name: UPLOADS_ALLOWED_MIMETYPES, value: "image/*,text/*,video/*,audio/*,application/*,font/*,message/rfc822" }
    - { name: CUSTOM_DOMAINS_CNAME, value: "pages.example.com" }
```
...
prismeai-runtime:
  ...
  env:
    - { name: RUNNER_MAX_THREADS, value: "4" }              # Increase to use multi-core nodes
    - { name: BROKER_EMIT_MAXLEN, value: "1000000" }        # Event payload soft limit
    - { name: UPLOADS_MAX_SIZE, value: "100000000" }
```
...
prismeai-events:
  ...
  env:
    - { name: EVENTS_BUFFER_FLUSH_EVERY, value: "1000" }
    - { name: BROKER_EMIT_MAXLEN, value: "1000000" }
    - { name: ELASTIC_SEARCH_TIMEOUT, value: "10000ms" }    # Default 2s is too short for heavy queries
    - { name: SOCKETIO_COOKIE_MAX_AGE, value: "1200" }
```
...
prismeai-functions:
  env:
    - { name: REQUEST_MAX_SIZE, value: "20mb" }
    - { name: NODE_OPTIONS, value: "--max-old-space-size=2200" }
    - { name: KERNEL_POOL_SIZE, value: "10" }               # Python jupyter processes per pod
  persistence:
...
prismeai-crawler:
  env:
    - { name: MAX_CONTENT_LEN, value: "2000000" }           # Max indexed doc size (2 M chars)
    - { name: ELASTIC_INDICES_PREFIX, value: "" }           # Set when sharing Elasticsearch
```
...
spec:
  rules:
    - host: api.example.com
      http: { paths: [{ path: "/", pathType: Prefix, backend: { service: { name: prismeai-core-prismeai-api-gateway, port: { number: 80 } } } }] }
    - host: studio.example.com
      http: { paths: [{ path: "/", pathType: Prefix, backend: { service: { name: prismeai-core-prismeai-console, port: { number: 80 } } } }] }
  tls:
    - hosts: ["api.example.com", "studio.example.com"]
