...
| Name | Service | Description | Default Value |
|------|---------|-------------|---------------|
| WORKSPACES_STORAGE_TYPE | runtime & workspaces | Storage driver type (FILESYSTEM, S3_LIKE, or AZURE_BLOB) | FILESYSTEM |
...
| WORKSPACES_STORAGE_S3_LIKE_BASE_URL | runtime & workspaces | Base download URL (if omitted, workspaces API will be used as proxy) | |
...
| Name | Service | Description | Default Value |
|------|---------|-------------|---------------|
| UPLOADS_STORAGE_TYPE | workspaces | Storage driver type (FILESYSTEM, S3_LIKE, or AZURE_BLOB) | FILESYSTEM |
| UPLOADS_STORAGE_FILESYSTEM_DIRPATH | workspaces | Directory path for filesystem storage | ../../data/uploads |
...
| UPLOADS_STORAGE_AZURE_BLOB_CONTAINER | workspaces | Uploads Azure Blob container name |  |
 ...
 **Notes on uploads bucket:** 
 ...
 By default, the S3 driver stores all uploads inside the same bucket for both public and private files. This bucket must allow public access and enable object-level ACLs, letting the S3 driver selectively set objects as public or private.
 
 If these S3 options are restricted in your environment, you can configure two separate S3 buckets for public/private objects:
 
 1. Both buckets can maintain default S3 settings (which prohibit public access and disable object-level ACLs)
...
To avoid public S3 buckets without creating a second bucket behind CloudFront, you can force all file download requests to go through the Prisme.ai API by **not providing** the `UPLOADS_STORAGE_S3_LIKE_BASE_URL` environment variable.
