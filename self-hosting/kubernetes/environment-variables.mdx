...
| BROKER_CLUSTER_HASH_TAG      | api-gateway, workspaces, events, runtime | **Required when the broker Redis runs in cluster mode** (e.g. Azure Managed Redis). Wraps all broker stream keys in a common `{tag}` hash tag so they map to the same cluster slot, avoiding `CROSSSLOT` errors on multi-stream `XREAD`/`XREADGROUP`. Any non-empty value works (e.g. `prismeai`). Leave **unset** on single-node / non-cluster Redis. See [Redis › Cluster mode](/self-hosting/databases/redis#cluster-mode-hash-tag). |                          |
