...
#### Secrets read by scheduled runs

A scheduled run carries no user, so it cannot read a workspace secret under anyone's permissions. That matters for connectors: the OAuth token a member deposited when connecting their account is exactly what a nightly synchronisation needs to replay.

There is nothing to configure here. A workspace states in its own security rules that its scheduled runs may read a family of its secrets, and the platform grants those runs the reserved `schedules` role so ordinary RBAC decides the rest. See [RBAC and secure secrets](/products/ai-builder/rbac#secure-secrets) for how a workspace declares it.

<Warning>
Nothing can present itself as `schedules`. The role refuses any `auth` declaration, so no API key and no identity-provider binding can take it on, and the grant reaches only the workspace whose own scheduler started the run. A workspace called by another workspace's schedule gains nothing from it.
</Warning>
