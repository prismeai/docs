...
A scheduled run — a knowledge base synchronisation, for instance — carries no user, so it cannot read a workspace secret under the caller's own permissions. Reading one requires an explicit delegation, which only the platform may grant.

The import writes that delegation itself, so a connector shipped with the platform synchronises without any manual setup. It derives **which** secrets are delegated from each workspace's own `secure_secrets` security rules, and takes **who** may read them from the variable below.

| Name | Service | Description | Default Value |
| --- | --- | --- | --- |
| `DELEGATED_SECRET_CALLERS` | workspaces | Comma-separated list of workspace **slugs** allowed to read another workspace's delegated secrets without a user. Resolved to workspace ids when written. | `knowledge-sync` |

The default covers the standard platform: `knowledge-sync` is the component that runs scheduled knowledge base ingestion. Override it only to add a caller of your own, and note that an empty value falls back to the default — to grant no delegation at all, set it to a slug that does not exist.

<Warning>
A workspace can neither write nor read these declarations itself: they live in the reserved `prismeai_` namespace. This is deliberate — it prevents a connector from authorising access to its own users' credentials. Only the platform import may grant a delegation.
</Warning>
