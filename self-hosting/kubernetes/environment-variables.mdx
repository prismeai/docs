...
Platform git repositories are shared repositories automatically available to **all workspaces** for versioning (push/pull). They are configured entirely through environment variables and require no per-workspace setup.

Each workspace's files are stored in a subdirectory named after the workspace slug within the git repository. This allows a single repository to serve as a centralized versioning backend for every workspace on the platform.
...
| --------------- | ----------------------------------------------------------------- | ------------- |
| `URL`          | Git repository URL (**required** — the repository is only enabled if this is set) |               |
| `NAME`         | Display name shown in the UI                                      | `{repoId}`    |
| `BRANCH`       | Git branch to use                                                 | `main`        |
| `MODE`         | Access mode: `read-write` or `read-only`                          | `read-write`  |
| `DIRPATH`      | Base directory path inside the repository. Each workspace's files are stored under `{dirpath}/{workspaceSlug}`. If omitted, workspace files are stored directly under `{workspaceSlug}` at the repository root. |               |
| `AUTH_USER`    | Username for HTTPS authentication                                 |               |
| `AUTH_PASSWORD` | Password or personal access token (PAT) for HTTPS authentication |               |
| `AUTH_SSHKEY`  | SSH private key content for SSH authentication                    |               |

**Example — single platform repository:**
...
**Example — with a custom directory path:**
...
With this configuration, a workspace with slug `myapp` would be stored under `prisme/workspaces/myapp/` in the repository.

**Example — multiple platform repositories:**

```dotenv
# Primary read-write repository
