...
| CONSOLE_URL      | api-gateway, console, runtime        | Console URL, used for emails, auth redirections & runtime variable `{{global.studioUrl}}`. Accepts a **comma-separated list** to expose the same frontend under several domains: every URL serves the same console and all of them call the single `API_URL`. All listed origins are auto-allowlisted for CORS and OIDC redirects.                                                                                                            | http://studio.local.prisme.ai:3000    |
...
| BROKER_TOPIC_MAXLEN          | api-gateway, workspaces, events, runtime | Redis streams max length before getting truncated ([Capped Streams](https://redis.io/topics/streams-intro#capped-streams)) | 10000                    |
...
| OIDC_EXTERNAL_CLIENTS          | api-gateway                          | JSON array of extra OIDC clients to declare, e.g. native mobile apps (see note below). Each entry needs at least `client_id` and `redirect_uris`; the rest defaults to a public PKCE native client. Defaults to a ready-to-use client for the Prisme.ai mobile app (`client_id: ai.prisme.app`); override to change or extend it. | Prisme.ai mobile client (see note)                             |
| OIDC_USERINFO_TIMEOUT          | api-gateway                          | Timeout in milliseconds for the call to an external OIDC provider's `userinfo_endpoint` (see [Authentication](/self-hosting/entreprise/authentication#reading-profile-claims-from-the-userinfo-endpoint)) | 5000                                                           |
...
Besides the studio, whose `redirect_uris` are web console URLs, the api-gateway
also ships a **default native client for the Prisme.ai mobile app** so it works
out of the box — no configuration needed:

```json
[
  {
    "client_id": "ai.prisme.app",
    "redirect_uris": ["ai.prisme.app://oauth/callback"],
    "post_logout_redirect_uris": ["ai.prisme.app://oauth/logout"]
  }
]
```

Set `OIDC_EXTERNAL_CLIENTS` to **override** this default and declare your own
public clients — the variable **replaces** the built-in mobile client entirely,
so re-add it if you still need it.

The value is a **JSON array** of [oidc-provider client metadata](https://github.com/panva/node-oidc-provider/blob/main/docs/README.md#clients)
objects. Only `client_id` and `redirect_uris` are required; the api-gateway applies
defaults so every entry is treated as a **public PKCE native client**:

| Field                        | Default                                                                                      | Overridable |
| ---------------------------- | -------------------------------------------------------------------------------------------- | ----------- |
| `application_type`           | `native` (allows custom URI schemes and https claimed links)                                 | yes         |
| `grant_types`                | `["authorization_code", "refresh_token"]`                                                    | yes         |
| `response_types`             | `["code"]`                                                                                    | yes         |
| `allowedResources`           | the platform API resource server                                                             | yes         |
| `resourceScopes`             | same broad scope set as the studio client                                                    | yes         |
| `token_endpoint_auth_method` | `none` (public client) — **forced, not configurable**                                        | no          |
| `isInternalClient`           | `false` — **forced**                                                                          | no          |

PKCE is automatically required for these public clients — no extra setting needed.

<Warning>
For a native client, `oidc-provider` validates the `redirect_uris` per RFC 8252:

- **Custom URI scheme** → must be **reverse-domain** (the scheme must contain a `.`).
  `ai.prisme.app://oauth/callback` is accepted, `prisme://oauth/callback` is **rejected**.
...
OIDC_EXTERNAL_CLIENTS='[
  ...
  {
    "client_id": "com.acme.myapp",
    "redirect_uris": ["com.acme.myapp://oauth/callback"]
  }
]'
```
...
| WEB_SESSION_TICKET_TTL      | api-gateway | Default lifetime of a web session ticket, in seconds (`POST /v2/user/webSessionTicket`) | 60 |
| WEB_SESSION_TICKET_MAX_TTL  | api-gateway | Maximum lifetime a caller may request for a web session ticket, in seconds  | 300 (5 min)                         |
| WEB_SESSION_ALLOWED_REDIRECT_HOSTS | api-gateway | Extra hostnames accepted as a web session `redirect` target, on top of the platform's own API, console and pages hosts. Comma separated, hostname only (no scheme) | |
...
| MFA_FORCE_LOCAL                 | api-gateway    | Require a second factor (passkey or authenticator app) for all local (password) accounts. SSO and anonymous accounts are exempt. Set to `false` to disable. See [Multi-Factor Authentication](/self-hosting/entreprise/authentication#multi-factor-authentication-mfa). | true |
| WEBAUTHN_RP_ID                  | api-gateway    | Domain passkeys are bound to: the console domain or one of its parent domains. Passkeys stay disabled while it is unset. Choose it once: changing it makes existing passkeys unusable. See [Enable passkeys](/self-hosting/entreprise/authentication#enable-passkeys). | |
| WEBAUTHN_ORIGINS                | api-gateway    | Comma-separated origins accepted when verifying a passkey, compared exactly. Each must be `WEBAUTHN_RP_ID` or one of its subdomains. | `CONSOLE_URL` origins |
| WEBAUTHN_RP_NAME                | api-gateway    | Name displayed by the user's passkey manager. | Prisme.ai |
| WEBAUTHN_PASSWORDLESS           | api-gateway    | Allow signing in with a passkey alone, without a password. | true |
| MFA_STEPUP_MAX_AGE              | api-gateway    | How long (seconds) a re-authentication allows changing second factors or pairing the mobile app. | 300 |
| RATE_LIMIT_PASSKEY_OPTIONS      | api-gateway    | Passwordless sign-in challenges per IP address and per minute. | 60 |
...
| FETCH_ALLOW_H2                                           | runtime | Use HTTP/2 for `fetch` instruction calls, negotiated via ALPN with fallback to HTTP/1.1                              | undici default (HTTP/1.1)                                |
| FETCH_HEADERS_TIMEOUT                                    | runtime | Inactivity timeout in milliseconds while waiting for the response headers of a `fetch` instruction. Overridable per call with the `timeout` option | 300000 (5 min, the undici default) |
| FETCH_BODY_TIMEOUT                                       | runtime | Inactivity timeout in milliseconds between two body chunks of a `fetch` instruction. Not a total duration cap: a response that keeps sending is never interrupted | 300000 (5 min, the undici default) |
| FETCH_EMIT_REQUEST_EVENT                                 | runtime | When enabled, emit a `runtime.fetch.requested` security audit event before each outbound `fetch` call, recording the request metadata (method, URL, header **names**, content-type, body size) — never any body nor header value — so the URLs called by workspaces can be investigated | false |
| WORKER_STREAM_FIRST_CHUNK_TIMEOUT                        | runtime | How long, in milliseconds, a streamed run waits for the model's first token before the stream is closed with an explicit time-to-first-token error | 60000 (60s) |
...
<Note>
**`FETCH_ALLOW_H2` and LLM streaming.** Some corporate egress proxies and TLS-inspecting
gateways buffer entire HTTP/1.1 response bodies before forwarding them. This breaks
Server-Sent Events from LLM providers: the agent's answer reaches the browser in one
block at the end of generation instead of token by token. HTTP/2 usually traverses those
same proxies unbuffered.
</Note>

<Note>
**Bounding silent peers.** `FETCH_HEADERS_TIMEOUT` and `FETCH_BODY_TIMEOUT` are **inactivity** bounds, not total duration caps, so a long streaming completion is never cut off. Their purpose is the opposite case: a host that accepts the connection and then says nothing, which used to hold the calling automation for the full 300s without emitting anything. Authors can tighten a single call with the `fetch` instruction's `timeout` option, see [Automations → `fetch`](/products/ai-builder/automations#fetch).
</Note>

<Note>
**`WORKER_STREAM_FIRST_CHUNK_TIMEOUT` and slow models.** When a model takes longer than this to emit its first token, the run now ends on an explicit time-to-first-token error, rather than on the generic "LLM gateway stream returned no data". Large generations on some providers legitimately exceed 60s to start; raise the variable rather than reading the error as a platform failure.
</Note>

<Note>
**`FETCH_EMIT_REQUEST_EVENT` for outbound-traffic auditing.** Off by default. When enabled, every `fetch` instruction emits a `runtime.fetch.requested` event just **before** the request is dispatched — so a call that hangs, times out or crashes still leaves a trace of the URL it targeted. The payload carries request metadata only (method, URL, header **names**, content-type, body size) and **never** any request body nor any header value, so it is safe to persist and search. Failures are still reported separately by `runtime.fetch.failed`. Intra-platform workspace→workspace calls (webhook direct calls) are not audited. Platform-wide switch only — it cannot be toggled per `fetch`.
</Note>
...
A run that carries no user cannot read a workspace secret under anyone's permissions. That matters for connectors: the OAuth token a member deposited when connecting their account is exactly what a nightly synchronisation needs to replay.

There is nothing to configure here, and no operator gesture at all. A workspace declares a role in its own security rules, grants it read over a family of its secrets, and its automations take that role on where they need it. See [RBAC and secure secrets](/products/ai-builder/rbac#secure-secrets).
...
For **automation rate limits** (also `RATE_LIMIT_*` but on `prismeai-runtime`), see [API Reference: Rate Limits](/api-reference/rate-limits#configuration-options). Unlike the gateway limits, those don't break execution; they **deliberately slow automations down** instead of failing them.
...
| RATE_LIMIT_TOKEN_EXCHANGE  | api-gateway | Maximum OIDC token exchanges (`/v2/login/token-exchange`) per IP per minute. Uses its own counter, separate from anonymous logins. Token exchange is usually brokered server-side, so all users share one source IP; raise this for production. | 60            |
...
| RATE_LIMIT_WEB_SESSION     | api-gateway | Maximum web session ticket exchanges (`GET /v2/user/webSession`) per IP per minute. The exchange is unauthenticated by nature, since it is what opens the session | 30 |
