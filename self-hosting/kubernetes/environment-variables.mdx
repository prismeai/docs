...
| OIDC_EXTERNAL_CLIENTS          | api-gateway                          | JSON array of extra OIDC clients to declare, e.g. native mobile apps (see note below). Each entry needs at least `client_id` and `redirect_uris`; the rest defaults to a public PKCE native client. | `[]`                                                           |
...
By default the only public first-party client is the studio, whose `redirect_uris`
are web console URLs — so a native redirect such as `ai.prisme.app://oauth/callback`
would be rejected with `invalid_redirect_uri`. Use `OIDC_EXTERNAL_CLIENTS` to declare
additional public clients (typically a native mobile app) directly from configuration.
