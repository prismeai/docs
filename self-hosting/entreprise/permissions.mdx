...
In addition to the scopes declared on a role, Agent Creator and Knowledge projects expose their own **Share** dialog: a resource (one agent, one knowledge base) can be shared with specific users or groups under a product-managed role that maps back to the same `product:resource:action` permissions.
...
<Steps>
  ...
  <Step title="Per-resource role (Agent Creator / Knowledge)">
