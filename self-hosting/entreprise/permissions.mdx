...
<CardGroup cols={2}>
  ...
  <Card title="Workspace Roles" icon="folder-tree">
    Developer and manager permissions assigned on Builder workspace-by-workspace basis
  </Card>
...
<Tabs>
  ...
  <Tab title="Products">
    ...
    **Products are user-facing applications built on workspaces.** Knowledges, Agent Creator, AI Insight, for example, are Products.
...
<Accordion title="Understanding Product Roles">
  ...
  These roles are managed through Governe and apply to the end-user experience of the products.
</Accordion>

<Accordion title="Example Product Roles">
  Different products come with predefined roles. For example, in Knowledges:
...
<Accordion title="Managing Product Roles through Governe">
Role management is managed through the Governe module of the Prisme.ai platform.

Access to this Governance module is granted to users with the Manager role "Platform Manager" or "Group Manager":
- The "Platform Manager" Manager role allows the user to administer all configurable features of Governe.
...
- **Authorized:** grants access to the Builder module of the Prisme.ai platform
</Accordion>

<Accordion title="Knowledges">

- **Authorized:** grants access to the Knowledges module of the Prisme.ai platform and the bank of Knowledges projects shared with the user:
  - If the project was shared in "User" mode: the user can access the instantiated Agent Creator assistant of the Knowledges project
  - If the project was shared in "Editor" mode: the user can access the Knowledges project and all its settings. However, they cannot grant access to other users
  - If the project was shared in "Owner" mode: the user can access the Knowledges project and all its settings, including adding users

- **Create Agents:** allows the user to create Knowledges projects and access all their settings

- **Admin:** grants access to all Knowledges projects of the instance

- **Publish agent:** grants the user the right to make the agent public via the "Settings" tab of an Knowledges project
</Accordion>

<Accordion title="Agent Creator">

- **Authorized:** grants access to the Agent Creator module of the Prisme.ai platform and the bank of Agent Creator projects shared with the user

- **Create Agents:** allows the user to create Agent Creator agents and access all their settings. To create Agent Creator Agents, the user must have the right to create Knowledges projects
</Accordion>

<Accordion title="Insights">

- **Authorized:** grants access to the Insights module of the Prisme.ai platform and the bank of supervision agents of Insights assistants that the user has created. The user also has the ability to create new Insights agents on Knowledges projects they have access to
</Accordion>
...
<Accordion title="Best Practices">
  ...
  - For day-to-day operations, Super Admins should share important workspaces (Knowledges, Agent Creator...) with secondary accounts that have more limited permissions
...
<Accordion title="How SSO Access Works">
  Prisme.ai products like Knowledges and Agent Creator use custom security rules to automatically grant access to authenticated SSO users.
...
<Accordion title="Configuring SSO Access">
  ...
  1. Access the product's workspace through Builder
...
<Steps>
  ...
  <Step title="Product Permission Evaluation">
    ...
    - The system verifies product-specific roles assigned in Governe
...
<Accordion title="Platform Administrator">
  ...
  **Recommended Permissions**:
  - Super Admin status
  - Access to all critical workspaces (Knowledges, Agent Creator, etc.)
  - Product Admin roles in key products
  - Manager role in Governe
...
<Accordion title="User Cannot Access Product Features">
  ...
  **Resolution Steps**:
  1. Check the user's product roles in Governe
...
<Accordion title="Super Admin Cannot Access a Product">
  ...
  **Resolution Steps**:
  ...
  4. If needed, update the user's permissions in Governe
</Accordion>
