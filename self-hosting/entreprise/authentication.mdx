...
- [**Insights**](/self-hosting/configuration/ai-insights#sso-access)

<Note>
  Legacy clients still running `prismeai-pages`: whenever you change the legacy `PAGES_HOST` environment variable, trigger a workspace update (e.g., by changing its description) to register the new redirect URI with your identity provider. New installations don't need this — `prismeai-pages` is disabled by default.
</Note>
