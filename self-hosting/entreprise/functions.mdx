...
<Accordion title="Optional: NPM Registry Access">
  All JavaScript dependencies needed by the v27 AI Products (Agent Creator, LLM Gateway, Storage, Governe, Insights, …) are **bundled into the `prismeai-functions` image**. On top of that, a set of commonly used JavaScript packages (node-fetch, js-yaml, dayjs, mongodb, redis, sharp, etc.) is **pre-installed in the Docker image** and shared across all workspaces.

  **No npm registry access is required to run the platform or the AI Products** — including in offline / private-network deployments.

  An npm registry is only needed if you write Custom Code that pulls in additional JavaScript packages beyond the bundled ones. In that case, you can:

  - Set `NPM_CONFIG_REGISTRY` to point to your own internal registry (typical for offline deployments with an internal mirror).
  - Or set the workspace `disableNpmRegistry` option to `true` to forbid any runtime `npm install` (only the bundled and pre-installed shared packages will be available).

  By default, when neither is set, the service tries to install any additional whitelisted packages from `https://www.npmjs.com/`.
</Accordion>
...
<Accordion title="Python Configuration">
  ...
  | `KERNEL_POOL_SIZE`               | Number of python processes (each thread can execute only 1 function at a time)                                     | 10 |
  ...
  | `PYTHON_PACKAGES_AUTO_INSTALL`   | Comma-separated list of pip packages to install at startup (supports version notation like `pandas==2.0.0`). Packages listed here are automatically added to `PYTHON_PACKAGES_WHITELIST_IMPORT` so they remain importable.        |                                      |
  | `PYTHON_PACKAGES_DIR`            | Path to the custom python packages install directory        | data/functions-py/packages/ |
  | `PYTHON_PACKAGES_WHITELIST_IMPORT` | Comma-separated list of allowed third-party packages for import. Automatically extended with packages from `PYTHON_PACKAGES_AUTO_INSTALL`.              | numpy, pandas, tabulate, dateparser, urllib3, requests, pptx, docx                                     |
...
<Accordion title="NodeJS Configuration">
  ...
  | `REQUEST_MAX_SIZE`                       | Maximum request body size (format from bodyParser.json)                                                            | `20mb`                                                       |
...
<CardGroup cols={2}>
  <Card title="Builder" icon="toolbox">
    Within Builder, you can create custom automations that execute functions, allowing you to extend workflows with custom logic.
  </Card>
  <Card title="Knowledges" icon="brain">
    Custom functions can be used as tools in Knowledges agents, enabling them to perform specialized operations like data transformation or external API calls.
  </Card>
  <Card title="Agent Creator" icon="store">
    Functions can power features in agents published to the Agent Creator, adding custom capabilities to shared agents.
  </Card>
