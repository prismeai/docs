...
<Accordion title="Python Configuration">
  ...
  | `PYTHON_PACKAGES_DIR`            | Path to the custom python packages install directory        | /www/apps/python-packages/ |
  | `PYTHON_PACKAGES_WHITELIST_IMPORT` | Comma-separated list of allowed third-party packages for import.              | numpy, pandas, geopandas, tabulate, dateparser, urllib3, requests                                     |
...
<Accordion title="NodeJS Configuration">
  ...
  | `NODEJS_PACKAGES_WHITELIST_IMPORT`       | Comma-separated list of allowed external packages for require() and install. When not set, all packages are allowed. |                                                             |
  | `NODEJS_PACKAGES_BLACKLIST_IMPORT`      | Comma-separated list of blocked npm packages for install. Useful when whitelist is not set.                         |                                                             |
