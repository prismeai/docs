...
- **Single shared cluster** for the three databases (`users`, `permissions`, `collections`); this keeps the initial deployment simple. 3-node cluster (primary + 2 standby on PostgreSQL, or a 3-node replica set on MongoDB), ≥ 2 GB RAM, 2 vCPU, 1,000 IOPS and ≥ 10 GB disk per node.
