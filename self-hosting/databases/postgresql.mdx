...
- **Raise `max_connections`** on the cluster (requires a restart on most managed services). Size it against the total expected pool: roughly _(replicas across all services connecting to that database) × (pool size per replica)_, plus headroom for migrations and admin sessions.
