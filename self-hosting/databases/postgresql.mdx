...
### Single-database deployments (schema isolation)

Some managed PostgreSQL offerings give you **one database and a non-superuser role that
cannot `CREATE DATABASE`** (e.g. Scalingo). In that case the three stores can share a
**single database**, isolated by **PostgreSQL schema**, by adding a `?schema=` query param
to each connection URL:

```
postgres://user:password@host:5432/appdb?sslmode=require&schema=users
postgres://user:password@host:5432/appdb?sslmode=require&schema=permissions
postgres://user:password@host:5432/appdb?sslmode=require&schema=collections
```

The backend reads `schema`, sets it as the MikroORM schema (every query and the schema
introspection are confined to that namespace), and creates the schema at startup
(`CREATE SCHEMA IF NOT EXISTS` — the role must **own** the database, which does not require
`CREATEDB`). Without `?schema=`, everything targets `public` as before.

<Note>
Isolation here is **logical** (by schema), not physical: a single shared role can technically
reach every schema, so this is not a security boundary between stores — use it for
consolidation on constrained managed Postgres, not for hard multi-tenant separation.
It works transparently behind **PgBouncer transaction mode** (queries are schema-qualified,
no session `search_path` dependency).
</Note>
...
| RPO | 5 min or less |
| RTO | 1 h or less |
| Retention | 30 days or more, with 7 days or more in PITR |
| Criticality | Maximum. Irreversible loss: without this database there is no authentication and no workspace data. |

<Warning>
PITR (WAL archiving) is required. A `pg_dump` alone does not meet an RPO under 24 hours.
</Warning>

<Note>
Once you scale with `RUNNER_MAX_THREADS` of 2 or more, run **PgBouncer in transaction mode**: connections multiply with pods and pools. See [scaling](/self-hosting/operations/scaling).
</Note>
