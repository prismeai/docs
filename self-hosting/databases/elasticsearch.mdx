...
| **AI Knowledge vector store** *(alternative to Redis)* | AI Knowledge product | Uses dense vector mappings. |
...
- **`${aikPrefix}`** corresponds to the `vectorStore.vectorIndexPrefix` key in the Knowledges workspace configuration (when ES/OS is used as the vector database).
