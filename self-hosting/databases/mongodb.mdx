...
By default, the three databases (`users`, `permissions`, `collections`) live on a **single shared cluster**, which keeps the initial deployment simple.
...
### Core role (`users` and `permissions`)
...
| Criticality | Maximum. Irreversible loss: authentication and workspace data. |

<Warning>
Continuous oplog backup is required. A `mongodump` alone does not meet an RPO under 24 hours. Keep the oplog window at 2x the backup interval or more.
</Warning>
...
The Collections app is hit by `apps-prismeai-functions` which is shared across tenants, so IAM-per-pod isn't an option. Create a standard user:
...
4. Store the credentials in your secret manager (e.g. `<tenant>/appstore/collections.mongoURI`), as the full URL including username and password.
