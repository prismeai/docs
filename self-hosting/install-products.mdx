...
This guide walks you through installing and configuring the **platform products** (**Agent Creator**, **LLM Gateway**, **Storage** (vector store), **Governe**, **Insights**, **Builder**, **Helper Agents**) on a self-hosted Prisme.ai deployment, once the platform itself is up and running.
...
<Steps>
  ...
  <Step title="Verify the agent">
    Open the **Agent Creator** product from the left menu; you should see your Secure Chat agent ready to use and test.
  </Step>
</Steps>

### 5.1 Customize the Secure Chat agent

The **Secure-chat Agent** workspace is the single source of truth for the agent: everything lives under `config.value.agent`: name, description, instructions, tools (including MCP capabilities), skills, starters and budgets. To customize Secure Chat later (add a capability, a skill, change the starters, …):

<Steps>
  <Step title="Edit the workspace configuration">
    In Builder, open the **Secure-chat Agent** workspace settings and edit the `agent` configuration; for example add an MCP tool under `agent.tools` or a skill under `agent.skills`.
  </Step>
  <Step title="Re-run update-agent with the agent id">
    Run `update-agent` again, this time passing the `agentId` returned by the first run:

    ```json
    { "agentId": "agent_be13a30ea3f24b14abfdf6cb6181d933" }
    ```

    Without `agentId`, the automation **creates a new agent** instead of updating the existing one. The output confirms `"updated": true`.
  </Step>
  <Step title="Publish the agent">
    Conversations always run on the **published** version of an agent; `update-agent` only updates the draft. Open the agent in **Agent Creator** and publish it: until you do, users keep seeing the previous configuration.
  </Step>
</Steps>

<Warning>
  Configuration changes take up to a minute to propagate to the runtime after saving. If you run `update-agent` immediately after saving, it may still read the previous values and silently re-apply them; wait a moment, or re-run the automation if your changes don't show up on the agent.
</Warning>

<Tip>
  If the agent's instructions contain a skill map (an "Activation des compétences" section listing when to activate each skill), keep it in sync when you add a skill: the model follows the map over individual skill descriptions.
</Tip>
