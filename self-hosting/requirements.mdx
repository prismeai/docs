...
If you need to support thousands of concurrent users, follow the **Performance** profile and scale node counts proportionally.
...
- **Minimum viable**: 3 nodes will run the platform at steady state but cannot absorb HPA peaks, so this is only suitable for non-production environments.
- **Performance profile**: start at 8 nodes and scale via cluster autoscaler.
- **Disk space**: Minimum 50 GB per node for ephemeral storage. See [Databases overview](/self-hosting/databases/overview#database-requirements) for per-database minimum disk space requirements
...
If **no object storage solution is available**, a filesystem can be used instead, but only for local development or single-replica evaluation setups.

<Warning>
**Object storage is strongly recommended for workspaces models in production.** Filesystem writes are not atomic: with multiple `runtime` / `workspaces` replicas (or an RWX volume shared across pods), concurrent writes to the same workspace can interleave and corrupt the model YAML files. Object storage (S3-compatible, Azure Blob or GCS) writes each object atomically and avoids this. See [`WORKSPACES_STORAGE_TYPE`](/self-hosting/kubernetes/environment-variables#workspaces-storage).
</Warning>
...
- `studio.<your-domain>` for the console.
...
- **TLS termination**: Terminate TLS at the ingress with a certificate covering API and console.
...
- **Network policies**: For default-deny clusters, open only the service, database and external egress flows listed in [Network Policies](/self-hosting/kubernetes/network-policies).
...
- **Elasticsearch or OpenSearch UI**: direct access to the cluster that backs the events store, crawler indexes and vector store. Critical for shard accounting, ILM monitoring, reindex / cleanup recipes (see [Elasticsearch Operations & Troubleshooting](/self-hosting/databases/elasticsearch#operations--troubleshooting)).
...
| Elasticsearch or OpenSearch | **Kibana** (or **OpenSearch Dashboards**) | Mandatory in practice: index browsing, ILM status, reindex tasks, shard allocation, slow queries. The Prisme.ai troubleshooting recipes assume you have it. |
...
- **OIDC Providers**: Compatible with Google, Azure, and other generic OIDC providers.
