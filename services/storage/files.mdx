...
| `purpose` | string | Classification label (default: `user_data`) |
...
- **Strategy** — `static` (fixed-size), `semantic` (heading-aware), or `auto` (detects content type: prose, markdown, code, structured)
...
Files are automatically cleaned up by the nightly `cleanup-expired-files` job based on the `user_data_expires_after` config (default: 1800 seconds). This is used for temporary files (e.g., conversation attachments) that don't need permanent storage.
