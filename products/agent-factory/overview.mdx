---
title: "Overview"
description: "Agent Creator: the no-code way to build agents that act, using your skills and knowledge."
---

<Frame>
  <img src="/images/flow-agent-creator.svg" alt="How Agent Creator works: write instructions, add capabilities (knowledge, tools, MCP), test in the Playground, publish to the Store" />
</Frame>

Agent Creator is the **no-code way onto the foundation**, designed for the people closest to the business need. It's where **AI champions** (business experts) and technical teams alike create agents without writing code: from a simple Q&A assistant to a sophisticated agent with tools and knowledge bases, everything in one place. The agents you build here run on the same governed, traceable platform as everything else, no separate stack, no shadow IT.

<Frame>
  <img src="/images/agent-factory-gallery.png" alt="Agent Creator Gallery" />
</Frame>
...
| Profile | Level | Planning | Reflection | Skills | Memory | Built-in tools | Max Turns | Tool Calls | Token Budget |
|---------|-------|----------|------------|--------|--------|----------------|-----------|------------|-------------|
| `simple` | 1 | No | No | No | Working | None | 1 | 0 | 50K |
| `workflow` | 2 | No | No | No | Working + Session | None | 10 | 10 | 100K |
| `agent_light` | 3 | No | No | Yes | Working + Session | Task tracking | 10 | 15 | 100K |
| `agent_full` | 4 | Yes | Yes | Yes | All (incl. long-term) | All | 30 | 50 | 300K |
| `orchestrator` | 5 | Yes | Yes | Yes | All | All + Delegation | 50 | 100 | 300K |

<Note>
**Knowledge bases and custom tools are not profile-gated.** Whatever you attach under Capabilities is available on every profile; what the table's **Built-in tools** column lists is the platform's own tools — task tracking (`todo_write`), asking the user a question mid-task (`human_ask_clarification`) and budget introspection (`budget_get_remaining`). `simple` and `workflow` ship without any of them: they answer with the tools you gave them and nothing else, which is also the fastest path to an answer. `agent_light` gets task tracking, which brings the mid-task question with it — an agent that can hold a plan has to be able to pause and ask. What otherwise bounds a low profile is its **tool call** and **turn** limits.
</Note>

<Note>
**Max Turns and Token Budget are profile defaults.** They bound the work done on **one message**, not on the whole conversation, and any agent can override them under [Settings → Budget Limits](/products/agent-factory/settings#budget-limits), within the built-in hard cap of 50 turns. The tool call and cost budgets that come with each profile are listed in [Defaults per profile](/products/agent-factory/settings#defaults-per-profile).
</Note>
...
<Tabs>
  ...
  <Tab title="Agent Light">
    Basic ReAct loop: the agent can reason and use tools iteratively, and keeps a visible task list, but without planning or reflection. Best for research assistants, data lookup, interactive troubleshooting.
  </Tab>
...
The widget ships a complete design-token palette (light and dark) scoped to its own root. Two options brand it, and they behave differently:

| Option | What it sets | When to use it |
|---|---|---|
| `colors` | `primary` / `accent` / `ring`, `background`, `foreground` only | A quick accent tweak. Pushing background and foreground far from the defaults can break contrast, since the ~18 other tokens keep theirs (the chat panel sits on `card`, which `colors` does not touch, which is how icons end up white on white) |
| `cssVariables` | Any or all of the tokens, applied inline on the embed root | Real branding. Inline custom properties beat every stylesheet rule whatever the CSS load order, so the host palette always wins |

```html
<script src="https://<platform-host>/embed.js"
  data-agent-id="…" data-workspace-slug="…"
  data-css-variables='{"--primary":"262 83% 58%","--primary-foreground":"0 0% 100%","--card":"0 0% 100%","--card-foreground":"240 10% 4%"}'>
</script>
```

The same object is accepted as the `cssVariables` mount option when you boot the widget from JavaScript.

<Warning>
Values are **raw HSL channels** (`H S% L%`, no `hsl()` wrapper), because the embed stylesheet reads them through `hsl(var(--token))`. Keys may omit the leading `--`. Always override a token **with its `-foreground` partner**: `background/foreground`, `card/card-foreground` (the chat panel), `popover/popover-foreground`, `primary/primary-foreground`, `secondary/secondary-foreground`, `muted/muted-foreground`, `accent/accent-foreground` (ghost-button hover), `destructive/destructive-foreground`, plus the structural `border`, `input` and `ring`. Moved in pairs, no surface can lose its contrast.
</Warning>

<Tip>
If you can only ship a stylesheet, raise your specificity above the embed's own block with a doubled class, `.prisme-embed-root.prisme-embed-root { --primary: 262 83% 58%; }`. `cssVariables` stays preferable: it cannot lose a load-order race.
</Tip>
