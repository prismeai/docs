...
{/* TODO: screenshot — chat tools dropdown open, with the "On demand" section visible */}
...
{/* TODO: screenshot — Permissions tab, Tool activation section with default radio and one per-tool override row */}
...
3. **Not a security boundary.** A motivated jailbreak can convince the agent to ask the user to run the tool indirectly. Treat the approval click, not the activation, as the source of truth.
4. **MCP granularity is coarse.** Locking an MCP locks every tool it exposes; no per-child toggle.
