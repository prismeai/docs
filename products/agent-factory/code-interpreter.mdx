...
Calls to the Code Interpreter are authenticated by **workspace identity** (trusted-source), not a per-call API key, so only Agent Factory agents can reach it. Because it runs arbitrary code, combine it with **Ask first** approval for any sensitive context. The sandbox isolation and import allow-list are managed by your platform operator.
