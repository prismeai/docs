...
Create an agent to automate tasks, answer questions, or help users accomplish goals. Agent Creator is the no-code door onto the foundation: the creation wizard guides you through the essential configuration, and the agent you build runs on the same governed, traceable platform as everything else, no separate stack.
...
<Frame>
  <img src="/images/agent-factory-create.png" alt="Create Agent Wizard" />
</Frame>
...
<Note>
Capability URLs that point at the current platform (e.g. built-in tool servers) are
exported as a portable `${PRISME_API_URL}` placeholder instead of a hardcoded host.
On import, the placeholder is automatically resolved to the target environment's API
URL, so an agent exported from one environment works after import into another without
manually editing tool URLs. External URLs (third-party MCP servers) are kept as-is.
Environment-specific resource IDs (such as a knowledge base's `vector_store_id`) are
not remapped and may need to be reconnected after import.
</Note>
