...
`simple` and `workflow` agents get none of the platform's own tools: they answer with the capabilities you attached and nothing else. `human_ask_clarification` always comes with `todo_write`, so an agent holding a plan can still pause to ask the user.
