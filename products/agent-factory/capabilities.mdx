...
Capabilities give your agent the ability to do more than just chat. Connect tools to call APIs, knowledge bases to search documents, and integrations to interact with external systems. Every capability is built on the shared foundation, so what you wire up once, a tool, a knowledge base, an MCP connector, can be reused across any agent rather than rebuilt per project.
...
<Note>
**Why some tools need configuration**: System tools like Web Search are pre-configured and work immediately. Custom Functions and MCP servers connect to endpoints outside the agent, so they need URLs and (for external services) credentials. The agent discovers available tools from descriptions you provide, then decides when to use each tool based on the user's request.
</Note>
...
### MCP servers with a self-signed certificate

By default the platform validates a custom MCP server's TLS certificate and refuses to connect if it can't be verified against a trusted CA. For an **internal MCP server presenting a self-signed certificate**, enable **Allow self-signed certificate** in the MCP server's configuration to skip that check.

<Warning>
Disabling TLS verification removes protection against man-in-the-middle attacks. Only enable it for a trusted internal server you control on a private network — never for a public endpoint.
</Warning>
...
|------|-------------|---------------|
| `conversation_file_search` | Search the files uploaded in the conversation — see [Conversation file search](/products/agent-factory/knowledge-architecture#conversation-file-search-what-happens-when-a-user-uploads-a-file) | All profiles (once a file is indexed) |
| `todo_write` | Create and manage task lists | Agent Light+ |
| `human_ask_clarification` | Ask the user for clarification | Agent Light+ |
...
| `memory_remember`, `memory_recall`, `memory_forget` | Store, search and delete long-term memories | Agent Full+ |
| `human_request_approval` | Request human approval for actions | Agent Full+ |
| `delegate_<sub-agent>` | Hand a subtask to that sub-agent — one tool per configured sub-agent | Any profile with sub-agents |

Working and session memory are not tools: the platform feeds them into the context itself. Knowledge bases are not system tools either: each one you attach becomes its own search tool, on every profile. `simple` and `workflow` agents get none of the platform's own tools beyond conversation file search: they answer with the capabilities you attached and nothing else. `human_ask_clarification` always comes with `todo_write`, so an agent holding a plan can still pause to ask the user.

### Asking the user for clarification

When an agent calls `human_ask_clarification`, the chat renders an interactive panel above the input rather than a plain message: numbered options to click, a free-text field, and a **Skip** action. A tool that asks several questions walks through them one at a time, showing progress and letting the user step back to a previous answer.

The question itself stays the message text, so the conversation reads normally once answered. The answer continues the same conversation, and a clarification left open survives a page refresh, so the panel is still there to answer when the user comes back.

<Note>
System tools are never gated behind [human approval](/products/agent-factory/tool-permissions). Asking for a clarification, writing a todo or reading the remaining budget is the agent's own machinery, not an action on the outside world, so an "always ask" policy applies to the agent's real tools instead of stopping on its internal ones.
</Note>
