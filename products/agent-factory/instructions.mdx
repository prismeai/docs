...
Instructions are the foundation of your agent's behavior. They tell the AI who it is, what it should do, and how it should interact with users. Because agents run on a natively traceable platform, your instructions can even weave in runtime context, the current user, the date, a correlation ID, so every conversation stays governed and auditable end to end.
