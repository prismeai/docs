...
Every answer carries a **View trace** action that opens the turn's execution trace — the fastest way to understand *why* the agent replied as it did:

- **What it found** — the knowledge-base passages retrieved and their relevance scores;
- **What it did** — the tool calls, with their arguments and results;
- **Chronology** — the tree of automations that ran, with timings, an errors banner that jumps to the failing step, and, per step, its inputs / outputs and the raw event;
- a **JSON export** of the whole trace.

It's available on each answer in the Playground **and** in Chat. Because a trace exposes what users sent the agent and the payloads it exchanged, reading one requires the dedicated `agent-factory:traces:read` permission (see [Identity & Access → Roles & Permissions](/products/ai-governance/identity-access#roles--permissions)) — it is deliberately separate from the right to edit the agent.
