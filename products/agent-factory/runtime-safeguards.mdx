...
Every agent on the platform runs on the same governed, traceable foundation, and every turn goes through its **ReAct loop**: think (LLM call) → act (tool execution) → observe → repeat. Without guardrails, this loop could spin forever, burn through your budget, or repeatedly feed the model the same context. The runtime ships with built-in protections that handle these cases without you having to configure anything, and exposes a handful of knobs for the cases where you want tighter limits.
...
| **Message Token Budget** | Cumulative input + output tokens for one message, cached input reads counted at 0.1x | Stream `limits_reached: token_budget`, exit to fallback |
...
<Note>
**Cached input counts at 0.1x.** The token budget covers one message, but every turn resends the conversation history, which providers serve from their prompt cache. Charging those cached reads in full made a long conversation exhaust its budget on history alone, before the agent had done any new work. They now count at a tenth of their volume, so the budget mostly measures what this turn actually adds.
</Note>

Left empty, each budget falls back to the value of the agent's [profile](/products/agent-factory/overview#agent-profiles).
...
If the loop exits because of a budget or hard cap (not because the LLM said it was done), the runtime makes **one final forced LLM call** with:

- `tool_choice: none`, meaning no more tools, just text
