...
The loop is decomposed into specialized sub-automations for each concern:

1. **Init state** (`_react-init-state`) — Unpack loop counters and state from the previous iteration
2. **LLM call** — Send the full context to the LLM via [LLM Gateway](/services/llm-gateway/overview) (`_llm-chat-completions`)
3. **Error handling** (`_react-handle-llm-error`) — Detect and handle LLM errors (rate limits, timeouts, malformed responses)
4. **Permission check** (`_react-check-permissions`) — Evaluate HITL policies for each tool call; resolve approvers via `_resolve-hitl-approvers`
5. **Tool execution** (`_react-execute-tools`) — Batch-execute tool calls in parallel, then process results sequentially via `_route-tool`
6. **Usage tracking** (`_react-accumulate-usage`) — Accumulate tokens, cost, and duration from the LLM response
7. **Budget check** — Verify tokens, tool calls, and cost are within profile limits
8. **Context size check** — If context exceeds `context_budget`, trigger compaction
