...
A **custom tool** lets your agent call any HTTP endpoint you control. The most common pattern is to build the endpoint as a [Builder automation](/products/ai-builder/automations), the full-code door onto the same foundation, and plug it into the agent in two minutes, but any reachable URL works (internal API, third-party SaaS, serverless function, etc.).
...
![Builder automation editor showing the Get customer profile automation with a Webhook trigger and an HTTP Request step](/images/howto/custom-tools-automation-editor.png)
...
{/* Screenshot to add: screenshot of the "Add Capability" modal on the Custom tab with "Custom Function" selected, then the configuration form with Display Name, Function Name, Endpoint URL, Description and Parameters Schema fields filled in. */}
...
{/* Screenshot to add: Playground screenshot showing an agent message with an expanded "Tool call" block listing the function name, the arguments JSON, and the response JSON. */}
