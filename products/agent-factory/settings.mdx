...
Add per-agent safety checks that run around the model: before it (input), after the response (output), or before a tool runs (action).

- **Input guardrails**: PII detection (mask or block), prompt injection (block), off-topic / topic guard (redirect or block).
- **Output guardrails**: toxicity (flag, block, or regenerate), hallucination (checks answers against the agent's knowledge base).

Each guardrail is added from the Capabilities catalog and tuned per agent. For the full list, the settings of each guardrail, and how to adapt them, see [Guardrails](./guardrails).

Tool-call approval (human-in-the-loop) is configured under [Tool Permissions](./tool-permissions), not here.
