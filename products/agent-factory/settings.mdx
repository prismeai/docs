...
Control resource usage. Each limit is checked **at the start of every turn**, before the next LLM call — a turn already in flight is never killed mid-way. Leave a field empty to fall back to the profile default (see below); if no `Max Turns` is set at all, the platform's built-in 50-turn hard cap still applies.

| Limit | Unit | Default (global) | Description |
|-------|------|------------------|-------------|
| **Max Turns** | LLM round-trips | 10 | Number of think→act→observe cycles in the agentic loop before it stops and hands off to the fallback |
| **Token Budget** | Cumulative input + output tokens | 20,000 | Total tokens across the whole conversation. Cached input reads are weighted at **0.1**, so re-reading cached context is cheap against the budget |
| **Tool Call Budget** | Tool invocations | 15 | Total tool calls across the whole conversation |
