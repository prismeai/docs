...
A scanned PDF or an image dropped into the chat can come back empty: the agent's `conversation_file_search` store uses a parser **without OCR** by default, so no text is extracted from documents that aren't already digitized.

To make the agent process such files, switch the parser of its conversation store to **Tika + OCR**:

<Steps>
  <Step title="Open Knowledges and reveal agent stores">
  ...
  <Step title="Locate the agent's conversation store">
    Find the store named **`<Agent Name> Conversations`** (the auto-created store described above).
  </Step>
  <Step title="Switch the parser to Tika + OCR">
    Open its **RAG Configuration** and set the parser to **Tika + OCR** (`tika-ocr`).
  </Step>
  <Step title="Reindex existing files">
    Reindex the store so documents already uploaded are reprocessed with OCR.
  </Step>
</Steps>

<Warning>
OCR significantly slows down indexing — every file ingested into this store now goes through the OCR pipeline. The change only applies to **new** uploads until you reindex the existing ones. See [RAG Settings → Configuring the Document Parser](/products/ai-knowledge/rag-settings#configuring-the-document-parser) for the full list of parser options.
</Warning>
