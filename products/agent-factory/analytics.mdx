...
Analytics show how users interact with your published agents. Because the platform is natively traceable, every agent run emits normalized events, these metrics come straight from the same event stream that powers governance and audit, with no extra instrumentation. Use these insights to improve agent quality and demonstrate value.
