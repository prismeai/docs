...
A knowledge base is a searchable collection of documents, the unit you work with in the foundation's knowledge layer. Create knowledge bases for different topics, departments, or projects, then connect them to agents that need that information so their answers stay grounded in your content. (For structured, tabular data, the counterpart is a [Collection](/products/ai-collection/overview).)
...
Changing the parser or chunking settings only drives the documents indexed from then on. Saving those settings therefore opens a dialog that lists what changed and offers two answers:

- **Do nothing** — the existing documents keep the settings they were indexed with
- **Reindex all documents** — every document, uploaded file and crawled page alike, is queued for re-processing with the settings just saved

You can also start a re-index at any time, without changing anything:

1. Go to **Settings**
2. Scroll to **Reindex** and click **Reindex all documents**
3. Confirm in the dialog, then wait for processing to complete

The documents go back to *pending*, then *in progress*, then *success* on their own — the Documents tab updates as each one finishes. Existing chunks are replaced document by document.

<Note>
The button is inert while the parser or chunking form holds unsaved changes: a re-index always runs on the settings currently saved, so save first. Starting a second re-index while one is still running is refused with "A reindex is already in progress".
</Note>
