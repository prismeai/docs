...
The documents can have tags set. You can use this data to reduce the load of the embedding model when it retrieve documents, so it search on fewer documents.
This can also solve some documents that say opposite thing, depending on the context.

You have three ways to use these tags:
...
<Tabs>
  <Tab title="Automatic">
    You can have an LLM deduce by himself the relevant tags to filter on, using the `self-query` option, in **AI > Self Query > Enabled**. For every question the documents will automatically be filtered.
  </Tab>
  <Tab title="Set by the user">
    In **AI > Self Query > Enabled by the user**. Users will see a "+" button in the AI Store to add tags manually
  </Tab>
  <Tab title="Mandatory for a user or group">
    In the **Users** tab, you can choose, per user or group, which tags to be mandatory for them.
  </Tab>
</Tabs>

When some tags are set in multiple features, the logic is as such :
- All the tags set in the `user` panel are combined with an "OR" together.
- All the tags set in the AI Store, from the chat, are combined with an "OR" together.
- Then, both list of tags are combined with an "AND" together.

This mean, a user with mandatory list [A,B] tags, selecting [C,D] tags can retrieve documents with at least the tags [A,C], [A,D], [B,C], [B,D]
