...
Documents can have **tags** assigned to them. These tags can be used to **reduce the load on the embedding model** when retrieving documents by narrowing the search scope. They can also help avoid conflicts when multiple documents express **opposing statements** depending on the context.

You have three ways to use tags:
...
<Tabs>
  ...
  <Tab title="Set by the user">
    Enable **AI \> Self Query \> Enabled by the user** to allow users to manually select tags via the **“+”** button in the AI Store.
  </Tab>
  <Tab title="Mandatory for a user or group">
    In the **Users** tab, you can define which tags are mandatory for specific users or user groups.\
    These tags will always be applied when retrieving documents for them.
  </Tab>
</Tabs>
