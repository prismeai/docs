...
Documents are the raw material of the knowledge layer: they are what your knowledge bases turn into grounded, retrievable answers for agents. This guide covers how to add, organize, and maintain your document collections.
...
When the knowledge base holds tagged pages, a row of tag cards sits above the list. Each card toggles the page results carrying that tag, so you can narrow a large crawled source to one section without typing a query. Tags applied at [crawl entry](./crawl-website#tag-the-pages-at-crawl-entry) show up here automatically, since every discovered page inherits them.
...
### How tags are matched at search time

Beyond the document list, tags are usable as **filters in knowledge base search**, either from the search API or from an agent calling a retrieval tool (`file_search`, `conversation_file_search`).

- **Matching is exact.** A filter on `invoice` returns documents tagged exactly `invoice`, never `invoice-2024` or `old_invoice`. Partial (substring) matching is not supported here, unlike the free-text search box described in [Searching Documents](#searching-documents).
- **Filter on the file name with `file_name`**, not with a tag. When an LLM calls a retrieval tool with a `filename` filter, the platform rewrites it to `file_name` so the search still works, but prefer the correct name when you write the filter yourself.

<Tip>
Because matching is exact, keep your tag vocabulary short and consistent. If you need hierarchy, encode it as separate tags (`invoice`, `2024`) rather than as one compound tag you would later want to match partially.
</Tip>
...
When you change chunking or parsing settings, existing documents keep their old chunks. Saving those settings offers to re-process them all right away (see [Reindexing a knowledge base](/products/ai-knowledge/knowledge-bases#reindexing)); the rest of this section covers doing it later, or one document at a time.
...
The dialog shown after saving the knowledge base's parser or chunking settings does this in one click. To start the same pass later:
