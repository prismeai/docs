...
<Tabs>
  <Tab title="Overview">
    ...
    | **Documents** | LLM when `parser=llm` | Complex layouts, scanned PDFs with OCR, preserving document structure |

    For images and audio, LLM processing is automatic when a capable model is configured. For documents, you must explicitly request LLM parsing.
  </Tab>

  <Tab title="Processing Pipeline">
    The LLM analysis follows a three-phase flow:

    **1. Parsing Phase**
    - File is sent to the configured LLM with vision/audio capabilities
    - LLM extracts and describes content as text
    - Result is stored as `document.text`
    ...
    **3. Query Phase**
    ...
    - For capable models, original files can be injected alongside text context
    - LLM generates response using both text chunks and file content
  </Tab>

  <Tab title="Configuration">
    Configure LLM parsing in your workspace:

    **Workspace Configuration**
    ```yaml
    # In workspace config
    defaultModels:
      parsing: "your-vision-capable-model"
    ```

    **Model Capabilities**

    Ensure your parsing model has the required capabilities:
    - `vision`: For image analysis
    - `audio`: For audio transcription
    - `file`: For document processing

    **UI Configuration**

    Navigate to: **AI Knowledge > Project Settings > Processing > Parser Model**
  </Tab>
  ...
  <Tab title="API Usage">
    ...
    ```bash
    curl -X POST "https://api.prisme.ai/v2/knowledge/{projectId}/documents" \
    ...
    **Parser Values**
    ...
    | `tika` | Apache Tika extraction (fast, free) |
    | `unstructured` | Unstructured.io extraction (structured output) |
    | `llm` | LLM-based extraction (intelligent, handles complex layouts) |
  </Tab>
</Tabs>
...
- **File size**: Subject to model context limits
- **Token costs**: Large documents consume significant tokens
- **Processing time**: Slower than traditional parsers
- **Model availability**: Requires a configured vision/audio-capable model
