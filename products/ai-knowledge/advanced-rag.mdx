...
While basic Retrieval Augmented Generation (RAG) is powerful for many use cases, complex knowledge scenarios often require more sophisticated approaches. Advanced RAG architectures push the knowledge layer further, addressing challenges such as multi-step reasoning, diverse information types, and specialized domain knowledge. Because every agent runs on the same governed foundation, these patterns can also draw on structured facts from [Collections](/products/ai-collection/overview) alongside retrieved documents.
...
## Agentic RAG with the Filesystem MCP

Classic RAG runs one semantic retrieval and hands the top passages to the model. Agentic RAG lets the agent **navigate** the knowledge base instead: it lists, searches, reads, and greps documents in a loop, deciding what to open next from what it just found. Each knowledge base exposes a [Model Context Protocol](https://modelcontextprotocol.io) endpoint that makes this access deterministic and exhaustive, complementary to semantic top-k search.

The endpoint is scoped to a single knowledge base:

```http
POST {{api_host}}/v1/knowledge_bases/:knowledgeBaseId/mcp
```

It speaks JSON-RPC 2.0 (MCP spec 2025-06-18). The knowledge base id is fixed by the URL, so an agent connected here can only read that base and never has to pass the id. Authentication reuses the same access control as the REST API, so an agent only ever reads what its identity is allowed to see.

### The nine tools

| Tool | Shell analogy | Purpose |
|------|---------------|---------|
| `kb_search` | semantic | Top-k passage retrieval, the classic RAG search |
| `kb_ls` | `ls` | List the documents in the base |
| `kb_find` | `find` | Find documents by name |
| `kb_cat` | `cat` | Read a full document, chunks reassembled and de-duplicated |
| `kb_grep` | `grep` | Find lines matching a pattern (literal by default), with line numbers |
| `kb_head` | `head` | First N lines of a document |
| `kb_tail` | `tail` | Last N lines of a document |
| `kb_sed` | `sed` | Return a transformed view of a document (output only, never persisted) |
| `kb_source_url` | citation | Resolve a clickable source link for a document |

### How an agent uses it

<Steps>
  <Step title="Connect the base">
    Add the knowledge base MCP endpoint as an MCP tool on the agent (see [Agent Capabilities](/products/agent-factory/capabilities)). The agent gains the nine tools above, already scoped to that base.
  </Step>
  <Step title="Locate">
    The agent narrows the search space with `kb_search` for a concept, or `kb_ls` and `kb_find` to target documents by name.
  </Step>
  <Step title="Read">
    It opens what it found: `kb_cat` for a whole document, `kb_head` and `kb_tail` for the edges, `kb_grep` for the exact lines around a token.
  </Step>
  <Step title="Refine">
    From what it read, it decides the next move: another grep, a neighbouring document, a full read, until it has enough to answer. That loop is what makes the retrieval agentic.
  </Step>
</Steps>

### Semantic search or filesystem tools

<CardGroup cols={2}>
  <Card title="Semantic search" icon="magnifying-glass">
    Open or conceptual questions where the answer is worded differently from the query. Use `kb_search`.
  </Card>
  <Card title="Filesystem tools" icon="folder-tree">
    Exact tokens, exhaustive reading, or full-document context: an identifier, a legal clause, a contract read end to end. Use `kb_grep`, `kb_cat`, `kb_head` and `kb_tail`.
  </Card>
</CardGroup>

The two compose: an agent can `kb_search` for a concept, `kb_cat` the document it surfaced to read it in full, then `kb_grep` for an exact reference that an embedding model would blur.

<Note>
  Document content returned by these tools is redacted from the platform activity and execution events. Exposing a knowledge base over MCP does not leak its content into logs.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="vial"
    href="/products/agent-factory/evaluations"
  >
