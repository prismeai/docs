...
<Warning>
**Per-user access control is off by default.** A connection created without it serves
every synced document to anyone who can query the agent, whatever their rights in the
source. Turn it on when the source holds anything that is not readable by the whole
organization. An administrator can also enforce it organization-wide, which overrides
the per-connection setting.
</Warning>
