...
| **Outlook** | Available from the catalog — mail folders, one message per document |
| **WebDAV** | Available by URL — Nextcloud, ownCloud, any WebDAV server, configured in your own workspace |

What you actually see in the picker is what **your** organization's catalog holds and what
has passed the probe, so the list can be shorter or longer than the one above — a
connector being rolled out appears as soon as its capability is saved.
...
<Note>
Other MCP capabilities (Confluence, Jira…) are listed in the catalog but do
...
<Frame>
  <img src="/images/knowledge-connectors/provider-picker.png" alt="The provider picker: the organization's compatible connectors, then Your own MCP server" />
</Frame>
...
<Frame>
  <img src="/images/knowledge-connectors/custom-server-dialog.png" alt="Connect your own MCP server: the URL field, before checking" />
</Frame>

<Frame>
  <img src="/images/knowledge-connectors/custom-server-compatible.png" alt="After Check: Compatible connector — webdav, Levels folder then file, and a Continue button" />
</Frame>
...
<Steps>
  ...
  <Step title="Sync, then verify">
    ...
    The connection then reads *Service account* under **Authentication**, and its source
    is the canonical WebDAV URI of the folder you picked — `webdav://files/…`.
  </Step>
</Steps>

<Frame>
  <img src="/images/knowledge-connectors/webdav-connection-synced.png" alt="A synced WebDAV connection: source webdav://files/…, Service account authentication, and its recent runs" />
</Frame>
...
|---|---|
| **Completed** | The traversal finished and everything it accepted went through |
| **Partial** | The traversal finished, some documents were synchronized, and others were **rejected** — an oversized file, an unsupported format, a file that could not be fetched |
| **Failed** | Nothing came through at all |
| **Running** / **Pending** | In progress, or waiting its turn |

**Partial is not a broken connection.** The connection stays *Active* and keeps its
schedule: a handful of rejected files must not take a working source out of the automatic
synchronization. Open **Synchronization errors** to see which documents were rejected and
why — that is where a partial run explains itself, not in the connection's health.

A run that is *Partial* with three zeros is therefore normal on a settled base: nothing
changed, and the same files as last time were rejected again.
