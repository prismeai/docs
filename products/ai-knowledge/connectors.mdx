...
| **File too large** | Files with a downloadable link (e.g. SharePoint) are fetched server-side and are only bound by the platform upload ceiling; this now only affects providers that send inline content with no link, which is capped at 2 MB |
