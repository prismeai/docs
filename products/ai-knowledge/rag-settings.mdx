...
Top K is the knowledge base default, not a hard ceiling. An agent connected to the knowledge base can set its own `max_num_results` on the knowledge tool, which applies to that agent's searches only. Leave it unset and the knowledge base's Top K applies.

This is useful when one knowledge base serves agents with different needs: a support agent answering short factual questions can stay at 5 chunks, while a research agent over the same corpus pulls 15, without either of them forcing a setting on the other.
