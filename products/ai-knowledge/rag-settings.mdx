...
RAG (Retrieval Augmented Generation) settings control how your documents are processed and retrieved, the mechanism that grounds an agent's answers in the knowledge layer rather than in the model's guesswork. Fine-tuning these settings can significantly improve the quality of agent responses.
...
The overlap slider stops at **half the chunk size**: past that point a recursive splitter can no longer make progress. Shrinking the chunk pulls an overlap that has become too large down with it, so every preset stays reachable.

<Note>
**On the `auto` strategy, an overlap is only sent when you chose one.** Leave the slider untouched on a new knowledge base and the backend picks the overlap itself, per detected content type (around 250 characters on prose, 500 on markdown and code). Move the slider, or open a knowledge base that already carries an explicit overlap, and that value is what applies, including an explicit `0`. The `static` and `semantic` strategies have no such backend default and always send both bounds.
</Note>
...
Saving opens a dialog that lists what changed and offers to re-process the documents already indexed. **Do nothing** leaves them on their current chunks; **Reindex all documents** queues the whole knowledge base with the settings you just saved. The offer does not appear on a knowledge base that holds no document yet.

<Warning>
Declining the offer is not undoable from a banner: the new chunking then applies to new documents only, and the existing ones keep their old chunks until you come back and use **Reindex all documents** in the Reindex section.
</Warning>

The **Override chunking** form of the re-index modal seeds itself from what the knowledge base actually holds, whether it was written from the interface or through the API in the flat `{ type: "static", max_chunk_size_tokens, chunk_overlap_tokens }` shape. Confirming a re-index without touching the sliders therefore re-applies the store's own configuration, not a default preset.
...
#### Overriding Top K from an agent

An agent connected to the knowledge base can set its own `max_num_results` on the knowledge tool, which applies to that agent's searches only. Leave it unset and the system default of **5 chunks** applies — the Top K configured here (Settings > Retrieval) is not inherited by agents that query the knowledge base through the knowledge tool.

This is useful when one knowledge base serves agents with different needs: a support agent answering short factual questions can stay at the default 5 chunks, while a research agent over the same corpus sets 15, without either of them forcing a setting on the other.
...
4. Click **Save**, then answer the re-index offer — **Reindex all documents** to re-process the existing ones with the new parser

<Warning>
Switching the parser only affects new documents until the existing ones are re-processed, either from the offer shown after saving or from **Reindex all documents** in the Reindex section. OCR parsers (`tika-ocr`, `unstructured-ocr`) extract text from scans and images but are noticeably slower to index.
</Warning>
