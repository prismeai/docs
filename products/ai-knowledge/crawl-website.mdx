...
description: "Discover, index, and keep website pages searchable in a knowledge base"
---
...
<Frame>
  <img src="/images/ai-knowledge-web-sources.png" alt="Knowledges base with configured web sources and crawled pages" />
</Frame>
...
| **Auto-Discovery** | You need Knowledges to follow links and index several pages from the same site. |
...
<Frame>
  <img src="/images/ai-knowledge-crawl-website-config.png" alt="Auto-Discovery configuration for an Knowledges website crawl" />
</Frame>
...
### Tag the pages at crawl entry

Tags can be set on the crawl itself, next to the starting URL, before you click **Start Crawling**. Every page the auto-discovery finds inherits those tags as it is indexed, so a crawl of `/support` can land pre-tagged as `support` without any per-document editing afterwards.

This is the cheapest way to keep a mixed knowledge base filterable: crawl each section as its own web source with its own entry tags, then filter or search on those tags later. See [Document Tags](./documents#document-tags) for how tags behave once the pages are indexed.
