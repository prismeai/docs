...
Runtime modules are the built-in capabilities you call from automations on the foundation, and the **crypto** module is one of them. It provides cryptographic helpers built on Node's `crypto`, with no external dependencies. Typical uses: **OAuth PKCE**, **webhook signature verification** (HMAC), and content fingerprints.
