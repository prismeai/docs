...
- To enable WebSockets with multiple instances and HTTP polling as a first choice method, HTTP traffic between api-gateway and events must be forwarded with sticky sessions
- Otherwise, users may experience HTTP 400 errors with "Session ID unknown" messages
- You can either configure these sticky sessions or set HTTP method as a **second-choice method**, by adding this environment variable to console and pages services (recommanded) :   
```
WEBSOCKETS_DEFAULT_TRANSPORTS='websocket,polling'
```

If you want to keep HTTP polling as main websocket method, here is an example Istio configuration for sticky sessions:
