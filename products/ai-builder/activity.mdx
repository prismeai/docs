...
When a `source.correlationId` filter is active, Activity unlocks a trace mode: a view selector appears with five ways to read the same correlated event set.

| View | What it shows |
| --- | --- |
| **Table** | The correlated events as rows |
| **Trace** | The full call tree of the turn — collapsible spans, an error banner that jumps to the failing step, each step's inputs/outputs and raw event, plus JSON export. The richest view for debugging, and the same viewer as the Agent Factory [answer trace](/products/agent-factory/playground#view-trace). |
