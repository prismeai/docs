...
| `schemas` | Conditional input/output schemas — the current way to declare the inputs (and outputs) an automation expects (see [Input & output schemas](#input--output-schemas)). Takes precedence over `arguments` for validation. |
| `arguments` | Legacy map of the inputs the automation expects. Still accepted and auto-converted into a single `schemas` branch on save (see [Input & output schemas](#input--output-schemas)). |
...
| `validateArguments` | When `true`, the runtime rejects calls whose inputs do not match the active `schemas` branch (or the branch derived from `arguments`). |
| `private` | When `true`, the automation cannot be called from outside the workspace (only by calling it directly from another automation, using its slug as an instruction key). |
...
Interpolation is **single-pass**: it never re-scans what it just produced. A `{{`
with no matching `}}` is therefore left alone, and a value that *contains*
`{{something}}` is never resolved a second time.

That is what lets you build a binding meant to be **stored rather than
resolved** — the reference an app instance writes into its configuration, for
instance, which only resolves later, in another context:
...
- set:
    name: secretBinding
    value: '{{openBrace}}secret.myKey}}'           # → the string "{{secret.myKey}}"
```

The result survives every later interpolation, nested object values included,
and a workspace export/import round-trip. Conversely, to detect a binding that
was *not* resolved, compare a slice of it: `'{% slice({{value}}, 0, 2) %}'`
against `{{openBrace}}`, variable to variable.
...
<Accordion title="URL (Webhook/API Endpoint)">
  ...
  <Warning>
    `headers` never contains the platform's own headers: every `x-prismeai-*` header (`x-prismeai-workspace-id` included), `cookie`, `host`, `forwarded`, `x-real-ip` and `x-forwarded-*` are removed before the automation runs, because a caller could forge them. To identify the workspace that called the webhook, read `{{run.sourceWorkspaceId}}`.
  </Warning>
  ...
  Long-running SSE endpoints are kept alive to avoid proxy timeouts. When the caller sends `Accept: text/event-stream`, a `: keepalive` line is emitted every 5 seconds from the first chunk until the connection ends. It is an SSE comment line: `EventSource` and any conforming SSE client ignore it, so it never reaches the caller as data.

  You can also set the interval yourself, which opens the stream right away even if the caller did not ask for one:

  ```yaml
  - set:
      name: $http
      value:
        sseKeepAlive: 10000 # Keep-alive interval in milliseconds (minimum 5000ms)
  ```

  <Note>
    Older runtimes sent the keep-alive as a `data: {"keepAlive": true}` chunk. Clients that parse `data:` lines themselves should keep discarding that frame.
  </Note>
...
<AccordionGroup>
  <Accordion title="Run Scope">
    ...
    - `run.sourceWorkspaceId`: ID of the workspace that called this automation, when it was called from another workspace
...
<AccordionGroup>
  ...
  <Accordion title="Repeat" icon="arrows-rotate">
    ...
    # Iterate through an OBJECT: each item is a { key, value } pair
    - repeat:
        on: '{{queryParams}}'
        do:
          - set:
    ...
    `on` accepts an array, an object, or a stream. Given an object, the loop
    walks its entries and exposes each one as `{{item.key}}` / `{{item.value}}`
    — which is how you iterate over the keys of an object without a helper
    function.
...
* **assumeRole** : Run this call under one of the workspace's own roles (see below)
...
#### Acting for the workspace with `assumeRole`

A workspace sometimes has to do more on its own data than the caller in front of it may do: replay the credentials its members deposited, read a resource an endpoint must not expose directly. `assumeRole` lets an automation say so, for one call:
...
- run:
    ...
    parameters:
      name: 'spnRefresh_{{targetUserId}}'
      scope: workspace
```

The role must be declared in the workspace's own [security rules](./rbac#secure-secrets), and only such a role can be taken on: native roles like `owner` are refused, so you have to declare one that states what it opens. What it grants is then bounded by what the platform accepts to store in a rule, and the automation around the call is your authorisation layer.

The grant is scoped to that single call. The instructions that follow, the automations this one calls, an `emit`, a call into another workspace: none of them carry it. And it replaces the caller's rights rather than adding to them, so the role has to be sufficient on its own for what the call does.
...
Pure-JS text processing utilities: chunking, URL parsing, PII masking, grep-style search, regex substitution, diff and patch, URI encoding and diacritic folding.

<Card title="Text Module Reference" icon="scissors" href="/products/ai-builder/module-text">
  Functions: `splitText`, `parseUrl`, `filterPii`, `search`, `substitute`, `diff`, `applyPatch`, `encodeUriComponent`, `removeDiacritics`
</Card>

#### Crypto

Cryptographic helpers with no external dependencies: hashes, HMAC signatures, base64 (URL-safe on demand) and random material — enough to cover OAuth PKCE, webhook signature verification and content fingerprints.

<Card title="Crypto Module Reference" icon="lock" href="/products/ai-builder/module-crypto">
  Functions: `sha256`, `sha512`, `md5`, `hmac`, `base64Encode`, `base64Decode`, `randomBytes`, `randomUUID`
</Card>
...
| `options.persist` | boolean | `true` | When `false`, the event is broadcast but not stored, so it is invisible in Activity. |

**Output:** none. **Raises:** none. **Often chained with:** `wait`.
...
| `timeout` | number (ms) | `FETCH_HEADERS_TIMEOUT` / `FETCH_BODY_TIMEOUT` (300000) | Inactivity bound, applied both while waiting for the response headers and between two body chunks. It is not a total duration cap: a response that keeps sending is never interrupted, so a long LLM completion is left intact. |
...
<Note>
**Bounding a server that never answers.** Without `timeout`, a host that accepts the connection and then stays silent holds the automation for the underlying 300s default: the tool stays "running", the task stays in the working status and no error is raised. Set `timeout` on calls to endpoints you do not control, such as remote MCP servers, so a silent peer fails fast instead of freezing the run. Self-hosters can change the platform-wide defaults with `FETCH_HEADERS_TIMEOUT` and `FETCH_BODY_TIMEOUT`, see [Environment Variables](/self-hosting/kubernetes/environment-variables).
</Note>
...
There is no dedicated instruction for calling another automation: **use the target automation's slug directly as the instruction key**, with its inputs as the value. This is the same mechanism used for imported app actions (see [App and workspace automation calls](#app-and-workspace-automation-calls)).

```yaml
- mySlackInstance.sendMessage:   # app action: <appInstanceSlug>.<action>
    channel: "#general"
    text: "Hello"

- myAutomationSlug:              # workspace automation: slug used directly
    someParam: value
    output: result               # capture the target's output into `result`
```

The inputs are validated against the target automation's `schemas` (or the branch derived from its `arguments`). As with every other instruction, adding an `output` key stores the target automation's `output` into the named variable. Errors raised by the target propagate to the caller.

<Warning>
The slug is the raw YAML **key**, and instruction keys are **not** interpolated — only values are. So `{{ myVar }}` as a key does **not** resolve to a slug; the automation to call must be a static, hardcoded slug. To dispatch dynamically (slug from a variable), either branch with [`conditions`](#conditions) over known slugs, `emit` an event whose type is built from an expression (async), or `fetch` the target automation's webhook URL (the URL is a value, so it can be interpolated).
</Warning>
...
# String matching — a bare pattern is a SUBSTRING test, not a regex.
# `matches "^hel"` looks for the literal characters `^hel` and finds nothing.
# Use regex(...) below when you need anchors, classes or quantifiers.
"hello" matches "hel"
...
# Anchors work, so you can require a shape rather than merely find one:
{{callbackUrl}} matches regex("/webhooks/[A-Za-z0-9_.-]*oauthCallback$")
```

The pattern is **not interpolated** — a `{{variable}}` written inside it is
matched as those literal characters. Interpolating one would be a trap
anyway: a value carrying `.` or `/` turns into wildcards, which is how a host
check quietly becomes an open redirect. To test a value against another
variable, compare the two directly (`'{{a}} == {{b}}'`), slicing first when
you need a prefix.
...
number("42")  # 42 (parses a string to integer)
...
| Key | Purpose |
|---|---|
| `conditions` | Dot-path map matched against the incoming payload. The first branch whose conditions all match is active; a branch with no `conditions` is the default. |
| `payload` | Schema describing the inputs for this branch (a JSON-Schema-like object). |
| `output` | Schema describing the output for this branch. |
...
The `someToken` field defined with `secret: true` is automatically redacted from native runtime events to avoid accidental leaks of sensitive information (see [Protecting sensitive fields](#protecting-sensitive-fields)).
...
- **Values are compared as strings after scalar coercion** — `version: 2` matches `?version=2`, since query parameters always arrive as strings. An object, an array, or a null condition value (YAML's valueless `key:`) never matches.
...
| Marker | Effect |
|---|---|
| `secret: true` | Value-based scrubbing. **Only string values are tracked** — the string is masked everywhere it surfaces downstream, across the whole automation call chain. Objects, arrays, and numbers are ignored. |
| `redact: true` | Path-based redaction. The field is stripped from runtime execution events (`runtime.automations.executed`), whatever its type or size. |
...
Schemas support various validation formats including date, url, time, password, etc. Validation errors immediately stop the current and parent automations.

When `validateArguments` is `true` and **no branch matches** the payload, the call is rejected. Declare an unconditioned branch (one with no `conditions`) to accept internal calls or unlisted HTTP methods. Output schemas are never validation-enforced — they only document the output and drive `redact`.
...
```yaml
# Legacy form — still valid, equivalent to a single-branch schema
legacyArguments:
  do: []
  name: legacyArguments
  arguments:
    someString:
      type: string
    someToken:
