...
Once triggered, automations execute a sequence of instructions in order. Here are some of the core instructions available:
...
<CardGroup cols={2}>
  <Card title="Condition" icon="code-branch">
    ...
    [More details on condition syntax](/conditions)
  </Card>
...
<Tabs>
  ...
  <Tab title="Multi-LLM Orchestration">
    ...
    do:
      # Categorize feedback
      - fetch:
          url: "{{config.llmApiUrl}}/completions"
          method: POST
          body:
            model: "gpt-3.5-turbo"
            messages:
...
<CardGroup cols={3}>
  ...
  >
    Discover how to create interfaces that
  </Card>
    <Card
    title="Blocks"
    icon="puzzle-piece"
