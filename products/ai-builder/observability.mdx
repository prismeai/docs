...
Next to the period selector, an organization selector narrows every tab to a single organization, so the metrics you read describe one organization's traffic on this workspace rather than all of it combined. Leave it unset to keep the workspace-wide view.

This matters on workspaces serving several organizations: an error rate or a latency percentile averaged across all of them hides the one organization actually having a bad time.
