---
title: 'Versioning'
description: 'Synchronize workspaces with Git repositories, manage versions, and handle merge conflicts'
---

Everything you build on the foundation from Builder is versioned as code, Infrastructure-as-Code for AI, much like Terraform. Prisme.ai workspaces can therefore be synchronized with external Git repositories, enabling collaborative workflows between the platform and standard Git tooling. Changes made on either side are merged transparently using Git's built-in merge capabilities.
...
<Warning>
  **Azure DevOps is not compatible with the `auth.user` / `auth.password` fields.** Use one of these two workarounds instead:

  - **SSH keys** (recommended; see the SSH example below), or
  - **Credentials embedded directly in the URL**: set `config.url` to `https://<user>:<pat>@dev.azure.com/<org>/<project>/_git/<repo>` and leave the `auth` block empty. URL-encode any special character in the PAT.

  Azure DevOps issues HTTP redirects on git operations (e.g. `dev.azure.com` → `<org>.visualstudio.com`), and git drops HTTP auth headers across redirects for security reasons. Prisme injects credentials as a header, so the follow-up request goes out unauthenticated and fails with `Missing username or token`. Credentials embedded in the URL are re-sent on redirects, which is why they work.

  GitHub / GitLab / Bitbucket don't redirect on git operations, so the standard `auth.user` / `auth.password` fields work there.
</Warning>
...
Versioning saves the workspace's static configuration, including security roles, automations, page source files, imports, and apps, but not dynamic data like events, collections, or uploaded files.

<CardGroup cols={2}>
  <Card title="Included in Versioning" icon="check-circle">
    <p>Workspace components that are saved and deployed:</p>
    <ul>
      <li>Page source files and configuration</li>
      <li>React components and frontend assets stored in the page source tree</li>
...
The **Settings → Versioning** tab is where everything documented above is exposed as buttons. It is also the only place where the **Export** / **Import** / **Import (Prune)** UI lives (the underlying APIs are the archive endpoints below).

### The five buttons

| Button | What it does | Underlying API |
|---|---|---|
| **Push** | Opens the Push form: pick a target repository (any `read-write` or `write-only` repository, including the native `Prismeai`), add an optional description, and push the current workspace state as a new version. | `POST /v2/workspaces/{id}/push` |
| **Pull** | Opens the Pull form: pick a source repository, pick the version to restore from the list returned by the repository, optionally tick **Discard local changes**, then pull. | `POST /v2/workspaces/{id}/pull` |
| **Export** | Triggers an immediate browser download of the workspace as a `.zip` archive (DSUL, security, automations, pages, imports). | `GET /v2/workspaces/{id}/export` |
| **Import** | Opens a file picker for a workspace `.zip`. Files in the archive are merged into the workspace: keys that exist on both sides are overwritten with the archive's content; keys that exist only locally are kept. | `POST /v2/workspaces/{id}/import` |
| **Import (Prune)** | Same as Import, but also **deletes** any workspace object that is not present in the archive. Use it to make the workspace bit-for-bit identical to the archive. | `POST /v2/workspaces/{id}/import?prune=true` |

### Discard local changes

The **Discard local changes** checkbox on the Pull form skips the safety merge: the platform pulls the chosen version on top of the current state without committing local edits first. Anything not saved to Git will be lost. Use it when you knowingly want to roll back to a known-good version after a failed change.

### Write lock indicator

If another user has triggered a long-running pull, push, or bulk operation, the tab shows a **Workspace locked** banner with the locking user and the lock reason. Versioning operations are disabled while the lock is held; an **Unlock** button (visible to users with `ManageSecurity`) calls `DELETE /v2/workspaces/{id}/writeLock`.

### Import vs Import (Prune): worked example

Workspace before:
```
automations: send-welcome, daily-digest, archive
pages: home, dashboard
```

Archive contains:
```
automations: send-welcome (updated), daily-digest (unchanged)
pages: home (updated)
```

- **Import** result:
  ```
  automations: send-welcome (updated), daily-digest (unchanged), archive (unchanged)
  pages: home (updated), dashboard (unchanged)
  ```
- **Import (Prune)** result:
  ```
  automations: send-welcome (updated), daily-digest (unchanged)
  pages: home (updated)
  ```
  `archive` and `dashboard` were removed because they are absent from the archive.

`Prune` operations emit a `workspaces.imported` event whose `deleted` array lists everything that was removed; see [Import Results](#import-results).
...
Both can also be triggered from each workspace versioning UI after configuring  [`repositories` section](#repository-Configuration)
...
Both can also be triggered from the Platform workspace versioning UI, after configuring [platform repositories](/self-hosting/kubernetes/environment-variables#platform-repositories) and [workspace groups](/self-hosting/kubernetes/environment-variables#workspace-groups).  
...
<CardGroup cols="3">
  ...
  <Card title="Workspaces" icon="cubes" href="/products/ai-builder/workspaces">
    Workspace configuration, secrets, and events
  </Card>
