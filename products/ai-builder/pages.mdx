---
title: 'Pages'
description: 'Create dynamic user interfaces by assembling blocks into complete pages in AI Builder'
---

<Frame>
  <img src="/images/pages-hero.png" alt="AI Builder Pages Interface" />
</Frame>

Pages in AI Builder are the containers where blocks come together to create complete user interfaces. They define the structure, layout, and navigation of your application, providing the environments where users interact with your AI solutions.

## Understanding Pages

<Tabs>
  <Tab title="What are Pages?">
    <Frame>
      <img src="/images/page-concepts.png" alt="Page Concepts" />
    </Frame>
    
    Pages are complete views composed of blocks and define:
    
    - **Structure**: The organization and hierarchy of content
    - **Layout**: How blocks are arranged visually
    - **Navigation**: How users move between different views
    - **Interaction Model**: How users engage with the interface
    - **Access Control**: Who can view and interact with the page
    
    Each page has a unique URL within your workspace and can be configured for various use cases.
  </Tab>
  
  <Tab title="Page Types">
    <Frame>
      <img src="/images/page-types.png" alt="Page Types" />
    </Frame>
    
    AI Builder supports several page types for different purposes:
    
    - **Standard Pages**: General-purpose interfaces
    - **_doc Pages**: Documentation pages with special formatting
    - **index Pages**: Home/landing pages for workspaces
    - **401/404 Pages**: Custom error and authentication pages
    - **Template Pages**: Reusable page structures
    
    Different page types have specific behaviors and optimizations.
  </Tab>
  
  <Tab title="Page Architecture">
    <Frame>
      <img src="/images/page-architecture.png" alt="Page Architecture" />
    </Frame>
    
    Pages in AI Builder are built on modern web technologies:
    
    - **Next.js**: For server-side rendering and optimal performance
    - **React**: For component-based UI development
    - **Event System**: For communication between components
    - **Responsive Design**: For cross-device compatibility
    
    This architecture ensures fast loading, SEO-friendly content, and interactive experiences.
  </Tab>
</Tabs>

## Working with Pages

<Steps>
  <Step title="Creating Pages">
    Start a new page through the AI Builder interface:
    
    <Frame>
      <img src="/images/creating-pages.png" alt="Creating Pages" />
    </Frame>
    
    To create a page:
    1. Navigate to the "Pages" folder in your workspace
    2. Click the "+" button to add a new page
    3. Choose a page type (standard, _doc, etc.)
    4. Enter a unique slug for the page URL
    5. Provide a name and description
    
    The page slug becomes part of the URL: `https://[workspace-slug].pages.prisme.ai/[language]/[page-slug]`
  </Step>
  
  <Step title="Page Properties">
    Configure the basic properties of your page:
    
    <Frame>
      <img src="/images/page-properties.png" alt="Page Properties" />
    </Frame>
    
    Key properties include:
    - **Slug**: Unique identifier in the URL (technical name)
    - **Name**: Human-readable title displayed to users
    - **Description**: Short explanation of the page's purpose
    - **Access Control**: Public, private, or specific user access
    - **Language**: Default language for localization
    - **SEO Settings**: Title, description, and metadata for search engines
  </Step>
  
  <Step title="Adding Blocks">
    Populate your page with blocks to create the user interface:
    
    <Frame>
      <img src="/images/adding-blocks-to-pages.png" alt="Adding Blocks to Pages" />
    </Frame>
    
    The process involves:
    1. Select from available blocks in the sidebar
    2. Drag blocks onto the page canvas
    3. Arrange blocks in the desired layout
    4. Configure block properties for this specific instance
    5. Create event connections between blocks
    
    Blocks can be nested, grouped, and arranged to create complex layouts.
  </Step>
  
  <Step title="Page Layout">
    Organize blocks into a cohesive layout:
    
    <Frame>
      <img src="/images/page-layout.png" alt="Page Layout" />
    </Frame>
    
    Layout approaches include:
    - **Grid System**: Arrange blocks in rows and columns
    - **Flex Layout**: Create flexible, responsive arrangements
    - **Container Blocks**: Group related blocks together
    - **Responsive Design**: Adjust layouts for different screen sizes
    - **Spacing Controls**: Manage margins and padding between elements
    
    The right layout ensures content is presented clearly and functions well on all devices.
  </Step>
  
  <Step title="Page Styling">
    Customize the visual appearance of your page:
    
    <Frame>
      <img src="/images/page-styling.png" alt="Page Styling" />
    </Frame>
    
    Styling options include:
    - **CSS Editor**: Write custom CSS for precise control
    - **Theme Variables**: Use consistent styling across pages
    - **Style Presets**: Apply predefined style combinations
    - **Dark/Light Mode**: Support different color schemes
    - **Typography Settings**: Control text appearance
    
    Proper styling creates a professional, branded appearance.
  </Step>
  
  <Step title="Event Configuration">
    Set up communication between blocks on the page:
    
    <Frame>
      <img src="/images/page-event-configuration.png" alt="Page Event Configuration" />
    </Frame>
    
    Event configuration includes:
    - Creating event flows between blocks
    - Setting up page-level events
    - Connecting blocks to backend automations
    - Handling user interactions
    - Managing data flow through the interface
    
    The event system creates interactive, dynamic experiences.
  </Step>
  
  <Step title="Testing & Preview">
    Validate your page before publishing:
    
    <Frame>
      <img src="/images/page-testing.png" alt="Page Testing" />
    </Frame>
    
    Testing options include:
    - **Live Preview**: See changes in real-time
    - **Device Preview**: Test on different screen sizes
    - **Event Monitoring**: Verify event flows work correctly
    - **User Testing**: Observe real users interacting with the page
    - **Accessibility Checking**: Ensure the page works for all users
    
    Thorough testing ensures a quality user experience.
  </Step>
</Steps>

## Page Features and Capabilities

<AccordionGroup>
  <Accordion title="Navigation Systems">
    <Frame>
      <img src="/images/page-navigation.png" alt="Page Navigation" />
    </Frame>
    
    Pages can include various navigation patterns:
    
    - **Menu Systems**: Structured navigation between pages
    - **Breadcrumbs**: Show location in page hierarchy
    - **Tabs**: Organize content within a page
    - **Links**: Connect to other pages and external resources
    - **Routing**: Define URL patterns and parameters
    
    <Properties>
      <Property name="URL Structure" value="https://[workspace-slug].pages.prisme.ai/[language]/[page-slug]">
        Example: https://my-workspace.pages.prisme.ai/en/customer-dashboard
      </Property>
      <Property name="Query Parameters" value="Support for URL parameters to control page state">
        Example: ?view=details&id=12345
      </Property>
      <Property name="Navigation State" value="Saved and restored across sessions">
        Features: Browser history integration, deep linking, state persistence
      </Property>
    </Properties>
  </Accordion>
  
  <Accordion title="Access Control">
    <Frame>
      <img src="/images/page-access-control.png" alt="Page Access Control" />
    </Frame>
    
    Pages support multiple access control models:
    
    - **Public Pages**: Accessible to anyone with the URL
    - **Private Pages**: Require authentication
    - **Role-Based Access**: Available to specific user roles
    - **Email-Based Access**: Shared with specific email addresses
    - **SSO Integration**: Enterprise authentication systems
    
    <Properties>
      <Property name="Authentication Methods" value="Multiple identity verification approaches">
        Supported: SSO, email/password, social login, JWT, API key
      </Property>
      <Property name="Authorization Rules" value="Fine-grained access control">
        Options: Role-based, attribute-based, explicit user lists
      </Property>
      <Property name="Access Inheritance" value="Hierarchical permission model">
        Behavior: Child resources inherit parent permissions by default
      </Property>
    </Properties>
  </Accordion>
  
  <Accordion title="Page Events">
    <Frame>
      <img src="/images/page-events.png" alt="Page Events" />
    </Frame>
    
    Pages can emit and listen for various events:
    
    - **Lifecycle Events**: Load, mount, unmount
    - **User Interaction Events**: Clicks, form submissions
    - **State Change Events**: Data updates, selection changes
    - **System Events**: Authentication, errors, notifications
    - **Custom Events**: Application-specific events
    
    <Properties>
      <Property name="Event Flow" value="Communication patterns between components">
        Patterns: Parent-child, sibling, cross-page, page-automation
      </Property>
      <Property name="Event Payload" value="Data transmitted with events">
        Structure: JSON objects with typed properties
      </Property>
      <Property name="Event Handling" value="Response to received events">
        Options: State updates, UI changes, automation triggers
      </Property>
    </Properties>
  </Accordion>
  
  <Accordion title="State Management">
    <Frame>
      <img src="/images/page-state-management.png" alt="Page State Management" />
    </Frame>
    
    Pages maintain state at various levels:
    
    - **Page State**: Shared across blocks on the page
    - **URL State**: Preserved in the URL for bookmarking and sharing
    - **Session State**: Persisted during the user's session
    - **User State**: Maintained across sessions for the same user
    - **Application State**: Shared across pages and components
    
    <Properties>
      <Property name="State Storage" value="Where state data is maintained">
        Options: Memory, localStorage, sessionStorage, database, URL
      </Property>
      <Property name="State Access" value="How components interact with state">
        Methods: Context providers, state managers, event updates
      </Property>
      <Property name="State Persistence" value="How long state is maintained">
        Types: Transient, session, user, permanent
      </Property>
    </Properties>
  </Accordion>
  
  <Accordion title="Localization">
    <Frame>
      <img src="/images/page-localization.png" alt="Page Localization" />
    </Frame>
    
    Pages support multi-language content:
    
    - **Text Translation**: Content in multiple languages
    - **Language Selection**: User-controlled language preference
    - **RTL Support**: Right-to-left language accommodation
    - **Date/Number Formatting**: Locale-specific formatting
    - **Content Adaptation**: Cultural and regional variations
    
    <Properties>
      <Property name="Language Path" value="URL segment indicating language">
        Example: /en/ for English, /fr/ for French
      </Property>
      <Property name="Translation System" value="How content is translated">
        Methods: JSON files, CMS integration, dynamic translation
      </Property>
      <Property name="Default Language" value="Fallback when preferred language is unavailable">
        Behavior: Configurable at workspace or page level
      </Property>
    </Properties>
  </Accordion>
</AccordionGroup>

## Page Templates

AI Builder provides several page templates to accelerate development:

<CardGroup cols={2}>
  <Card title="Dashboard Template" icon="gauge">
    <p>Structured layout for data visualization and metrics</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Card-based metrics display</li>
      <li>Multiple chart integrations</li>
      <li>Filterable data tables</li>
      <li>Action panels and quick access controls</li>
    </ul>
  </Card>
  
  <Card title="Agent Interface Template" icon="robot">
    <p>Optimized for AI agent interactions and conversations</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Chat interface with history</li>
      <li>Document upload and display</li>
      <li>Context panel for additional information</li>
      <li>Feedback collection mechanisms</li>
    </ul>
  </Card>
  
  <Card title="Admin Portal Template" icon="gear">
    <p>Management interface for system configuration</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Navigation sidebar with sections</li>
      <li>Configuration forms and editors</li>
      <li>User management tools</li>
      <li>System status indicators</li>
    </ul>
  </Card>
  
  <Card title="Data Entry Template" icon="keyboard">
    <p>Form-focused layout for information collection</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Multi-step form workflow</li>
      <li>Field validation and error handling</li>
      <li>Progress tracking</li>
      <li>Save and resume capability</li>
    </ul>
  </Card>
  
  <Card title="Knowledge Base Template" icon="book">
    <p>Documentation and information repository layout</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Category navigation</li>
      <li>Search functionality</li>
      <li>Article display with table of contents</li>
      <li>Related content suggestions</li>
    </ul>
  </Card>
  
  <Card title="Analytics Template" icon="chart-pie">
    <p>Comprehensive data analysis and reporting interface</p>
    <p><strong>Key Features:</strong></p>
    <ul>
      <li>Multiple visualization types</li>
      <li>Filter and dimension controls</li>
      <li>Date range selection</li>
      <li>Export and sharing options</li>
    </ul>
  </Card>
</CardGroup>

## Page Composition Patterns

Effective pages combine blocks according to established design patterns:

<Tabs>
  <Tab title="Master-Detail">
    <Frame>
      <img src="/images/master-detail-pattern.png" alt="Master-Detail Pattern" />
    </Frame>
    
    A two-panel layout showing a list of items and detailed information:
    
    **Implementation:**
    - List block on one side (master)
    - Detail display on the other side (detail)
    - Selection events connect the two
    - Updates detail when master selection changes
    
    **Common Uses:**
    - Email clients
    - Document management
    - Contact directories
    - Product catalogs
  </Tab>
  
  <Tab title="Dashboard">
    <Frame>
      <img src="/images/dashboard-pattern.png" alt="Dashboard Pattern" />
    </Frame>
    
    A grid of information cards and visualizations:
    
    **Implementation:**
    - Grid or flex container as the base
    - Card blocks for individual metrics
    - Chart blocks for visualizations
    - Filter blocks for data control
    
    **Common Uses:**
    - Business intelligence
    - System monitoring
    - Performance tracking
    - Executive summaries
  </Tab>
  
  <Tab title="Wizard">
    <Frame>
      <img src="/images/wizard-pattern.png" alt="Wizard Pattern" />
    </Frame>
    
    A step-by-step sequence of forms or inputs:
    
    **Implementation:**
    - Container with step indicator
    - Form blocks for each step
    - Navigation buttons between steps
    - Progress tracking
    
    **Common Uses:**
    - Setup processes
    - Multi-step forms
    - Guided workflows
    - Onboarding sequences
  </Tab>
  
  <Tab title="Conversation">
    <Frame>
      <img src="/images/conversation-pattern.png" alt="Conversation Pattern" />
    </Frame>
    
    An interface focused on back-and-forth communication:
    
    **Implementation:**
    - Chat container block
    - Message display for conversation history
    - Input block for user messages
    - Optional context panel or sidebar
    
    **Common Uses:**
    - AI assistants
    - Customer support
    - Collaborative tools
    - Messaging applications
  </Tab>
</Tabs>

## Advanced Page Features

<AccordionGroup>
  <Accordion title="Dynamic Page Generation">
    <Frame>
      <img src="/images/dynamic-page-generation.png" alt="Dynamic Page Generation" />
    </Frame>
    
    Pages can be dynamically generated based on data:
    
    - **Template Pages**: Base structures with variable content
    - **Data-Driven Pages**: Content populated from databases
    - **Parameter-Based Rendering**: Pages that adapt based on URL parameters
    - **Conditional Content**: Sections that appear based on rules
    
    This enables scalable content management and personalized experiences.
  </Accordion>
  
  <Accordion title="Page Analytics">
    <Frame>
      <img src="/images/page-analytics.png" alt="Page Analytics" />
    </Frame>
    
    Track how users interact with your pages:
    
    - **Page Views**: Count and analyze page visits
    - **Interaction Tracking**: Monitor user engagement with components
    - **Conversion Metrics**: Measure completion of key actions
    - **User Behavior Analysis**: Understand navigation patterns
    
    Analytics provide insights for continuous improvement.
  </Accordion>
  
  <Accordion title="Page Optimization">
    <Frame>
      <img src="/images/page-optimization.png" alt="Page Optimization" />
    </Frame>
    
    Techniques to improve page performance:
    
    - **Lazy Loading**: Load content only when needed
    - **Code Splitting**: Divide code into smaller chunks
    - **Image Optimization**: Serve appropriately sized images
    - **Caching Strategies**: Store data for faster access
    
    Optimization ensures fast, responsive user experiences.
  </Accordion>
  
  <Accordion title="Advanced Styling">
    <Frame>
      <img src="/images/advanced-styling.png" alt="Advanced Styling" />
    </Frame>
    
    Sophisticated appearance customization:
    
    - **Theme Systems**: Comprehensive styling frameworks
    - **CSS Variables**: Dynamic style properties
    - **Style Inheritance**: Cascading style application
    - **Responsive Breakpoints**: Screen-size specific styling
    
    Advanced styling creates polished, branded interfaces.
  </Accordion>
</AccordionGroup>

## Page Integration with Automations

Pages connect with backend processes through the event system:

<Steps>
  <Step title="Event Triggers">
    Page components can trigger automations:
    
    <Frame>
      <img src="/images/event-triggers.png" alt="Event Triggers" />
    </Frame>
    
    Common triggers include:
    - Button clicks
    - Form submissions
    - Data selection
    - Page loading
    - Scheduled intervals
  </Step>
  
  <Step title="Data Passing">
    Information flows between pages and automations:
    
    <Frame>
      <img src="/images/data-passing.png" alt="Data Passing" />
    </Frame>
    
    Data passing methods:
    - Event payloads with structured data
    - State variables and context
    - URL parameters and query strings
    - Session and user storage
  </Step>
  
  <Step title="Response Handling">
    Pages react to automation results:
    
    <Frame>
      <img src="/images/response-handling.png" alt="Response Handling" />
    </Frame>
    
    Response handling patterns:
    - Update UI components with returned data
    - Display success/error messages
    - Navigate to new views based on results
    - Update page state with processed information
  </Step>
  
  <Step title="Asynchronous Processing">
    Handle long-running operations gracefully:
    
    <Frame>
      <img src="/images/asynchronous-processing.png" alt="Asynchronous Processing" />
    </Frame>
    
    Asynchronous patterns:
    - Loading states during processing
    - Progress indicators for lengthy operations
    - Background processing with notifications
    - Polling for updates on long-running tasks
  </Step>
</Steps>

## Page Best Practices

<CardGroup cols={2}>
  <Card title="Consistent Layout" icon="ruler">
    <p>Maintain consistency across your interface:</p>
    <ul>
      <li>Use a grid system for alignment</li>
      <li>Establish consistent spacing</li>
      <li>Maintain a visual hierarchy</li>
      <li>Create recognizable patterns</li>
    </ul>
  </Card>
  
  <Card title="Responsive Design" icon="mobile-screen">
    <p>Ensure pages work on all devices:</p>
    <ul>
      <li>Test on multiple screen sizes</li>
      <li>Use flexible layouts</li>
      <li>Prioritize content for mobile</li>
      <li>Consider touch vs. mouse interactions</li>
    </ul>
  </Card>
  
  <Card title="Performance Focus" icon="gauge-high">
    <p>Optimize for speed and responsiveness:</p>
    <ul>
      <li>Minimize the number of blocks</li>
      <li>Use lazy loading where appropriate</li>
      <li>Optimize images and media</li>
      <li>Monitor and improve loading times</li>
    </ul>
  </Card>
  
  <Card title="Accessibility" icon="universal-access">
    <p>Create inclusive experiences for all users:</p>
    <ul>
      <li>Use semantic HTML structure</li>
      <li>Ensure keyboard navigation</li>
      <li>Maintain sufficient color contrast</li>
      <li>Add appropriate ARIA attributes</li>
    </ul>
  </Card>
  
  <Card title="Progressive Disclosure" icon="layer-group">
    <p>Reveal information progressively to avoid overwhelming users:</p>
    <ul>
      <li>Start with essential information</li>
      <li>Hide details until needed</li>
      <li>Use expandable sections</li>
      <li>Create logical information hierarchy</li>
    </ul>
  </Card>
  
  <Card title="Error Handling" icon="triangle-exclamation">
    <p>Design for when things go wrong:</p>
    <ul>
      <li>Provide clear error messages</li>
      <li>Offer recovery paths</li>
      <li>Preserve user input during errors</li>
      <li>Test error scenarios thoroughly</li>
    </ul>
  </Card>
</CardGroup>

## Next Steps

<CardGroup cols={2}>
  <Card
    title="Blocks"
    icon="puzzle-piece"
    href="/products/ai-builder/blocks"
  >
    Learn more about the components that make up pages
  </Card>
  <Card
    title="Automations"
    icon="gears"
    href="/products/ai-builder/automations"
  >
    Explore backend processes that power page functionality
  </Card>
  <Card
    title="Testing & Debugging"
    icon="vial"
    href="/products/ai-builder/testing-debugging"
  >
    Learn how to validate and troubleshoot pages
  </Card>
  <Card
    title="Use Cases"
    icon="lightbulb"
    href="/products/ai-builder/use-cases"
  >
    See examples of pages in real-world applications
  </Card>
</CardGroup>