...
Pages are the React front-ends you build on the foundation from Builder, the same place you build agents and agentic workflows, so the interface and the logic behind it stay one fluid, integrated experience. In the current Builder, a page is managed as a React application source tree, not as a block canvas. You edit files, preview the compiled app, and connect the interface to workspace automations through HTTP endpoints, WebSocket events, and the injected Prisme.ai SDK.
