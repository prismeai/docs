...
An alternative scenario without building docker image could be to bulk import the source git branch directly into the destination environment.  
However, the scenario described here have the following adventages :  
...
<CardGroup cols={3}>
  <Card
    title="Versioning"
    icon="code-branch"
    href="/products/ai-builder/versioning"
  >
    Git synchronization, merge conflicts, and bulk operations
  </Card>
  <Card
    title="RBAC"
    icon="user-lock"
