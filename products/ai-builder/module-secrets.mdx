...
| `action`: only `create` / `update` / `read` | `manage` is refused outright, and `delete` is grantable only to a role (below), never on a rule applying to every member. `read` is accepted here, but only ever makes sense on a role-carried rule: granting it to everyone would let each member read all the others' credentials. |
    ...
    - role: tokenResolver
      ...
      conditions:
        name:
          $regex: '^figRefresh_'
      reason: |
        The scheduled sync replays any connected member's token, stores the
        rotated one the provider hands back, and drops one rejected for good.
```
