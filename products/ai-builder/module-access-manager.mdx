...
| `action` | string | no | Action to check (typically `read`, `write`, `share`, `delete`, `manage`, but any string is accepted; it must match the keys in `roles[*].permissions`). Must be set together with `resourceType`. |
