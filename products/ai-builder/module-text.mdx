...
Percent-encodes a single URI component, exactly like JavaScript's `encodeURIComponent`.

`URLSearchParams(...).asString` already encodes, but only as `key=value` pairs and with `+` for spaces. A lone path segment has no other native way through — a namespaced project path such as `group/sub/repo` that must travel as `group%2Fsub%2Frepo`, for instance.
...
- run:
    ...
    parameters:
      value: "{{project.path}}"
    output: encoded
# encoded → "group%2Fsub%2Frepo"
```

| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| value | string | yes | | Value to encode. Non-strings are coerced; `null`/`undefined` yield `""`. |

Returns the encoded **string**. Everything outside `A-Z a-z 0-9 - _ . ! ~ * ' ( )` is escaped, and a space becomes `%20`, not `+`.

## removeDiacritics

Strips diacritical marks from text, leaving the base letters. Case is preserved, and text with nothing to fold comes back unchanged.
...
- run:
    ...
    parameters:
      text: "{{file.name}}"
    output: folded
# "Éléphant à Nîmes" → "Elephant a Nimes"
```

| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| text | string | yes | | Text to fold. Non-strings are coerced; `null`/`undefined` yield `""`. |
| compatibility | boolean | no | false | Also apply compatibility decomposition (NFKD), which folds ligatures and presentation forms: `ﬁ` → `fi`, `²` → `2`. Off by default so removing an accent stays the only visible effect. |

Two passes, because neither alone suffices. Unicode decomposition splits a precomposed letter into base + combining mark and every non-spacing mark is dropped, which covers Latin, Greek and Cyrillic without enumerating anything. A small table then handles the letters that have *no* decomposition — a stroke or a bar is part of the glyph, not a combining mark, so `ø`, `đ`, `ł`, `ħ` survive the first pass untouched; ligatures that are letters in their own right (`æ`, `œ`, `ß`) are expanded here too.

A typical use is normalising a filename before upload: macOS stores filenames in NFD, and services that store an upload under one normalisation but reference it under another will 404 on accented names.
