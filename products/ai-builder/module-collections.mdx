...
Runtime modules are the built-in capabilities you call from automations on the foundation, and the **collections** module is one of them. It provides structured data storage and querying for automations, supporting both MongoDB and SQL backends, with automatic namespace isolation per workspace and app.
