...
Builder is the full-code door onto the foundation, and Custom Code is where that goes all the way to the metal. The **Custom Code** app lets you embed JavaScript (Node.js) or Python functions inside any [automation](./automations). Reach for it when YAML instructions get awkward: complex transforms, regex parsing, signature computation, anything that's faster to write as a few lines of code.
...
![Builder Imports section expanded with Custom Code installed](/images/howto/custom-code-imports.png)
...
![Custom Code app configuration with the functions explorer on the left and the simplifyDocument function open in the Monaco editor on the right](/images/howto/custom-code-config.png)
...
![Activity tab showing recent runs of process-document and the Custom Code app, with one row expanded to reveal the JSON payload](/images/howto/custom-code-activity.png)
