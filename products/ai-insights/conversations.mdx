...
The **Conversations** page is the per-agent inventory of every conversation Insights knows about, with an analysis-status filter, message-level inspection, and on-demand analyze controls. Because every interaction on the foundation is captured as a traceable event, this inventory fills itself, nothing here is imported or instrumented by hand.
...
<Note>
Access to this tab is governed by the platform RBAC: seeing an agent in Insights does not by itself grant access to its conversations. Your org admin grants the permission through Governe, as described in [Getting started](./getting-started).
</Note>
...
The table is scoped by the [date filter](/products/ai-insights/dashboard#the-date-filter) shared with the organization dashboard and the agent Overview. Here the window applies to `last_message_at`, the date shown in the table, so a conversation that was analyzed inside the window but last active before it counts on the agent Overview without appearing in this list.
