...
When you click an agent on the Insights dashboard (or any agent card elsewhere), the sidebar swaps to the agent context and lands on **Overview**. This is where the fleet-wide view narrows to a single agent, answering one question: *how is this one agent doing?*
...
The header carries the same [date filter](/products/ai-insights/dashboard#the-date-filter) as the organization dashboard, presets of 7, 30 or 90 days or a custom range, and the selection follows you from the dashboard to this page and on to [Conversations](/products/ai-insights/conversations). Here the window applies to `analyzed_at`, when the analysis ran, not to when the conversation was last active.
