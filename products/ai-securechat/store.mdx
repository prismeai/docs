...
The Store is where you discover and consume the agents that live in Chat. It is your gateway to browsing every AI agent available to you, then starting a conversation with one in the same secure experience. Access it from the **Use** menu in the sidebar by clicking **Store**.
...
**Search**: Use the search bar to find agents by name or description. The **Available agents** tab of the chat Home page carries the same search: it queries the whole catalog server side rather than filtering the agents already loaded, so an agent that sits far down the list is one search away, and **Load more** keeps paginating within the search you typed.
