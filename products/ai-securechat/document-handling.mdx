...
description: 'Learn how to analyze, process, and work with documents in Chat'
---

One of Chat's most powerful features is its ability to process and analyze documents, so the agents you already work with in Chat can reason over your own files. This guide explains how to effectively work with documents to extract insights, answer questions, and leverage information from your files.
...
<Steps>
  <Step title="Access the upload function">
    ...
    <Frame>
      <img src="/images/chat-upload-file-picker.png" alt="Chat file picker open over the conversation view" />
    </Frame>
  </Step>
...
{/* TODO screenshot: Chat conversation with an OCR-extracted document open (e.g. scanned PDF on the left, extracted text on the right). Save as /images/chat-ocr-example.png. */}

Chat automatically extracts text from **images** sent in the conversation; they are routed to a vision model that reads their content. This works for:
...
<Warning>
  **Scanned PDF documents are different.** A PDF without a selectable text layer (a pure scan) cannot be read by the standard document parser: its indexing fails and Chat tells the user the document could not be processed. To let the agent recover from this automatically, enable an **OCR capability** on the agent; see [Enable OCR for scanned documents](#enable-ocr-for-scanned-documents-administrators) below.
</Warning>
...
### Enable OCR for scanned documents (administrators)

When a user attaches a scanned PDF, indexing fails with an "empty content" error (`DropItem` / `EmptyContent`). You can give the agent a recovery path so it reads the document through OCR and still answers the user's request. Three pieces work together:

<Steps>
  <Step title="An OCR capability (MCP connector)">
    The capability store ships an **Azure OCR** connector (Computer Vision for images, Document Intelligence for scanned documents). Install the connector app in a workspace, configure its Azure credentials through its configuration app, and register (or reuse) the capability entry pointing at its MCP endpoint. Any other OCR provider can be plugged the same way: publish an MCP connector and add one capability entry to the store, no core change needed.
  </Step>
  <Step title="The capability on the agent">
    Add the OCR MCP tool to the agent: from Agent Creator, or in the Secure-chat Agent workspace under `config.value.agent.tools`. The agent must also be allow-listed in the connector configuration, which validates the caller identity on every tool call.
  </Step>
  <Step title="A recovery skill">
    Add a skill that tells the agent what to do when an attached file failed indexing: call the OCR tool with the attachment URL and answer the original request from the extracted text; or, if OCR is unavailable or fails too, explain precisely why the document is unreadable and what the user can do (re-scan with OCR, unprotected PDF, paste the text).
  </Step>
</Steps>

<Tip>
  If the agent's instructions contain an explicit skill map ("activate skill X for task Y"), add the recovery skill to that map and mark it as **taking priority whenever an attached file failed processing**; otherwise the model keeps routing summary-like requests ("what is this document about?") to its regular research/summary skill, which cannot do anything with an unreadable document.
</Tip>
...
<Accordion title="Text recognition issues">
  ...
  - If scanned PDFs systematically fail with a "could not be processed" message, enable the OCR capability on the agent (see [Enable OCR for scanned documents](#enable-ocr-for-scanned-documents-administrators))
</Accordion>
