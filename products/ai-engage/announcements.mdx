...
On the Home page, active announcements are shown as cards, ordered **most recent first** rather than in the order the API returned them, so a new announcement is never pushed out of view by an older one.

Each card carries:

- **Where it comes from**, so a reader can tell a platform-wide message from one published by their own organization.
