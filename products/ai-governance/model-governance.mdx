...
| Anthropic | Claude Sonnet 4.5, Claude 3.7 Sonnet, Claude 3.5 Sonnet | Native API; optional custom `endpoint` for self-hosted / compatible / non-standard Azure hosts |
...
<Note>
Like the `openailike` headers above, this is configured in the workspace that owns the provider config — **LLM Gateway** (`config.value.providers.anthropic.endpoint`). The OpenAI app instance is a pass-through and is **not** the configuration surface.
</Note>
...
- **Authentication is unchanged** — requests still send the standard `x-api-key` + `anthropic-version` headers. This provider does not support the custom-header mechanism above, so the host must accept Anthropic-standard `x-api-key` auth.
