...
The Capabilities Catalog is where Governe defines, once and centrally, what every agent on the platform is allowed to reach. It is a centralized registry of tools and capabilities available to AI agents in your organization, including MCP servers, functions, file search, skills, guardrails, memory providers, and sub-agents. Whatever an agent can do, it does through a capability declared here and constrained by the guardrails around it.
...
The Capabilities tab reads and writes capabilities as JSON. Select the ones you want with the multiselect and **Export** them to a file, or **Import** a file to add them to the organization. It is the same mechanism as on [Join Rules](/products/ai-governance/identity-access#importing-and-exporting-rules) and Agents Controls: a catalog assembled and tested in one organization or environment can be replayed in another rather than rebuilt by hand.
...
When each end user must log in personally, via OAuth (Notion, GitHub, Outlook…) or a per-user API key, switch to the **per-user authentication** pattern below.
