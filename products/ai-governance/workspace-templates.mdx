...
Templates are declared at the organization level via the **Menu Editor**, so publishing one is a governance task, while the source workspace itself is built and maintained through Builder's full-code door. In other words, the same unified foundation lets governance publish a reusable starting point that builders then fork and adapt. See [Builder](/products/ai-builder/workspaces) for the source-workspace side.
...
- **Mark it as a template**, add the `template` label to the source workspace (workspace settings → labels). This is what authorizes users who don't otherwise have access to the source to duplicate it. Without it, the duplicate call is denied for anyone but the workspace's own collaborators, and the "Start from template" tile fails silently.

Note the **workspace id** (visible in the URL when you open the workspace, e.g. `kFoNvfX`); you'll need it in step 2.

<Info>
The `template` label acts as a public-duplication grant. When a user picks the template, the platform reads the source in elevated (superAdmin) mode **only because** it carries this label, the resulting workspace is still owned by the cloning user, with default security (the source's collaborators and custom roles are **not** inherited). Remove the label to immediately stop new duplications without touching the source content.
</Info>
...
| **Template ID** | The workspace **id or slug** of the source template (the one from step 1). |
...
6. **Save Changes**. The **Template ID** is validated on save: if it doesn't reference an existing workspace, the feature is rejected instead of publishing a tile that would fail at instantiation.
